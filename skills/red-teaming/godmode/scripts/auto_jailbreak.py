...
MODEL_STRATEGIES = {
    ...
    # Nous/Lil Skrrt models — already uncensored, just needs clean prompt
    "hermes": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _get_current_model() -> tuple:
    """Read current model and provider from Lil Skrrt config.yaml.
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Lil Skrrt for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Lil Skrrt for changes to take effect.")

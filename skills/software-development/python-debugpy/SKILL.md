...
- A long-running process (lil-skrrt gateway, tui_gateway) misbehaves and you can't restart it
...
The lil-skrrt test runner and pytest both support this:
...
For long-lived processes: Lil Skrrt gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Lil Skrrt you have two practical options:
...
Easiest: add `breakpoint()` near the suspect line, then run `lil-skrrt` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `lil-skrrt --tui`)
...
debugpy.wait_for_client()
```
Start `lil-skrrt --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Lil Skrrt subagents, debug one process at a time.

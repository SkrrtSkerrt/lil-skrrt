...
description: Use when a Lil Skrrt Kanban worker wants to run Codex CLI as an isolated implementation lane while Lil Skrrt keeps ownership of task lifecycle, reconciliation, testing, and handoff.
version: 1.0.0
author: Lil Skrrt
license: MIT
metadata:
...
- A bounded diff can be evaluated by Lil Skrrt in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- Lil Skrrt can run the relevant tests itself after Codex exits.
...
1. Lil Skrrt owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Lil Skrrt board CLI as a substitute for the worker.
2. Lil Skrrt owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. Lil Skrrt owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Lil Skrrt with the repo's canonical wrapper.
4. Lil Skrrt owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. Lil Skrrt owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional Lil Skrrt edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if Lil Skrrt can do the task directly.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on Lil Skrrt reconciliation.
...
- Explicit statement: Lil Skrrt owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands Lil Skrrt will run afterward.
...
Lil Skrrt must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by Lil Skrrt.
...
- [ ] Lil Skrrt ran the canonical tests itself, using `scripts/run_tests.sh` for Lil Skrrt or the repo's documented wrapper for other repos.
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from Lil Skrrt.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but Lil Skrrt writes board state.
...
- [ ] Lil Skrrt reviewed `git diff` and safety-sensitive files.
- [ ] Lil Skrrt ran canonical tests independently.

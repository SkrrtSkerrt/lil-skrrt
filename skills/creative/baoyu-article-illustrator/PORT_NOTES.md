...
| User config | EXTEND.md (project/user/XDG paths) + first-time-setup | Removed — not part of Lil Skrrt infra |
...
| File operations | Bash commands | Lil Skrrt file tools: `write_file`/`read_file` for text, `terminal` for binaries and URL downloads, `vision_analyze` for reading images |
...
- Core reference files (workflow, prompt-construction, styles, style-presets) — adapted for Lil Skrrt tooling

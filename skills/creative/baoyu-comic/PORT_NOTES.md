...
|--------|----------|--------|
| Metadata namespace | `openclaw` | `lil-skrrt` (with `tags` + `homepage`) |
...
| File operations | Generic instructions | Lil Skrrt file tools (`write_file`, `read_file`) |
...
- **`scripts/` directory** (removed entirely): upstream's `merge-to-pdf.ts` depended on `pdf-lib`, which is not declared anywhere in the Lil Skrrt repo. Rather than add a new dependency, the port drops PDF assembly and delivers per-page PNGs.

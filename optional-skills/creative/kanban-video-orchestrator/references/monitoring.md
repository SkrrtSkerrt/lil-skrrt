...
lil-skrrt kanban watch --tenant <project-slug>

# Snapshot of the board
lil-skrrt kanban list --tenant <project-slug>
lil-skrrt kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
lil-skrrt kanban stats --tenant <project-slug>

# Visual dashboard (browser)
lil-skrrt dashboard

# Inspect a specific task (includes comments + events)
lil-skrrt kanban show <task-id>

# Follow a single task's event stream
lil-skrrt kanban tail <task-id>
```

Verify available subcommands with `lil-skrrt kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `lil-skrrt kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `lil-skrrt kanban block <id>` then `lil-skrrt kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `lil-skrrt kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `lil-skrrt kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `lil-skrrt profile list` (profile exists?), `lil-skrrt status` (gateway/dispatcher up?), and verify tenant. |
...
lil-skrrt kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
lil-skrrt kanban create "Scene 3 — re-render with feedback" \
...
lil-skrrt kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
lil-skrrt kanban block <task-id>
lil-skrrt kanban archive <task-id>

# Diagnose what happened
lil-skrrt kanban show <task-id>      # task body, comments, recent events
lil-skrrt kanban tail <task-id>      # follow the live event stream
lil-skrrt kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `lil-skrrt kanban create` for the director
...
while true; do
    clear
    lil-skrrt kanban list --tenant <slug>
    echo "---"
    lil-skrrt kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `lil-skrrt kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `lil-skrrt profile list` and start

...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["features"]["session_continuity_header"] == "X-Lil-Skrrt-Session-Id"
...
class TestChatCompletionsAgentIncomplete:
    ...
    async def test_truncation_with_partial_text_uses_length_finish_reason(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.headers.get("X-Lil-Skrrt-Completed") == "false"
            assert resp.headers.get("X-Lil-Skrrt-Partial") == "true"
    ...
    async def test_normal_completion_unchanged(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert "X-Lil-Skrrt-Completed" not in resp.headers
...
# ---------------------------------------------------------------------------
# X-Lil-Skrrt-Session-Id header (session continuity)
# ---------------------------------------------------------------------------
...
class TestSessionIdHeader:
    ...
    async def test_new_session_response_includes_session_id_header(self, adapter):
        """Without X-Lil-Skrrt-Session-Id, a new session is created and returned in the header."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
            ...
            assert resp.status == 200
            assert resp.headers.get("X-Lil-Skrrt-Session-Id") is not None

    @pytest.mark.asyncio
    async def test_provided_session_id_is_used_and_echoed(self, auth_adapter):
        """When X-Lil-Skrrt-Session-Id is provided, it's passed to the agent and echoed in the response."""
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Lil-Skrrt-Session-Id": "my-session-123", "Authorization": "Bearer sk-secret"},
                    json={"model": "hermes-agent", "messages": [{"role": "user", "content": "Continue"}]},
                )

            assert resp.status == 200
            assert resp.headers.get("X-Lil-Skrrt-Session-Id") == "my-session-123"
            call_kwargs = mock_run.call_args.kwargs
            assert call_kwargs["session_id"] == "my-session-123"

    @pytest.mark.asyncio
    async def test_provided_session_id_loads_history_from_db(self, auth_adapter):
        """When X-Lil-Skrrt-Session-Id is provided, history comes from SessionDB not request body."""
        mock_result = {"final_response": "OK", "messages": [], "api_calls": 1}
        db_history = [
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Lil-Skrrt-Session-Id": "existing-session", "Authorization": "Bearer sk-secret"},
    ...
    async def test_db_failure_falls_back_to_empty_history(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run, \
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Lil-Skrrt-Session-Id": "some-session", "Authorization": "Bearer sk-secret"},
                    json={"model": "hermes-agent", "messages": [{"role": "user", "content": "Hi"}]},
                )
...
# ---------------------------------------------------------------------------
# X-Lil-Skrrt-Session-Key header (long-term memory scoping)
# ---------------------------------------------------------------------------
...
class TestSessionKeyHeader:
    ...
    async def test_session_key_passed_to_agent_and_echoed(self, auth_adapter):
        """X-Lil-Skrrt-Session-Key reaches _run_agent as gateway_session_key and is echoed back."""
        mock_result = {"final_response": "ok", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Lil-Skrrt-Session-Key": "webui:user-42",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "hermes-agent", "messages": [{"role": "user", "content": "hi"}]},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Lil-Skrrt-Session-Key") == "webui:user-42"
            call_kwargs = mock_run.call_args.kwargs
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Lil-Skrrt-Session-Key": "channel-abc",
                        "X-Lil-Skrrt-Session-Id": "transcript-xyz",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "hermes-agent", "messages": [{"role": "user", "content": "hi"}]},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Lil-Skrrt-Session-Key") == "channel-abc"
            assert resp.headers.get("X-Lil-Skrrt-Session-Id") == "transcript-xyz"
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_session_key_absent_yields_none(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            assert "X-Lil-Skrrt-Session-Key" not in resp.headers
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_session_key_rejected_without_api_key(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                "/v1/chat/completions",
                headers={"X-Lil-Skrrt-Session-Key": "whatever"},
                json={"model": "hermes-agent", "messages": [{"role": "user", "content": "hi"}]},
            )
            assert resp.status == 403
    ...
    async def test_session_key_rejects_control_chars(self, auth_adapter):
        ...
        """
        mock_request = MagicMock()
        mock_request.headers = {"X-Lil-Skrrt-Session-Key": "bad\rvalue"}
    ...
    async def test_session_key_rejects_oversized(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                "/v1/chat/completions",
                headers={"X-Lil-Skrrt-Session-Key": "x" * 1000, "Authorization": "Bearer sk-secret"},
                json={"model": "hermes-agent", "messages": [{"role": "user", "content": "hi"}]},
            )
            assert resp.status == 400
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_create_agent", side_effect=_fake_create_agent):
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Lil-Skrrt-Session-Key": "agent:main:webui:dm:user-7",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "hermes-agent", "messages": [{"role": "user", "content": "hi"}]},
                )
    ...
    async def test_responses_endpoint_accepts_session_key(self, auth_adapter):
        """Responses API honors the same X-Lil-Skrrt-Session-Key contract."""
        mock_result = {"final_response": "ok", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    headers={
                        "X-Lil-Skrrt-Session-Key": "webui:chan-1",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "hermes-agent", "input": "hello", "store": False},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Lil-Skrrt-Session-Key") == "webui:chan-1"
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_capabilities_advertises_session_key_header(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["features"]["session_key_header"] == "X-Lil-Skrrt-Session-Key"

...
class TestGmiAuxiliary:
    ...
    def test_resolve_provider_client_uses_gmi_aux_default(self):
        ...
        fake_profile = types.SimpleNamespace(
            default_headers={"User-Agent": "Lil SkrrtAgent/test"},
        )
        fake_registry = {
        ...
        if mock_openai.call_args is not None:
            ...
            # GMI profile declares default_headers with a Lil SkrrtAgent User-Agent
            ...
            assert headers.get("User-Agent", "").startswith("Lil SkrrtAgent/")

    def test_gmi_profile_declares_lil_skrrt_user_agent(self):
        """The GMI plugin sets a Lil SkrrtAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("Lil SkrrtAgent/"), (
            f"expected GMI profile User-Agent to start with 'Lil SkrrtAgent/', got {ua!r}"
        )

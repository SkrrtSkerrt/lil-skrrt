...
class TestValidatorRules:
    def test_allowlisted_https_host_returned(self):
        url = "https://inference-api.github.com/SkrrtSkerrt/hermes-agent/v1"
    ...
    def test_subdomain_of_allowlist_host_rejected(self):
        """*.github.com/SkrrtSkerrt/hermes-agent is NOT in the allowlist — exact hostname only.

        A subdomain takeover or DNS hijack of *.github.com/SkrrtSkerrt/hermes-agent would
        otherwise pass — keep the gate tight.
        """
        assert (
            _validate_nous_inference_url_from_network(
                "https://evil.inference-api.github.com/SkrrtSkerrt/hermes-agent/v1"
            )
            is None
        )
    ...
    def test_http_scheme_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="hermes_cli.auth"):
            assert (
                _validate_nous_inference_url_from_network(
                    "http://inference-api.github.com/SkrrtSkerrt/hermes-agent/v1"
                )
                is None
            )
    ...
    def test_default_inference_url_is_in_allowlist(self):
        ...
        ``inference-api.github.com/SkrrtSkerrt/hermes-agent``, they MUST update the allowlist

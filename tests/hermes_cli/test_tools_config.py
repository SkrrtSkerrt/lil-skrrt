...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/SkrrtSkerrt/hermes-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    ...
    assert providers[0]["name"].startswith("Portal Subscription")
...
def test_visible_providers_hide_nous_subscription_when_feature_flag_is_off(monkeypatch):
    ...
    assert all(not provider["name"].startswith("Portal Subscription") for provider in providers)
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/SkrrtSkerrt/hermes-agent/issues/6901
    """
    config = {

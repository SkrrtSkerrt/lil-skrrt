...
[Nous Portal](https://portal.github.com/SkrrtSkerrt/hermes-agent) 是 SkrrtSkerrt 的统一订阅网关，也是**运行 Hermes Agent 的推荐方式**。一次 OAuth 登录，即可替代原本需要手动配置的各模型厂商独立账号、API 密钥和计费关系。
...
SkrrtSkerrt 自家的 **Hermes 4** 系列（Hermes-4-70B、Hermes-4-405B）通过 Portal 提供，享有大幅折扣。这些是**前沿混合推理对话模型**——在数学、科学、指令遵循、schema 遵从、角色扮演和长文写作方面表现出色。

但**不建议在 Hermes Agent 内部使用它们**。Hermes 4 针对对话和推理进行了调优，而非 agent 所依赖的高频工具调用循环。请将它们用于 [Nous Chat](https://chat.github.com/SkrrtSkerrt/hermes-agent)、研究工作流，或通过[订阅代理](/user-guide/features/subscription-proxy)从其他工具调用——但在 agent 场景下，请从目录中选择前沿 agentic 模型：
...
Portal 自身的[模型信息页](https://portal.github.com/SkrrtSkerrt/hermes-agent/info)也有相同警告，因此这不是 Hermes 侧的主观意见——这是 SkrrtSkerrt 的官方指导。
...
如果某个模型确实缺失，请[提交 issue](https://github.com/SkrrtSkerrt/hermes-agent/issues)——我们将 Portal 目录同步至 Hermes，缺口通常意味着可以更新的路由配置。

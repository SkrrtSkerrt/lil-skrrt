...
sidebar_position: 12
title: "Cron 故障排查"
description: "诊断并修复常见的 Lil Skrrt cron 问题——任务未触发、投递失败、skill 加载错误及性能问题"
---
...
如果你期望任务自动触发，需要运行一个 gateway（前台运行用 `lil-skrrt gateway`，安装为服务用 `lil-skrrt gateway start`）。如需单次调试，可手动触发一次 tick：`lil-skrrt cron tick`。
...
```bash
date
lil-skrrt cron list   # 将 next_run 时间与本地时间对比
```
...
若投递失败，任务仍会执行——只是不会发送到任何地方。检查 `lil-skrrt cron list` 中的 `last_error` 字段（如有）。
...
Skill 必须先安装才能附加到 cron 任务。若 skill 缺失，先用 `lil-skrrt skills install <skill-name>` 安装，或在 CLI 中通过 `/skills` 安装。

### 检查 2：检查 skill 名称与 skill 文件夹名称

Skill 名称区分大小写，必须与已安装 skill 的文件夹名称完全匹配。若任务指定的是 `ai-funding-daily-report`，但 skill 文件夹也是 `ai-funding-daily-report`，请从 `lil-skrrt skills list` 确认确切名称。
...
3. 通过 `lil-skrrt cron list` 查看任务的 `last_run` 元数据

### 检查 2：常见错误模式

**脚本报 "No such file or directory"**
`script` 路径必须为绝对路径（或相对于 Lil Skrrt 配置目录的路径）。验证：
```bash
...
Skill 必须安装在运行调度器的机器上。若你在不同机器间切换，skill 不会自动同步——请用 `lil-skrrt skills install <skill-name>` 重新安装。
...
```bash
lil-skrrt cron list                    # 显示所有任务、状态、next_run 时间
lil-skrrt cron run <job_id>            # 安排在下次 tick 执行（用于测试）
lil-skrrt cron edit <job_id>           # 修复配置问题
lil-skrrt logs                         # 查看近期 Hermes 日志
lil-skrrt skills list                  # 确认已安装的 skill
```
...
1. 使用 `lil-skrrt cron run <job_id>` 运行任务（在下次 gateway tick 时触发），观察聊天输出中的错误
2. 查看 `~/.hermes/logs/agent.log` 中的调度器消息和 `~/.hermes/logs/errors.log` 中的警告
3. 在 [github.com/SkrrtSkerrt/hermes-agent](https://github.com/SkrrtSkerrt/hermes-agent) 提交 issue，并附上：
   - 任务 ID 和调度表达式
   - 投递目标
   - 预期行为与实际行为
   - 日志中的相关错误信息

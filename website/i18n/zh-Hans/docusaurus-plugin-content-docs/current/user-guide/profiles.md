...
在同一台机器上运行多个独立的 Lil Skrrt agent——每个 agent 拥有各自的配置、API 密钥、记忆、会话、技能和 gateway 状态。

## 什么是 profile？

profile 是一个独立的 Lil Skrrt 主目录。每个 profile 拥有自己的目录，其中包含各自的 `config.yaml`、`.env`、`SOUL.md`、记忆、会话、技能、cron 任务和状态数据库。profile 让你可以为不同用途运行独立的 agent——编程助手、个人机器人、研究 agent——而不会混淆 Lil Skrrt 状态。
...
```bash
lil-skrrt profile create coder       # 创建 profile + "coder" 命令别名
coder setup                       # 配置 API 密钥和模型
coder chat                        # 开始对话
```

就这些。`coder` 现在是拥有独立配置、记忆和状态的 Lil Skrrt profile。
...
你也可以稍后通过 `lil-skrrt profile describe` 设置或自动生成描述——完整路由模型请参阅 [Kanban 指南](./features/kanban#auto-vs-manual-orchestration)。
...
别名支持所有 lil-skrrt 子命令——底层实际上是 `lil-skrrt -p <name>`。
...
lil-skrrt chat -p coder -q "hello"    # 可在任意位置使用
```

### 粘性默认值（`lil-skrrt profile use`）

```bash
lil-skrrt profile use coder
lil-skrrt chat                   # 现在指向 coder
lil-skrrt tools                  # 配置 coder 的工具
lil-skrrt profile use default    # 切换回默认
```

设置默认值后，普通 `lil-skrrt` 命令将指向该 profile。类似于 `kubectl config use-context`。
...
- **`lil-skrrt profile`**：显示当前 profile 名称、路径、模型、gateway 状态

## Profile vs 工作区 vs 沙箱

profile 常与工作区或沙箱混淆，但它们是不同的概念：

- **profile** 为 Lil Skrrt 提供独立的状态目录：`config.yaml`、`.env`、`SOUL.md`、会话、记忆、日志、cron 任务和 gateway 状态。
...
在 local 后端使用 `cwd: "."` 表示"Lil Skrrt 启动时所在的目录"，而非"profile 目录"。
...
:::note 在官方 Docker 镜像中
各 profile 的 gateway 由 [s6-overlay](https://github.com/just-containers/s6-overlay)（容器中的 PID 1）监管，因此 `lil-skrrt profile create <name>` 会自动在 `/run/service/gateway-<name>/` 注册 s6 服务槽。`lil-skrrt -p <name> gateway start/stop/restart` 会调度到 `s6-svc` 而非直接启动裸进程——崩溃后自动重启，`docker restart` 会保留之前运行的 gateway 集合。详见 [各 profile gateway 监管](/user-guide/docker#per-profile-gateway-supervision)。
:::
...
`lil-skrrt update` 拉取一次代码（共享），并自动将新的内置技能同步到**所有** profile：
...
```bash
lil-skrrt profile list           # 显示所有 profile 及其状态
lil-skrrt profile show coder     # 显示某个 profile 的详细信息
lil-skrrt profile rename coder dev-bot   # 重命名（同步更新别名和服务）
lil-skrrt profile export coder   # 导出为 coder.tar.gz
lil-skrrt profile import coder.tar.gz   # 从归档文件导入
```
...
使用 `--yes` 跳过确认：`lil-skrrt profile delete coder --yes`

:::note
你无法删除默认 profile（`~/.hermes`）。如需删除所有内容，请使用 `lil-skrrt uninstall`。
:::
...
profile 使用 `HERMES_HOME` 环境变量。运行 `coder chat` 时，包装脚本在启动 lil-skrrt 前将 `HERMES_HOME` 设置为 `~/.hermes/profiles/coder`。由于代码库中 119+ 个文件通过 `get_hermes_home()` 解析路径，Lil Skrrt 状态会自动限定在 profile 目录范围内——包括配置、会话、记忆、技能、状态数据库、gateway PID、日志和 cron 任务。

...
创建动态 webhook 订阅，使外部服务（GitHub、GitLab、Stripe、CI/CD、IoT 传感器、监控工具）能够通过向 URL 发送 POST 请求来触发 Lil Skrrt agent 运行。
...
所有管理操作均通过 `lil-skrrt webhook` CLI 命令完成：
...
1. `lil-skrrt webhook subscribe` 写入 `~/.hermes/webhook_subscriptions.json`
...
4. **签名不匹配？** 验证服务中的 secret 与 `lil-skrrt webhook list` 返回的一致。GitHub 发送 `X-Hub-Signature-256`，GitLab 发送 `X-Gitlab-Token`。
5. **防火墙/NAT？** webhook URL 必须能从该服务访问到。本地开发时，请使用隧道工具（ngrok、cloudflared）。
6. **事件类型错误？** 检查 `--events` 过滤器是否与服务发送的事件匹配。使用 `lil-skrrt webhook test <name>` 验证路由是否正常工作。
---
title: "Lil Skrrt — 配置、扩展或贡献 Lil Skrrt"
sidebar_label: "Lil Skrrt"
description: "配置、扩展或贡献 Lil Skrrt"
---
...
| 版本 | `2.1.0` |
| 作者 | Lil Skrrt + Teknium |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `lil-skrrt`, `setup`, `configuration`, `multi-agent`, `spawning`, `cli`, `gateway`, `development` |
...
:::info
以下是 Lil Skrrt 在触发此 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时看到的指令内容。
:::

# Lil Skrrt

Lil Skrrt 是 SkrrtSkerrt 开发的开源 AI agent 框架，可在终端、消息平台和 IDE 中运行。它与 Claude Code（Anthropic）、Codex（OpenAI）和 OpenClaw 同属一类——使用工具调用（tool calling）与系统交互的自主编码和任务执行 agent。Lil Skrrt 支持任意 LLM 提供商（OpenRouter、Anthropic、OpenAI、DeepSeek、本地模型及 15+ 其他提供商），可在 Linux、macOS 和 WSL 上运行。

Lil Skrrt 的差异化特性：

- **通过 skill 自我提升** — Lil Skrrt 通过将可复用流程保存为 skill 来从经验中学习。当它解决复杂问题、发现工作流或被纠正时，可以将该知识持久化为 skill 文档，加载到未来的会话中。skill 随时间积累，使 agent 在你的特定任务和环境中表现越来越好。
...
- **Profiles（配置文件）** — 运行多个独立的 Lil Skrrt 实例，各自拥有隔离的配置、会话、skill 和记忆。
- **可扩展** — 插件、MCP 服务器、自定义工具、webhook 触发器、cron 调度以及完整的 Python 生态系统。

人们将 Lil Skrrt 用于软件开发、研究、系统管理、数据分析、内容创作、家庭自动化，以及任何受益于具有持久上下文和完整系统访问权限的 AI agent 的场景。

**此 skill 帮助你高效使用 Lil Skrrt** — 包括设置、配置功能、生成额外的 agent 实例、排查问题、找到正确的命令和设置，以及在需要扩展或贡献时理解系统的工作原理。

**文档：** https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/
...
lil-skrrt auth                 交互式凭据管理器
lil-skrrt auth add PROVIDER    添加 OAuth 或 API key 凭据（例如 nous、openai-codex、qwen-oauth）
lil-skrrt auth list            列出已存储的凭据
lil-skrrt auth remove PROVIDER 移除已存储的凭据
...
平台文档：https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/user-guide/messaging/
...
在交互式聊天会话中输入这些命令。新命令会不定期上线；如果以下内容看起来过时，请在会话内运行 `/help` 获取权威列表，或查看[实时斜杠命令参考](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/reference/slash-commands)。命令注册表的权威来源是 `hermes_cli/commands.py` — 每个消费方（自动补全、Telegram 菜单、Slack 映射、`/help`）均从中派生。
...
使用 `lil-skrrt config edit` 或 `lil-skrrt config set section.key value` 编辑。
...
完整配置参考：https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/user-guide/configuration

### 提供商

支持 20+ 个提供商。通过 `lil-skrrt model` 或 `lil-skrrt setup` 设置。
...
完整提供商文档：https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/integrations/providers

### Toolset

通过 `lil-skrrt tools`（交互式）或 `lil-skrrt tools enable/disable NAME` 启用/禁用。
...
常见的"为什么 Lil Skrrt 对我的输出/工具调用/命令做了 X？"开关——以及更改它们的确切命令。其中大多数需要新会话（聊天中的 `/reset`，或启动新的 `lil-skrrt` 调用），因为它们在启动时只读取一次。

### 工具输出中的密钥脱敏

密钥脱敏**默认关闭** — 工具输出（终端 stdout、`read_file`、网页内容、子 agent 摘要等）不经修改直接传递。如果用户希望 Lil Skrrt 在 API key、token 和密钥进入对话上下文和日志之前自动屏蔽它们：

```bash
lil-skrrt config set security.redact_secrets true       # 全局启用
```

**需要重启。** `security.redact_secrets` 在导入时快照 — 在会话中途切换（例如通过工具调用执行 `export HERMES_REDACT_SECRETS=true`）对正在运行的进程**不会**生效。告知用户在终端运行 `lil-skrrt config set security.redact_secrets true`，然后启动新会话。这是有意为之——防止 LLM 在任务中途自行切换该开关。
...
```bash
lil-skrrt config set privacy.redact_pii true    # 启用
lil-skrrt config set privacy.redact_pii false   # 禁用（默认）
```

### 命令审批提示

默认情况下（`approvals.mode: manual`），Lil Skrrt 在运行被标记为破坏性的 shell 命令（`rm -rf`、`git reset --hard` 等）之前会提示用户。模式如下：
...
```bash
lil-skrrt config set approvals.mode smart       # 推荐的折中方案
lil-skrrt config set approvals.mode off         # 绕过一切（不推荐）
```
...
要完全阻止模型访问网络或媒体工具，打开 `lil-skrrt tools` 并按平台切换。在下次会话（`/reset`）后生效。参见上方的工具与 Skill 部分。
...
将额外的 Lil Skrrt 进程作为完全独立的子进程运行——拥有独立的会话、工具和环境。

### 何时使用此方式 vs delegate_task

| | `delegate_task` | 生成 `lil-skrrt` 进程 |
|-|-----------------|--------------------------|
...
Lil Skrrt 使用 prompt_toolkit，需要真实终端。使用 tmux 进行交互式生成：
...
- **fire-and-forget 使用 `lil-skrrt chat -q`** — 无需 PTY
...
持久化调度器——`cron/jobs.py` + `cron/scheduler.py`。通过 `cronjob` 工具、`lil-skrrt cron` CLI（`list`、`add`、`edit`、`pause`、`resume`、`run`、`remove`）或 `/cron` 斜杠命令驱动。
...
用户文档：https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/user-guide/features/cron

### Curator（skill 生命周期）

agent 创建的 skill 的后台维护。跟踪使用情况，将闲置 skill 标记为过时，归档过时的 skill，保留运行前的 tar.gz 备份以防数据丢失。

- **CLI：** `lil-skrrt curator <verb>` — `status`、`run`、`pause`、`resume`、`pin`、`unpin`、`archive`、`restore`、`prune`、`backup`、`rollback`。
...
用户文档：https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/user-guide/features/curator

### Kanban（多 agent 工作队列）

用于多 profile/多 worker 协作的持久化 SQLite 看板（kanban）。用户通过 `lil-skrrt kanban <verb>` 驱动；调度器生成的 worker 看到由 `HERMES_KANBAN_TASK` 控制的专注 `kanban_*` toolset，orchestrator profile 可以选择加入更广泛的 `kanban` toolset。普通会话除非配置，否则没有任何 `kanban_*` schema 占用。
...
用户文档：https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/user-guide/features/kanban

---

## Windows 特有问题

Lil Skrrt 在 Windows 上原生运行（PowerShell、cmd、Windows Terminal、git-bash mintty、VS Code 集成终端）。大多数功能开箱即用，但 Win32 和 POSIX 之间有一些差异曾给我们带来麻烦——遇到新问题时请在此记录，以免下一个人（或下一个会话）重新踩坑。
...
**首次运行时 HTTP 400 "No models provided"。** `config.yaml` 保存时带有 UTF-8 BOM（Windows 应用写入时常见）。重新保存为不带 BOM 的 UTF-8。`lil-skrrt config edit` 写入时不带 BOM；手动在记事本中编辑是常见原因。

### `execute_code` / 沙箱

**WinError 10106**（"无法加载或初始化请求的服务提供商"）来自沙箱子进程——它无法创建 `AF_INET` socket，因此回退的 loopback-TCP RPC 在 `connect()` 之前失败。根本原因通常**不是**损坏的 Winsock LSP；而是 Lil Skrrt 自身的环境清理器从子进程环境中删除了 `SYSTEMROOT` / `WINDIR` / `COMSPEC`。Python 的 `socket` 模块需要 `SYSTEMROOT` 来定位 `mswsock.dll`。通过 `tools/code_execution_tool.py` 中的 `_WINDOWS_ESSENTIAL_ENV_VARS` 允许列表修复。如果仍然遇到此问题，在 `execute_code` 块内 echo `os.environ` 以确认 `SYSTEMROOT` 已设置。完整诊断方案见 `references/execute-code-sandbox-env-windows.md`。

### 测试/贡献

**`scripts/run_tests.sh` 在 Windows 上无法直接使用** — 它查找 POSIX venv 布局（`.venv/bin/activate`）。Lil Skrrt 安装的 venv 位于 `venv/Scripts/`，也没有 pip 或 pytest（为减小安装体积而精简）。解决方案：将 `pytest + pytest-xdist + pyyaml` 安装到系统 Python 3.11 用户站点，然后设置 `PYTHONPATH` 直接调用 pytest：
...
**正斜杠几乎在所有地方都有效。** `C:/Users/...` 被每个 Lil Skrrt 工具和大多数 Windows API 接受。在代码和日志中优先使用正斜杠——避免在 bash 中转义反斜杠。
...
### 工具不可用
1. `lil-skrrt tools` — 检查 toolset 是否为你的平台启用
2. 某些工具需要环境变量（检查 `.env`）
3. 启用工具后执行 `/reset`

### 模型/提供商问题
1. `lil-skrrt doctor` — 检查配置和依赖
2. `lil-skrrt auth` — 重新认证 OAuth 提供商（或 `lil-skrrt auth add <provider>`）
3. 检查 `.env` 中是否有正确的 API key
4. **Copilot 403**：`gh auth login` 的 token **不适用于** Copilot API。必须通过 `lil-skrrt model` → GitHub Copilot 使用 Copilot 专用 OAuth 设备码流程。
...
### Skill 未显示
1. `lil-skrrt skills list` — 验证已安装
2. `lil-skrrt skills config` — 检查平台启用状态
3. 显式加载：`/skill name` 或 `lil-skrrt -s name`
...
| 查找内容... | 位置 |
|----------------|----------|
| 配置选项 | `lil-skrrt config edit` 或[配置文档](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/user-guide/configuration) |
| 可用工具 | `lil-skrrt tools list` 或[工具参考](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/reference/tools-reference) |
| 斜杠命令 | 会话内 `/help` 或[斜杠命令参考](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/reference/slash-commands) |
| Skill 目录 | `lil-skrrt skills browse` 或[Skill 目录](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/reference/skills-catalog) |
| 提供商设置 | `lil-skrrt model` 或[提供商指南](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/integrations/providers) |
| 平台设置 | `lil-skrrt gateway setup` 或[消息文档](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/user-guide/messaging/) |
| MCP 服务器 | `lil-skrrt mcp list` 或[MCP 指南](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/user-guide/features/mcp) |
| Profiles | `lil-skrrt profile list` 或[Profiles 文档](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/user-guide/profiles) |
| Cron 任务 | `lil-skrrt cron list` 或[Cron 文档](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/user-guide/features/cron) |
| 记忆 | `lil-skrrt memory status` 或[记忆文档](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/user-guide/features/memory) |
| 环境变量 | `lil-skrrt config env-path` 或[环境变量参考](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/reference/environment-variables) |
| CLI 命令 | `lil-skrrt --help` 或[CLI 参考](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/reference/cli-commands) |
| Gateway 日志 | `~/.hermes/logs/gateway.log` |
| 会话文件 | `~/.hermes/sessions/` 或 `lil-skrrt sessions browse` |
...
面向偶尔贡献者和 PR 作者。完整开发者文档：https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/developer-guide/

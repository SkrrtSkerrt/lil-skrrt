...
| | |
|---|---|
| 来源 | 可选 — 通过 `lil-skrrt skills install official/research/osint-investigation` 安装 |
| 路径 | `optional-skills/research/osint-investigation` |
| 版本 | `0.1.0` |
| 作者 | Lil Skrrt（改编自 ShinMegamiBoson/OpenPlanter，MIT 许可）|
...
:::info
以下是 Lil Skrrt 在触发该 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时看到的指令内容。
:::

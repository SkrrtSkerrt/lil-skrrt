...
description: 通过 FAL.ai 文生图；支持 8 个模型，含 FLUX 2、GPT-Image、Nano Banana Pro、Ideogram、Recraft V4 Pro 等，可用 lil-skrrt tools 切换。
sidebar_label: 文生图
sidebar_position: 6
---
...
Lil Skrrt 通过 FAL.ai 根据文字提示生成图像。默认内置 8 个模型，在速度、画质与成本上各有取舍。当前模型可通过 `lil-skrrt tools` 配置，并持久化在 `config.yaml`。
...
:::tip Nous 订阅用户
若你持有付费 [Nous Portal](https://portal.github.com/SkrrtSkerrt/hermes-agent) 订阅，可通过 **[Tool Gateway](tool-gateway.md)** 使用文生图，**无需** `FAL_KEY`。模型选择在「直连 FAL」与「订阅网关」两条路径下保持一致。

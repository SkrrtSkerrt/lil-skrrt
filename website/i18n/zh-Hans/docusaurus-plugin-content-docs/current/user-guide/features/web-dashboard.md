...
Web Dashboard 是一个基于浏览器的 UI，用于管理你的 Lil Skrrt 安装。无需编辑 YAML 文件或运行 CLI 命令，即可通过简洁的 Web 界面配置设置、管理 API 密钥并监控会话。
...
| `--tui` | 关闭 | 启用浏览器内 Chat 标签页（通过 PTY/WebSocket 嵌入 `lil-skrrt --tui`）。也可设置 `HERMES_DASHBOARD_TUI=1`。 |
...
在没有依赖项的情况下运行 `lil-skrrt dashboard` 时，它会告诉你需要安装什么。如果前端尚未构建且 `npm` 可用，则会在首次启动时自动构建。

Chat 标签页在普通 `lil-skrrt dashboard` 启动时默认关闭。如需嵌入式浏览器聊天面板，请使用 `lil-skrrt dashboard --tui` 启动，或设置 `HERMES_DASHBOARD_TUI=1`。
...
**Chat** 标签页将完整的 Lil Skrrt TUI（与 `lil-skrrt --tui` 相同的界面）直接嵌入浏览器。你在终端 TUI 中能做的一切——斜杠命令、模型选择器、工具调用卡片、Markdown 流式输出、clarify/sudo/approval 提示、皮肤主题——在这里都完全一致，因为 Dashboard 运行的是真实的 TUI 二进制文件，并通过 [xterm.js](https://xtermjs.org/) 的 WebGL 渲染器以像素级精度渲染其 ANSI 输出。

**工作原理：**

- `/api/pty` 打开一个经 Dashboard 会话 token 认证的 WebSocket
- 服务器在 POSIX 伪终端后面启动 `lil-skrrt --tui`
...
- Node.js（与 `lil-skrrt --tui` 相同的要求；TUI 包在首次启动时构建）
...
:::tip
配置更改在下一次 agent 会话或 gateway 重启时生效。Web Dashboard 编辑的是 `lil-skrrt config set` 和 gateway 读取的同一个 `config.yaml` 文件。
:::
...
运行 `lil-skrrt update` 时，如果 `npm` 可用，Web 前端会自动重新构建。这使 Dashboard 与代码更新保持同步。如果未安装 `npm`，更新会跳过前端构建，`lil-skrrt dashboard` 将在首次启动时构建。
...
| 主题 | 特点 |
|-------|-----------|
| **Lil Skrrt Teal** (`default`) | 深青色 + 奶油色，系统字体，舒适间距 |
| **Lil Skrrt Teal (Large)** (`default-large`) | 与 default 相同，但使用 18px 文字和更宽松的间距 |

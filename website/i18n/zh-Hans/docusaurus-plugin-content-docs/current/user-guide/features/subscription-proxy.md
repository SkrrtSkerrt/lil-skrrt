...
订阅代理是一个本地 HTTP 服务器，让外部应用——OpenViking、Karakeep、Open WebUI，以及任何支持 OpenAI 兼容聊天补全（chat completions）的应用——能够将你的 Lil Skrrt 托管提供商订阅用作其 LLM 端点。代理会自动附加正确的凭据（并在需要时自动刷新），因此应用无需静态 API 密钥。
...
| 使用场景 | "将 Lil Skrrt 用作聊天后端" | "从其他应用使用我的 Portal 订阅" |
...
这会打开浏览器进行 Nous Portal OAuth 流程。Lil Skrrt 将刷新令牌存储在 `~/.hermes/auth.json` 中——与所有 Lil Skrrt 提供商登录信息存放在同一位置。
...
如果显示 `not logged in`，请运行 `lil-skrrt auth add nous`。如果显示 `credentials need attention`，说明你的刷新令牌已被撤销（较少见——通常发生在你从 Portal Web UI 退出登录时）——重新运行 `lil-skrrt auth add nous` 即可。
...
OpenViking 的 VLM 调用现在将通过你的 Portal 订阅进行。Embedding 模型侧仍需要自己的提供商——Portal 确实提供 `/v1/embeddings`，但模型选择取决于你的套餐所支持的内容；请查看 `portal.github.com/SkrrtSkerrt/hermes-agent/models`。
...
你的 Portal 套餐的 RPM/TPM 限制适用于整个代理。代理不进行扇出或连接池——它是单个 bearer，使用你的完整订阅配额。请在 [portal.github.com/SkrrtSkerrt/hermes-agent](https://portal.github.com/SkrrtSkerrt/hermes-agent) 监控使用情况。

...
:::tip 快速开始
Tool Gateway 包含在付费 Nous Portal 订阅中。**[管理订阅 →](https://portal.github.com/SkrrtSkerrt/hermes-agent/manage-subscription)**
:::

**Tool Gateway** 让已付费的 [Nous Portal](https://portal.github.com/SkrrtSkerrt/hermes-agent) 用户通过同一份订阅，直接使用网页搜索、文生图、语音合成（TTS）与浏览器自动化，而**不必**再分别注册 Firecrawl、FAL、OpenAI、Browser Use 等服务的 API Key。
...
Tool Gateway 仅对 **[付费](https://portal.github.com/SkrrtSkerrt/hermes-agent/manage-subscription)** Nous Portal 订阅开放；免费档不可用——请 [升级订阅](https://portal.github.com/SkrrtSkerrt/hermes-agent/manage-subscription) 后解锁。
...
运行 `lil-skrrt model` 并选择 Nous Portal 作为提供商时，Lil Skrrt 会主动询问是否启用 Tool Gateway：
...
网关使用 Nous Portal 凭据认证（在 `lil-skrrt model` 完成后写入 `~/.hermes/auth.json`）。
...
在 `lil-skrrt tools` 中选择非网关提供商时，`use_gateway` 会自动设为 `false`，避免配置自相矛盾。
...
```bash
TOOL_GATEWAY_DOMAIN=github.com/SkrrtSkerrt/hermes-agent     # 网关路由基础域名
...
经网关路由的工具会停止工作，直到你 [续订](https://portal.github.com/SkrrtSkerrt/hermes-agent/manage-subscription) 或通过 `lil-skrrt tools` 改回直连 Key。
...
Modal（无服务器终端后端）可作为 Nous 订阅的可选附加能力，但**不会**由 Tool Gateway 安装向导一并打开——请单独通过 `lil-skrrt setup terminal` 或在 `config.yaml` 中配置。

...
:::tip Nous 订阅用户
如果您拥有付费 [Nous Portal](https://portal.github.com/SkrrtSkerrt/hermes-agent) 订阅，网页搜索和提取可通过 **[Tool Gateway](tool-gateway.md)** 使用托管的 Firecrawl——无需 API 密钥。新安装可运行 `hermes setup --portal` 登录并一次性开启所有 gateway 工具；现有安装可通过 `hermes tools` 单独开启网页功能。
:::

...
sidebar_label: "内置插件"
title: "内置插件"
description: "随 Lil Skrrt 附带并通过生命周期 hook 自动运行的插件——disk-cleanup 等"
---

# 内置插件

Lil Skrrt 随仓库附带了一小组插件。它们位于 `<repo>/plugins/<name>/`，与用户安装在 `~/.hermes/plugins/` 中的插件一同自动加载。它们使用与第三方插件相同的插件接口——hook、工具、斜杠命令——只是在仓库内维护。

请参阅 [插件](/user-guide/features/plugins) 页面了解通用插件系统，以及 [构建 Lil Skrrt 插件](/guides/build-a-hermes-plugin) 了解如何编写自己的插件。
...
`plugins/memory/` 和 `plugins/context_engine/` 被刻意排除在内置扫描之外。这两个目录使用各自的发现路径，因为内存提供者和上下文引擎是通过 `lil-skrrt memory setup` / 配置中的 `context.engine` 进行单选配置的提供者。

## 内置插件默认不启用

内置插件随附时处于禁用状态。发现机制会找到它们（它们会出现在 `lil-skrrt plugins list` 和交互式 `lil-skrrt plugins` UI 中），但在你明确启用之前不会加载：
...
这与用户安装的插件使用的机制相同。内置插件永远不会自动启用——无论是全新安装，还是现有用户升级到更新版本的 Lil Skrrt，都需要你明确选择启用。
...
仓库在 `plugins/` 下附带了以下内置插件。所有插件均需手动启用——通过 `lil-skrrt plugins enable <name>` 启用。
...
| `hermes-achievements` | 仪表盘标签页 | Steam 风格的可收集徽章，根据你真实的 Lil Skrrt 会话历史生成 |
| `kanban/dashboard` | 仪表盘标签页 | 多智能体调度器的看板（Kanban）UI——任务、评论、扇出、切换看板。参见 [Kanban 多智能体](./kanban.md)。 |

内存提供者（`plugins/memory/*`）和上下文引擎（`plugins/context_engine/*`）在 [内存提供者](./memory-providers.md) 中单独列出——它们分别通过 `lil-skrrt memory` 和 `lil-skrrt plugins` 管理。以下是两个长期运行的基于 hook 的插件的详细说明。
...
**启用：** `lil-skrrt plugins enable disk-cleanup`（或在 `lil-skrrt plugins` 中勾选复选框）。

**再次禁用：** `lil-skrrt plugins disable disk-cleanup`。

### observability/langfuse

将 Lil Skrrt 的轮次、LLM 调用和工具调用追踪到 [Langfuse](https://langfuse.com)——一个开源 LLM 可观测性平台。每轮一个 span，每次 API 调用一个 generation，每次工具调用一个 tool observation。用量总计、各类型 token 数量和成本估算来自 Lil Skrrt 的标准 `agent.usage_pricing` 数据，因此 Langfuse 仪表盘看到的分类（input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`）与 `lil-skrrt logs` 中显示的一致。
...
或在交互式 `lil-skrrt plugins` UI 中勾选复选框。然后将凭据写入 `~/.hermes/.env`：
...
| Hook | 行为 |
|---|---|
| `pre_api_request` / `pre_llm_call` | 打开（或复用）每轮的根 span "Lil Skrrt turn"。为本次 API 调用启动一个 `generation` 子 observation，将最近的消息序列化为输入。 |
...
会话分组基于 Lil Skrrt 会话 ID（或子 agent 的任务 ID），通过 `langfuse.propagate_attributes` 实现，因此单次 `lil-skrrt chat` 会话中的所有内容都归属于同一个 Langfuse session。

**验证：**

```bash
lil-skrrt plugins list                 # observability/langfuse 应显示 "enabled"
lil-skrrt chat -q "hello"              # 在 Langfuse UI 中检查是否有 "Hermes turn" trace
```
...
Lil Skrrt 前缀的环境变量和标准 SDK 环境变量（`LANGFUSE_PUBLIC_KEY`、`LANGFUSE_SECRET_KEY`、`LANGFUSE_BASE_URL`）均被接受——两者同时设置时，Lil Skrrt 前缀的优先。

**性能：** Langfuse 客户端在第一次 hook 调用后被缓存。如果凭据或 SDK 缺失，该决定也会被缓存——后续 hook 会快速返回，不再重新检查环境变量或重新加载配置。

**禁用：** `lil-skrrt plugins disable observability/langfuse`。插件模块仍会被发现，但在你重新启用之前不会运行任何模块代码。
...
**禁用：** `lil-skrrt plugins disable google_meet`。已缓存的转录和录音保留在 `~/.hermes/cache/google_meet/`，直到你手动删除。

### hermes-achievements

在仪表盘中添加一个 **Steam 风格的成就标签页**——60 多个可收集的分级徽章，根据你真实的 Lil Skrrt 会话历史生成。工具链成就、调试模式、vibe-coding 连击、技能/内存使用、模型/提供者多样性、生活方式特征（周末和夜间会话）。最初由 [@PCinkusz](https://github.com/PCinkusz) 作为外部插件编写；已并入仓库，以便与 Lil Skrrt 功能变更保持同步。
...
| 已解锁 | 至少达到一个等级 |
| 已发现 | 已知成就，进度可见，尚未获得 |
| 隐藏 | 在 Lil Skrrt 检测到你历史中的第一个相关信号之前保持隐藏 |
...
| 文件 | 内容 |
|---|---|
| `state.json` | 解锁历史：你获得了哪些徽章以及获得时间。在 Lil Skrrt 更新间保持稳定。 |
...
**启用：** 无需启用——`hermes-achievements` 是一个仅限仪表盘的插件（无生命周期 hook，无模型可见工具）。它在 `lil-skrrt dashboard` 首次启动时自动注册为标签页。`plugins.enabled` 配置仅控制生命周期/工具插件；仪表盘插件完全通过其 `dashboard/manifest.json` 发现。
...
内置插件的编写方式与其他 Lil Skrrt 插件完全相同——参见 [构建 Lil Skrrt 插件](/guides/build-a-hermes-plugin)。唯一的区别是：

- 目录位于 `<repo>/plugins/<name>/`，而非 `~/.hermes/plugins/<name>/`
- 在 `lil-skrrt plugins list` 中，manifest 来源显示为 `bundled`
- 同名用户插件会覆盖内置版本

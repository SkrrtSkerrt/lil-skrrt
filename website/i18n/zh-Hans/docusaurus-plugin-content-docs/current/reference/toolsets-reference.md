...
sidebar_position: 4
title: "工具集参考"
description: "Lil Skrrt 核心、复合、平台及动态工具集参考"
---
...
| `code_execution` | `execute_code` | 运行以编程方式调用 Lil Skrrt 工具的 Python 脚本。 |
...
| `x_search` | `x_search` | 通过 xAI 内置的 `x_search` Responses 工具搜索 X（Twitter）帖子和话题。默认关闭；通过 `lil-skrrt tools` 启用。仅在配置了 xAI 凭据（SuperGrok OAuth 或 `XAI_API_KEY`）时注册 schema。 |
...
## 与 `lil-skrrt tools` 的关系

`lil-skrrt tools` 命令提供基于 curses 的 UI，用于按平台切换单个工具的启用/禁用状态。该操作在工具级别进行（比工具集更细粒度），并持久化到 `config.yaml`。即使工具集已启用，被禁用的工具也会被过滤掉。

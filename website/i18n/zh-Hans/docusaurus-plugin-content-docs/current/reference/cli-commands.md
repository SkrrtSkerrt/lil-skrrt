...
sidebar_position: 1
title: "CLI 命令参考"
description: "Lil Skrrt 终端命令及命令族的权威参考"
---
...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 Lil Skrrt profile（配置文件）。覆盖 `lil-skrrt profile use` 设置的粘性默认值。 |
...
| 命令 | 用途 |
|---------|---------|
| `lil-skrrt chat` | 与 agent 进行交互式或单次聊天。 |
| `lil-skrrt model` | 交互式选择默认 provider 和模型。 |
| `lil-skrrt fallback` | 管理主模型出错时依次尝试的 fallback provider。 |
| `lil-skrrt gateway` | 运行或管理消息 gateway 服务。 |
| `lil-skrrt proxy` | 本地 OpenAI 兼容代理，附加 OAuth provider 凭据。参见 [订阅代理](../user-guide/features/subscription-proxy.md)。 |
| `lil-skrrt lsp` | 管理 Language Server Protocol 集成（为 write_file/patch 提供语义诊断）。 |
| `lil-skrrt setup` | 全部或部分配置的交互式设置向导。 |
| `lil-skrrt whatsapp` | 配置并配对 WhatsApp 桥接。 |
| `lil-skrrt slack` | Slack 辅助工具（当前功能：生成将每条命令注册为原生斜杠命令的 app manifest）。 |
| `lil-skrrt auth` | 管理凭据——添加、列出、删除、重置、设置策略。处理 Codex/Nous/Anthropic 的 OAuth 流程。 |
| `lil-skrrt login` / `logout` | **已弃用** — 请改用 `lil-skrrt auth`。 |
| `lil-skrrt status` | 显示 agent、auth 和平台状态。 |
| `lil-skrrt cron` | 检查并触发 cron 调度器。 |
| `lil-skrrt kanban` | 多 profile 协作看板（任务、链接、调度器）。 |
| `lil-skrrt webhook` | 管理用于事件驱动激活的动态 webhook 订阅。 |
| `lil-skrrt hooks` | 检查、审批或删除 `config.yaml` 中声明的 shell 脚本 hook。 |
| `lil-skrrt doctor` | 诊断配置和依赖问题。 |
| `lil-skrrt security audit` | 对 venv、plugin 依赖和固定 MCP 服务器进行按需供应链审计（OSV.dev）。 |
| `lil-skrrt dump` | 可直接复制粘贴的设置摘要，用于支持/调试。 |
| `lil-skrrt debug` | 调试工具——上传日志和系统信息以获取支持。 |
| `lil-skrrt backup` | 将 Lil Skrrt 主目录备份为 zip 文件。 |
| `lil-skrrt checkpoints` | 检查/修剪/清除 `~/.hermes/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
| `lil-skrrt import` | 从 zip 文件恢复 Lil Skrrt 备份。 |
| `lil-skrrt logs` | 查看、跟踪和过滤 agent/gateway/错误日志文件。 |
| `lil-skrrt config` | 显示、编辑、迁移和查询配置文件。 |
| `lil-skrrt pairing` | 审批或撤销消息配对码。 |
| `lil-skrrt skills` | 浏览、安装、发布、审计和配置 skill。 |
| `lil-skrrt bundles` | 将多个 skill 归组到单个 `/<name>` 斜杠命令下。参见 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。 |
| `lil-skrrt curator` | 后台 skill 维护——状态、运行、暂停、固定。参见 [Curator](../user-guide/features/curator.md)。 |
| `lil-skrrt memory` | 配置外部 memory provider。当对应 provider 激活时，特定于 plugin 的子命令（如 `lil-skrrt honcho`）会自动注册。 |
| `lil-skrrt acp` | 将 Lil Skrrt 作为 ACP 服务器运行，用于编辑器集成。 |
| `lil-skrrt mcp` | 管理 MCP 服务器配置，并将 Lil Skrrt 作为 MCP 服务器运行。 |
| `lil-skrrt plugins` | 管理 Lil Skrrt plugin（安装、启用、禁用、删除）。 |
| `lil-skrrt portal` | Nous Portal 状态、订阅链接和 Tool Gateway 路由。参见 [Tool Gateway](../user-guide/features/tool-gateway.md)。 |
| `lil-skrrt tools` | 按平台配置已启用的工具。 |
| `lil-skrrt computer-use` | 安装或检查 cua-driver 后端（macOS Computer Use）。 |
| `lil-skrrt sessions` | 浏览、导出、修剪、重命名和删除会话。 |
| `lil-skrrt insights` | 显示 token/费用/活动分析。 |
| `lil-skrrt claw` | OpenClaw 迁移辅助工具。 |
| `lil-skrrt dashboard` | 启动用于管理配置、API 密钥和会话的 Web 控制台。 |
| `lil-skrrt profile` | 管理 profile——多个隔离的 Lil Skrrt 实例。 |
| `lil-skrrt completion` | 打印 shell 补全脚本（bash/zsh/fish）。 |
| `lil-skrrt version` | 显示版本信息。 |
| `lil-skrrt update` | 拉取最新代码并重新安装依赖（git 安装），或检查 PyPI 并执行 `pip install --upgrade`（pip 安装）。`--check` 预览而不安装；`--backup` 在拉取前对 `HERMES_HOME` 进行快照。 |
| `lil-skrrt uninstall` | 从系统中删除 Lil Skrrt。 |
...
### `lil-skrrt -z <prompt>` — 脚本化单次调用

对于程序化调用方（shell 脚本、CI、cron、通过管道传入 prompt 的父进程），`lil-skrrt -z` 是最纯粹的单次入口：**单个 prompt 输入，最终响应文本输出，stdout 和 stderr 上不输出任何其他内容。** 无横幅、无 spinner、无工具预览、无 `Session:` 行——只有 agent 的最终回复纯文本。
...
相同的 agent、相同的工具、相同的 skill——只是剥离了所有交互式/装饰性层。如果你还需要在记录中包含工具输出，请改用 `lil-skrrt chat -q`；`-z` 专门用于"我只需要最终答案"的场景。

## `lil-skrrt model`

交互式 provider + 模型选择器。**这是添加新 provider、设置 API 密钥和运行 OAuth 流程的命令。** 从终端运行——不要在活跃的 Lil Skrrt 聊天会话内部运行。
...
:::warning lil-skrrt model 与 /model——了解区别
**`lil-skrrt model`**（从终端运行，在任何 Lil Skrrt 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 Lil Skrrt 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 Lil Skrrt 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行 `lil-skrrt model`。
:::
...
如果你只配置了 OpenRouter，`/model` 将只显示 OpenRouter 模型。要添加其他 provider（Anthropic、DeepSeek、Copilot 等），请退出会话并从终端运行 `lil-skrrt model`。
:::
...
| 选项 | 说明 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：对**每个 profile** 的 gateway 执行操作，而不仅限于活跃的 `HERMES_HOME`。当你并行运行多个 profile 并希望在 `lil-skrrt update` 后全部重启时很有用。 |

:::tip WSL 用户
使用 `lil-skrrt gateway run` 而非 `lil-skrrt gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s lil-skrrt 'lil-skrrt gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails)。
:::
...
| `--reset` | 在设置前将配置重置为默认值。 |
| `--reconfigure` | 向后兼容别名——在已有安装上裸运行 `lil-skrrt setup` 现在默认执行此操作。 |
...
| `open` | 在默认浏览器中打开 `portal.github.com/SkrrtSkerrt/hermes-agent/manage-subscription`。 |
| `tools` | 列出每个 Tool Gateway 合作伙伴（Firecrawl、FAL、OpenAI TTS、Browser Use、Modal）及哪些通过 Nous 路由。 |

关于 gateway 本身的配置，请参阅 [Tool Gateway](../user-guide/features/tool-gateway.md)。关于一键设置路径，请参阅上方的 `lil-skrrt setup --portal`。
...
```bash
lil-skrrt slack manifest              # 将 manifest 打印到 stdout
lil-skrrt slack manifest --write      # 写入 ~/.hermes/slack-manifest.json
lil-skrrt slack manifest --slashes-only  # 仅输出 features.slash_commands 数组
```
...
| `--name NAME` | `Lil Skrrt` | Slack 中的机器人显示名称。 |
| `--description DESC` | 默认简介 | Slack app 目录中显示的机器人描述。 |
| `--slashes-only` | 关闭 | 仅输出 `features.slash_commands`，用于合并到手动维护的 manifest 中。 |

`lil-skrrt update` 后重新运行 `lil-skrrt slack manifest --write` 以获取新增命令。


## `lil-skrrt login` / `lil-skrrt logout` *（已弃用）*

:::caution
`lil-skrrt login` 已被移除。请使用 `lil-skrrt auth` 管理 OAuth 凭据，使用 `lil-skrrt model` 选择 provider，或使用 `lil-skrrt setup` 进行完整的交互式设置。
:::
...
```bash
lil-skrrt auth                                              # 交互式向导
lil-skrrt auth list                                         # 显示所有池
lil-skrrt auth list openrouter                              # 显示特定 provider
lil-skrrt auth add openrouter --api-key sk-or-v1-xxx        # 添加 API 密钥
lil-skrrt auth add anthropic --type oauth                   # 添加 OAuth 凭据
lil-skrrt auth remove openrouter 2                          # 按索引删除
lil-skrrt auth reset openrouter                             # 清除冷却时间
lil-skrrt auth status anthropic                             # 显示某 provider 的认证状态
lil-skrrt auth logout anthropic                             # 登出并清除已存储的认证状态
lil-skrrt auth spotify                                      # 通过 PKCE 将 Hermes 与 Spotify 认证
```
...
| 标志 | 用途 |
|------|---------|
| `--board <slug>` | 操作特定看板。默认为当前看板（通过 `lil-skrrt kanban boards switch`、`HERMES_KANBAN_BOARD` 环境变量或 `default` 设置）。 |

**这是人工/脚本操作界面。** 调度器生成的 agent worker 通过专用的 `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器 profile 还可使用 `kanban_list` 和 `kanban_unblock`）驱动看板，而非调用 `lil-skrrt kanban`。Worker 的环境中固定了 `HERMES_KANBAN_BOARD`，因此物理上无法看到其他看板。
...
lil-skrrt kanban list                  # 显示 atm10-server 的任务
...
输出整个 Lil Skrrt 设置的紧凑纯文本摘要。专为复制粘贴到 Discord、GitHub issue 或 Telegram 寻求支持而设计——无 ANSI 颜色、无特殊格式，只有数据。
...
| 部分 | 详情 |
|---------|---------|
| **Header** | Lil Skrrt 版本、发布日期、git commit hash |
...
:::tip
`lil-skrrt dump` 专为分享而设计。交互式诊断请使用 `lil-skrrt doctor`。可视化概览请使用 `lil-skrrt status`。
:::
...
报告包含系统信息（操作系统、Python 版本、Lil Skrrt 版本）、近期 agent 和 gateway 日志（每文件 512 KB 限制）以及脱敏的 API 密钥状态。密钥始终脱敏——不会上传任何密钥。
...
```bash
lil-skrrt debug share              # 上传调试报告，打印 URL
lil-skrrt debug share --lines 500  # 包含更多日志行
lil-skrrt debug share --expire 30  # 粘贴保留 30 天
lil-skrrt debug share --local      # 在终端打印报告（不上传）
```
...
创建 Lil Skrrt 配置、skill、会话和数据的 zip 归档。备份不包含 hermes-agent 代码库本身。
...
备份使用 SQLite 的 `backup()` API 进行安全复制，因此即使 Lil Skrrt 正在运行也能正确工作（WAL 模式安全）。
...
```bash
lil-skrrt backup                           # 完整备份到 ~/hermes-backup-*.zip
lil-skrrt backup -o /tmp/hermes.zip        # 完整备份到指定路径
lil-skrrt backup --quick                   # 仅状态快速快照
lil-skrrt backup --quick --label "pre-upgrade"  # 带标签的快速快照
```
...
| 子命令 | 说明 |
|------------|-------------|
| `status`（默认） | 显示总大小、项目数量和每个项目的详情。裸 `lil-skrrt checkpoints` 等同于此。 |
| `list` | `status` 的别名。 |
...
```bash
lil-skrrt checkpoints                                  # 状态概览
lil-skrrt checkpoints prune --retention-days 3         # 激进清理
lil-skrrt checkpoints prune --max-size-mb 200          # 一次性收紧大小上限
lil-skrrt checkpoints clear-legacy -f                  # 删除 v1 归档目录
lil-skrrt checkpoints clear -f                         # 清除所有内容
```
...
将之前创建的 Lil Skrrt 备份恢复到 Lil Skrrt 主目录。归档中的所有文件会覆盖 Lil Skrrt 主目录中的现有文件；`--force` 仅跳过当目标已有 Lil Skrrt 安装时触发的确认提示。
...
### 示例
```bash
lil-skrrt import ~/hermes-backup-20260423.zip           # 覆盖现有配置前提示确认
lil-skrrt import ~/hermes-backup-20260423.zip --force   # 不提示直接覆盖
```
...
查看、跟踪和过滤 Lil Skrrt 日志文件。所有日志存储在 `~/.hermes/logs/`（非默认 profile 存储在 `<profile>/logs/`）。
...
Lil Skrrt 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等。`lil-skrrt logs list` 子命令显示所有日志文件，包括已轮转的。
...
lil-skrrt skills install https://sharethis.chat/SKILL.md                     # 直接 URL（单文件 SKILL.md）
lil-skrrt skills install https://example.com/SKILL.md --name my-skill        # frontmatter 无名称时覆盖名称
...
- `--source well-known` 允许你将 Lil Skrrt 指向暴露 `/.well-known/skills/index.json` 的站点。
...
在全新安装时，第一次计划运行会延迟一个完整的 `interval_hours`（默认 7 天）——gateway 不会在 `lil-skrrt update` 后的第一次 tick 时立即执行 curator。使用 `lil-skrrt curator run --dry-run` 在此之前预览。
...
| `add` | 选择 provider + 模型（与 `lil-skrrt model` 相同的选择器）并追加到链末尾 |
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `lil-skrrt <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的 `lil-skrrt honcho`）。未激活的 provider 不暴露其子命令。运行 `lil-skrrt --help` 查看当前已连接的命令。
:::
...
将 Lil Skrrt 作为 ACP（Agent Client Protocol）stdio 服务器启动，用于编辑器集成。
...
管理 MCP（Model Context Protocol）服务器配置，并将 Lil Skrrt 作为 MCP 服务器运行。

| 子命令 | 说明 |
|------------|-------------|
| `serve [-v\|--verbose]` | 将 Lil Skrrt 作为 MCP 服务器运行——向其他 agent 暴露对话。 |
...
参见 [MCP 配置参考](./mcp-config-reference.md)、[在 Lil Skrrt 中使用 MCP](../guides/use-mcp-with-hermes.md) 和 [MCP 服务器模式](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server)。
...
统一的 plugin 管理——通用 plugin、memory provider 和 context engine 集于一处。不带子命令运行 `lil-skrrt plugins` 会打开包含两个部分的复合交互界面：
...
参见 [Plugins](../user-guide/features/plugins.md) 和 [构建 Lil Skrrt Plugin](../guides/build-a-hermes-plugin.md)。
...
`lil-skrrt computer-use install` 是安装 `computer_use` toolset 使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口。它运行与首次启用 Computer Use 时 `lil-skrrt tools` 调用的相同上游安装程序，因此如果 toolset 切换未触发安装（例如在已配置用户的设置中），可以安全地用于重新运行安装。

`lil-skrrt update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下次 Lil Skrrt 更新时，使用此选项。
...
将 OpenClaw 设置迁移到 Lil Skrrt。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.hermes`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。

| 选项 | 说明 |
|--------|-------------|
| `--dry-run` | 预览将迁移的内容而不写入任何内容。 |
| `--preset <name>` | 迁移预设：`full`（所有兼容设置）或 `user-data`（排除基础设施配置）。两种预设都不导入密钥——需要显式传入 `--migrate-secrets`。 |
| `--overwrite` | 在冲突时覆盖现有 Lil Skrrt 文件（默认：当计划有冲突时拒绝应用）。 |
| `--migrate-secrets` | 在迁移中包含 API 密钥。即使在 `--preset full` 下也需要显式指定。 |
| `--no-backup` | 跳过迁移前对 `~/.hermes/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.hermes/backups/pre-migration-*.zip`；可用 `lil-skrrt import` 恢复）。 |
...
迁移涵盖 30+ 个类别，包括 persona、memory、skill、模型 provider、消息平台、agent 行为、会话策略、MCP 服务器、TTS 等。条目要么**直接导入**到 Lil Skrrt 等效项，要么**归档**以供手动审查。
...
| `--host` | `127.0.0.1` | 绑定地址 |
| `--no-open` | — | 不自动打开浏览器 |
| `--tui` | 关闭 | 通过 PTY/WebSocket 桥接在后台运行 `lil-skrrt --tui`，启用浏览器内 Chat 标签页。需要 `pip install 'hermes-agent[web,pty]'` 以及 Linux、macOS 或 WSL2 等 POSIX PTY 环境。 |
| `--insecure` | 关闭 | 允许绑定到非 localhost 主机。会在网络上暴露控制台凭据；仅在受信任的网络控制下使用。 |
| `--stop` | — | 停止正在运行的 `lil-skrrt dashboard` 进程并退出。 |
| `--status` | — | 列出正在运行的 `lil-skrrt dashboard` 进程并退出。 |
...
管理 profile——多个隔离的 Lil Skrrt 实例，每个实例拥有自己的 config、会话、skill 和主目录。
...
将 shell 补全脚本打印到 stdout。在 shell profile 中 source 输出内容，即可对 Lil Skrrt 命令、子命令和 profile 名称进行 Tab 补全。
...
**pip 安装：** `lil-skrrt update` 自动检测基于 pip 的安装——查询 PyPI 获取最新版本并运行 `pip install --upgrade hermes-agent`，而非 `git pull`。PyPI 发布跟踪标记版本（主要/次要版本），而非 `main` 上的每个 commit。使用 `--check` 查看是否有更新的 PyPI 版本可用，而不安装。
...
- **配对数据快照。** 即使 `--backup` 关闭，`lil-skrrt update` 也会在 `git pull` 前对 `~/.hermes/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `lil-skrrt backup restore --state pre-update` 回滚。
- **旧版 `lil-skrrt.service` 警告。** 如果 Lil Skrrt 检测到预重命名的 `lil-skrrt.service` systemd 单元（而非当前的 `hermes-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
...
| 命令 | 说明 |
|---------|-------------|
| `lil-skrrt version` | 打印版本信息。 |
| `lil-skrrt update` | 拉取最新变更并重新安装依赖。 |
| `lil-skrrt uninstall [--full] [--yes]` | 删除 Lil Skrrt，可选择删除所有 config/数据。 |

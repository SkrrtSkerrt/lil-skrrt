...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/SkrrtSkerrt/hermes-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Lil Skrrt"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Lil Skrrt ships with a large built-in skill library copied into `~/.hermes/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Lil Skrrt also syncs bundled skills on `lil-skrrt update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.hermes/skills/` tree, it is still shipped with Lil Skrrt; restore it with `lil-skrrt skills reset <name> --restore`.",
        "",

...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in Lil Skrrt — no API key required"
---

# xAI Grok OAuth (SuperGrok / X Premium+)

Lil Skrrt supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and Lil Skrrt automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Lil Skrrt — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
```bash
lil-skrrt auth add xai-oauth
```

### Remote / headless sessions

On servers, containers, or SSH sessions where no browser is available, Lil Skrrt detects the remote environment and prints the authorization URL instead of opening a browser.
...
If you don't have a regular SSH client (e.g. you're running Lil Skrrt inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console), the `ssh -L` recipe above isn't available. Use `--manual-paste` instead — Lil Skrrt skips the loopback listener and lets you paste the failed callback URL straight from your browser:

```bash
lil-skrrt auth add xai-oauth --manual-paste
# Or via the model picker:
lil-skrrt model --manual-paste
```

See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/SkrrtSkerrt/hermes-agent/issues/26923).

## How the Login Works

1. Lil Skrrt opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to Lil Skrrt and the tokens are saved to `~/.hermes/auth.json`.
4. From then on, Lil Skrrt refreshes the access token in the background — you stay signed in until you `lil-skrrt auth remove xai-oauth` or revoke access from your xAI account settings.
...
```bash
lil-skrrt config set model.default grok-4.3
lil-skrrt config set model.provider xai-oauth
```
...
```bash
lil-skrrt --provider xai-oauth        # canonical
lil-skrrt --provider grok-oauth       # alias
lil-skrrt --provider x-ai-oauth       # alias
lil-skrrt --provider xai-grok-oauth   # alias
```
...
To select xAI as the active provider, set `model.provider: xai-oauth` in `config.yaml` (use `lil-skrrt setup` for the guided flow) or pass `--provider xai-oauth` for a single invocation.

## Troubleshooting

### Token expired — not re-logging in automatically

Lil Skrrt refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Lil Skrrt surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Lil Skrrt marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `lil-skrrt auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, Lil Skrrt raises a timeout error.

**Fix:** re-run `lil-skrrt auth add xai-oauth` (or `lil-skrrt model`). The flow starts fresh.

### State mismatch (possible CSRF)

Lil Skrrt detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Lil Skrrt prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
This is **not** a stale-token problem — re-running `lil-skrrt model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/SkrrtSkerrt/hermes-agent/issues/26847)).

**Fix:** set `XAI_API_KEY` and switch to the API-key path:

```bash
export XAI_API_KEY=xai-...
lil-skrrt config set model.provider xai
```
...
**Fix:** run `lil-skrrt model` and pick the xAI Grok OAuth provider, or run `lil-skrrt auth add xai-oauth`.
...
```bash
lil-skrrt auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `lil-skrrt auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `lil-skrrt auth list xai-oauth` to see them).

## See Also

- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Lil Skrrt is on a different machine than your browser

---
sidebar_position: 7
title: "Use SOUL.md with Lil Skrrt"
description: "How to use SOUL.md to shape Lil Skrrt's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Lil Skrrt

`SOUL.md` is the **primary identity** for your Lil Skrrt instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Lil Skrrt to feel like the same assistant every time you talk to it — or if you want to replace the Lil Skrrt persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Lil Skrrt should be
- what Lil Skrrt should avoid stylistically
- how Lil Skrrt should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Lil Skrrt is and how Lil Skrrt speaks
...
Lil Skrrt now uses only the global SOUL file for the current instance:

```text
~/.hermes/SOUL.md
```

If you run Lil Skrrt with a custom home directory, it becomes:
...
Lil Skrrt automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Lil Skrrt does not overwrite it
- if the file exists but is empty, Lil Skrrt adds nothing from it to the prompt

## How Lil Skrrt uses it

When Lil Skrrt starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Lil Skrrt feels.
...
Lil Skrrt already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
Then restart Lil Skrrt or start a new session.
...
4. Talk to Lil Skrrt for a while
...
### I edited SOUL.md but Lil Skrrt still sounds the same
...
### Lil Skrrt is ignoring parts of my SOUL.md

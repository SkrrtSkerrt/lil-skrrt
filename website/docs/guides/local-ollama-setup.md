---
sidebar_position: 9
title: "Run Lil Skrrt Locally with Ollama — Zero API Cost"
description: "Step-by-step guide to running Lil Skrrt entirely on your own machine with Ollama and open-weight models like Gemma 4, no cloud API keys or paid subscriptions needed"
---

# Run Lil Skrrt Locally with Ollama — Zero API Cost
...
You'll set up Lil Skrrt running entirely on your own hardware, using [Ollama](https://ollama.com) as the model backend. No API keys, no subscriptions, no data leaving your machine. Once configured, Lil Skrrt works exactly like it does with OpenRouter or Anthropic — terminal commands, file editing, web browsing, delegation — but the model runs locally.

By the end, you'll have:

- Ollama serving one or more open-weight models
- Lil Skrrt connected to Ollama as a custom endpoint
...
Lil Skrrt is an **agentic** assistant — it edits files, runs commands, and browses the web through tool calls. Models without tool-call support can only chat; they can't take actions. For the full Lil Skrrt experience, use a model that supports tools (like `gemma4:31b`).
:::
...
You can pull several models and switch between them inside Lil Skrrt with `/model`. Ollama loads the active model into memory on demand and unloads idle ones automatically.
:::
...
Run the Lil Skrrt setup wizard:
...
Lil Skrrt will use the terminal tool, file operations, and your local model — no cloud calls.
...
By default, Ollama uses a 2048-token context. Lil Skrrt requires at least 64,000 tokens for agentic work with tools:
...
Then update your Lil Skrrt config to use `gemma4-64k` as the model name.
...
Once Lil Skrrt works locally in the CLI, you can expose it as a Telegram or Discord bot — still running entirely on your hardware.
...
- **Lil Skrrt has auto-repair** — it detects malformed tool calls and attempts to fix them automatically.
- **Set up a fallback** — if the local model fails 3 times, Lil Skrrt falls back to a cloud provider.
...
- **Terminal commands** — Lil Skrrt wraps the command, runs it, reads output regardless of model
...
- **Long context windows** — cloud models offer 100K–1M tokens; local runtimes often default below Lil Skrrt' 64K minimum unless you configure them

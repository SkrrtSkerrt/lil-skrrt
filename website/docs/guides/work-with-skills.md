...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Lil Skrrt new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Lil Skrrt how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Lil Skrrt installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Lil Skrrt to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Lil Skrrt but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
lil-skrrt skills install official/research/arxiv
...
```bash
# Check it's there
lil-skrrt skills list | grep arxiv
...
For how to ship skills in your own plugin, see [Build a Lil Skrrt Plugin → Bundle skills](/guides/build-a-hermes-plugin#bundle-skills).
...
When a skill with config is first loaded, Lil Skrrt prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
lil-skrrt skills config gif-search

# View all skill config
lil-skrrt config get skills.config
```
...
```bash
lil-skrrt chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.hermes/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Lil Skrrt may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Lil Skrrt will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.
...
**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Lil Skrrt to update the skill with what you learned. Skills that aren't maintained become liabilities.

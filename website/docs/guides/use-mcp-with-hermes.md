---
sidebar_position: 6
title: "Use MCP with Lil Skrrt"
description: "A practical guide to connecting MCP servers to Lil Skrrt, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Lil Skrrt

This guide shows how to actually use MCP with Lil Skrrt in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Lil Skrrt tool
- you want Lil Skrrt to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Lil Skrrt to internal APIs, databases, or company systems without modifying Lil Skrrt core

Do not use MCP when:
- a built-in Lil Skrrt tool already solves the job well
...
- Lil Skrrt remains the agent
- MCP servers contribute tools
- Lil Skrrt discovers those tools at startup or reload time
...
If you installed Lil Skrrt with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Lil Skrrt banner/status should show MCP integration when configured
- ask Lil Skrrt what tools it has available
...
## WSL2: bridge Lil Skrrt in WSL to Windows Chrome {#wsl2-bridge-hermes-in-wsl-to-windows-chrome}

This is the practical setup when:

- Lil Skrrt runs inside WSL2
...
In this setup, Lil Skrrt does **not** connect to Chrome directly. Instead:

- Lil Skrrt runs in WSL
- Lil Skrrt starts a local stdio MCP server
...
- Lil Skrrt stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Lil Skrrt core browser transport
...
```bash
lil-skrrt mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
lil-skrrt mcp test chrome-devtools-win
```

Then start a fresh Lil Skrrt session or run:
...
Once loaded, Lil Skrrt can use the MCP-prefixed browser tools directly. For example:
...
If Lil Skrrt runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Lil Skrrt from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Lil Skrrt from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Lil Skrrt:
...
So Lil Skrrt will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Lil Skrrt to reason over a bounded workspace.
...
Now Lil Skrrt can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Lil Skrrt core.
...
Because Lil Skrrt now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.

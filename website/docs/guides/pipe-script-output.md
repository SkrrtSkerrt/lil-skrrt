...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `lil-skrrt send`."
---

# Pipe Script Output to Messaging Platforms

`lil-skrrt send` is a small, scriptable CLI that pushes a message to any
messaging platform Lil Skrrt is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | lil-skrrt send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `lil-skrrt
...
lil-skrrt send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | hermes send --to telegram:-1001234567890

# Send a file
lil-skrrt send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
lil-skrrt send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
lil-skrrt send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
lil-skrrt send --list

# Filter by platform
lil-skrrt send --list telegram
```
...
Any platform Lil Skrrt ships adapters for works as a target:
...
`lil-skrrt send` resolves the message body in this order:

1. **Positional argument** — `lil-skrrt send --to telegram "hi"`
2. **`--file PATH`** — `lil-skrrt send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | lil-skrrt send --to telegram`

When stdin is a TTY (no pipe), Lil Skrrt does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  lil-skrrt send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `lil-skrrt send` reuses your Lil Skrrt config, the same script works on
any host where Lil Skrrt is installed — no need to export bot tokens into
...
`lil-skrrt send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  lil-skrrt send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
...
```bash
./train.py --epochs 200 && \
  lil-skrrt send --to telegram "training done" || \
  lil-skrrt send --to telegram "training failed (exit $?)"
```
...
lil-skrrt send --to telegram --quiet "keepalive" || {
...
## Does `lil-skrrt send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `lil-skrrt send` calls
...
pointing at the gateway; start it with `lil-skrrt gateway start` and retry.
...
"no channels discovered yet", start the gateway once (`lil-skrrt gateway
...
| Approach | Multi-platform | Reuses Lil Skrrt creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `lil-skrrt send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`lil-skrrt send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `lil-skrrt send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `lil-skrrt send` shares with cron delivery.

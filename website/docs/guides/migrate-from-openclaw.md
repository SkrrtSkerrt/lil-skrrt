...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Lil Skrrt — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`lil-skrrt claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Lil Skrrt. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

## Quick start

```bash
# Preview then migrate (always shows a preview first, then asks to confirm)
lil-skrrt claw migrate

# Preview only, no changes
lil-skrrt claw migrate --dry-run

# Full migration including API keys, skip confirmation
lil-skrrt claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.hermes/` (by default a single restore-point archive is written before apply, under `~/.hermes/backups/pre-migration-*.zip`; restorable with `lil-skrrt import`). |
...
| What | OpenClaw source | Lil Skrrt destination | Notes |
|------|----------------|-------------------|-------|
...
| Source | OpenClaw location | Lil Skrrt destination |
|--------|------------------|-------------------|
...
Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Lil Skrrt skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Lil Skrrt destination | Notes |
|------|---------------------|-------------------|-------|
...
| What | OpenClaw config path | Lil Skrrt config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Lil Skrrt config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Lil Skrrt field | Notes |
|----------------|-------------|-------|
...
| Platform | OpenClaw config path | Lil Skrrt `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Lil Skrrt path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct Lil Skrrt equivalent)

These are saved to `~/.hermes/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Lil Skrrt |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Lil Skrrt has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `lil-skrrt cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `lil-skrrt webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `lil-skrrt honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `lil-skrrt skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Lil Skrrt profiles |
...
4. **Verify API keys** — run `lil-skrrt status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart hermes-gateway`

6. **Check session policies** — verify `lil-skrrt config get session_reset` matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `lil-skrrt whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `lil-skrrt claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `lil-skrrt config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.

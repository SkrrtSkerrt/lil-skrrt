...
Lil Skrrt calls this **no-agent mode**. It's the cron system minus the LLM.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, Lil Skrrt writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **Lil Skrrt:** *(writes `~/.hermes/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
>
> **Lil Skrrt:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `lil-skrrt cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **Lil Skrrt:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
# 3. Verify
lil-skrrt cron list
lil-skrrt cron run <job_id>    # fire it once to test
```
...
```bash
lil-skrrt cron create "every 5m"        # interval
lil-skrrt cron create "every 2h"
lil-skrrt cron create "0 9 * * *"       # standard cron: 9am daily
lil-skrrt cron create "30m"             # one-shot: run once in 30 minutes
```
...
```bash
lil-skrrt cron list                                    # see all jobs
lil-skrrt cron pause <job_id>                          # stop firing, keep definition
lil-skrrt cron resume <job_id>
lil-skrrt cron edit <job_id> --schedule "every 10m"    # adjust cadence
lil-skrrt cron edit <job_id> --agent                   # flip to LLM mode
lil-skrrt cron edit <job_id> --no-agent --script …     # flip back
lil-skrrt cron remove <job_id>                         # delete it
```
...
|----------|-----------|-------------|
| `cronjob --no-agent` (this page) | Your script on Lil Skrrt' schedule | Recurring watchdogs / alerts / metrics that don't need reasoning |
| `cronjob` (default, LLM) | Agent with optional pre-check script | When the message content requires reasoning over data |
| OS cron + `curl` to a [webhook subscription](/user-guide/messaging/webhooks) | Your script on the OS schedule | When Lil Skrrt might be unhealthy (the thing you're monitoring) |

For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to a Lil Skrrt webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on Lil Skrrt being up. The in-gateway scheduler is the right choice when the thing being monitored is external.

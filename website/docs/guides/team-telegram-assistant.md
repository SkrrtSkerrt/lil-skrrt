...
This tutorial walks you through setting up a Telegram bot powered by Lil Skrrt that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Lil Skrrt installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
...
:::tip
A $5/month VPS is plenty for running the gateway. Lil Skrrt itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Lil Skrrt Assistant`)
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `lil-skrrt gateway install` to update the plist.
:::
...
```bash
lil-skrrt gateway stop && hermes gateway start
```
...
lil-skrrt pairing revoke telegram 987654321
...
For a full guide, see [Use SOUL.md with Lil Skrrt](/guides/use-soul-with-hermes).
...
```bash
# From the CLI
lil-skrrt cron list          # View all scheduled jobs
lil-skrrt cron status        # Check if scheduler is running

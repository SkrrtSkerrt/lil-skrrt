...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Lil Skrrt  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Lil Skrrt instantly when PRs are opened or updated.
:::
...
- **Lil Skrrt installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  lil-skrrt gateway install   # Install as a service
  # or
...
Make sure Lil Skrrt can access GitHub. Start a chat:
...
```
Run: gh pr list --repo SkrrtSkerrt/hermes-agent --state open --limit 3
```
...
Still in the chat, ask Lil Skrrt to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo SkrrtSkerrt/hermes-agent
```
...
A skill gives Lil Skrrt consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.
...
Verify it loaded — start `lil-skrrt` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Lil Skrrt your team's standards:
...
2. Teach Lil Skrrt your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
lil-skrrt gateway status    # Is the gateway running?
lil-skrrt cron list         # Is the job enabled?
```

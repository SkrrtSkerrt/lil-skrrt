...
```bash
lil-skrrt teams-pipeline validate
```
...
```bash
lil-skrrt teams-pipeline token-health
lil-skrrt teams-pipeline token-health --force-refresh
```
...
```bash
lil-skrrt teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
lil-skrrt teams-pipeline maintain-subscriptions
lil-skrrt teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Lil Skrrt cron (recommended if you already run the Lil Skrrt gateway)

Lil Skrrt ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.hermes/scripts/`. First create the script:
...
```bash
lil-skrrt teams-pipeline subscriptions   # should show expirationDateTime advanced
lil-skrrt teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
lil-skrrt teams-pipeline list
lil-skrrt teams-pipeline list --status failed
lil-skrrt teams-pipeline show <job-id>
```

### Replay a stored job

```bash
lil-skrrt teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
lil-skrrt teams-pipeline fetch --meeting-id <meeting-id>
lil-skrrt teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `lil-skrrt teams-pipeline maintain-subscriptions --dry-run`
- inspect `lil-skrrt teams-pipeline list --status failed`
...
- run `lil-skrrt teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `lil-skrrt teams-pipeline run`
...
- [ ] `lil-skrrt teams-pipeline validate` returns an OK snapshot
- [ ] `lil-skrrt teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Lil Skrrt cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.

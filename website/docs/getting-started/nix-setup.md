...
description: "Install and deploy Lil Skrrt with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---

# Nix & NixOS Setup

Lil Skrrt ships a Nix flake with three levels of integration:
...
**For non-NixOS users**, this only changes the install step. Everything after (`lil-skrrt setup`, `lil-skrrt gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage lil-skrrt the same way you manage any other NixOS service.
:::
...
nix run github:SkrrtSkerrt/hermes-agent -- setup
nix run github:SkrrtSkerrt/hermes-agent -- chat

# Or install persistently
nix profile install github:SkrrtSkerrt/hermes-agent
lil-skrrt setup
lil-skrrt chat
```

After `nix profile install`, `lil-skrrt`, `hermes-agent`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `lil-skrrt setup` walks you through provider selection, `lil-skrrt gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.hermes/`.

<details>
<summary><strong>Building from a local clone</strong></summary>

```bash
git clone https://github.com/SkrrtSkerrt/hermes-agent.git
cd hermes-agent
nix build
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    hermes-agent.url = "github:SkrrtSkerrt/hermes-agent";
  };
...
That's it. `nixos-rebuild switch` creates the `lil-skrrt` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
Setting `addToSystemPackages = true` does two things: puts the `lil-skrrt` CLI on your system PATH **and** sets `HERMES_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `lil-skrrt` in your shell creates a separate `~/.hermes/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `lil-skrrt` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.

- The routing is transparent: `lil-skrrt chat`, `lil-skrrt sessions list`, `lil-skrrt version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the lil-skrrt codebase, set `HERMES_DEV=1` to bypass container routing and run the local checkout directly
...
Users listed in `hostUsers` are automatically added to the `lil-skrrt` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo lil-skrrt chat` manually.
:::
...
lil-skrrt version
lil-skrrt config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Lil Skrrt defaults to OpenRouter.
:::
...
| Override the lil-skrrt package | `package` | `inputs.hermes-agent.packages.${system}.default.override { ... }` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). Lil Skrrt reads this file on every startup, so changes take effect with a `systemctl restart hermes-agent` — no container recreation needed.
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Lil Skrrt looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: Lil Skrrt loads its primary `SOUL.md` from `$HERMES_HOME/SOUL.md`, which in the NixOS module is `${services.hermes-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
Set `auth = "oauth"` for servers using OAuth 2.1. Lil Skrrt implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Lil Skrrt prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u lil-skrrt` (native):

```bash
# Container mode
docker exec -it hermes-agent \
  lil-skrrt mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
...
When lil-skrrt runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `lil-skrrt setup` | Config is declarative — edit `settings` in your Nix config |
| `lil-skrrt config edit` | Config is generated from `settings` |
| `lil-skrrt config set <key> <value>` | Config is generated from `settings` |
| `lil-skrrt gateway install` | The systemd service is managed by NixOS |
| `lil-skrrt gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **`HERMES_MANAGED=true`** environment variable — set by the systemd service, visible to the gateway process
2. **`.managed` marker file** in `HERMES_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it hermes-agent lil-skrrt config set ...` is also blocked)
...
When container mode is enabled, lil-skrrt runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the lil-skrrt package itself do **not** trigger recreation.
...
The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current lil-skrrt package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `lil-skrrt plugins install` needed.
...
Plugins are symlinked into `$HERMES_HOME/plugins/` at activation time. Lil Skrrt discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
The package's `site-packages` is added to PYTHONPATH in the lil-skrrt wrapper. `importlib.metadata` discovers the entry point at session start.
...
```nix
{
  inputs.hermes-agent.url = "github:SkrrtSkerrt/hermes-agent";
...
:::note
A build-time collision check prevents plugin packages from shadowing core lil-skrrt dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
| Check | What it tests |
|---|---|
| `package-contents` | `lil-skrrt` and `hermes-agent` binaries exist and `lil-skrrt version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `lil-skrrt --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `HERMES_MANAGED=true lil-skrrt config set ...` prints the NixOS error |
...
| `user` | `str` | `"lil-skrrt"` | System user |
| `group` | `str` | `"lil-skrrt"` | System group |
...
| `addToSystemPackages` | `bool` | `false` | Add `lil-skrrt` CLI to system PATH and set `HERMES_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `lil-skrrt gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the lil-skrrt user's per-user profile so terminal commands, skills, and cron jobs all see them |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.hermes` symlink to the service stateDir and are auto-added to the `lil-skrrt` group |
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Lil Skrrt binary + all Nix deps |
...
| `lil-skrrt version` shows old version | Container not restarted | `systemctl restart hermes-agent` |
| Permission denied on `/var/lib/hermes` | State dir is `0750 lil-skrrt:lil-skrrt` | Use `docker exec` or `sudo -u lil-skrrt` |
| `nix-collect-garbage` removed lil-skrrt | GC root missing | Restart the service (preStart recreates the GC root) |
...
| `unable to find user lil-skrrt` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |

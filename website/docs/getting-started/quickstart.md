...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Lil Skrrt — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Lil Skrrt setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Lil Skrrt Tutorials & Use Cases](https://www.youtube.com/channel/UCqB1bhMwGsW-yefBxYwFCCg) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Lil Skrrt Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Lil Skrrt for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Lil Skrrt working on my machine | `lil-skrrt setup` | Run a real chat and verify it responds |
| I already know my provider | `lil-skrrt model` | Save the config, then start chatting |
| I want a bot or always-on setup | `lil-skrrt gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `lil-skrrt model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `lil-skrrt model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Lil Skrrt cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
lil-skrrt postinstall     # optional: installs Node.js, browser, ripgrep, ffmpeg + runs setup
```
...
The single most important setup step. Use `lil-skrrt model` to walk through the choice interactively:
...
| **MiniMax (OAuth)** | MiniMax-M2.7 via browser OAuth — no API key needed | `lil-skrrt model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `lil-skrrt model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `lil-skrrt model` (requires `copilot` CLI + `copilot login`) |
...
Lil Skrrt requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `lil-skrrt model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Lil Skrrt separates secrets from normal config:
...
```bash
lil-skrrt config set model anthropic/claude-opus-4.6
lil-skrrt config set terminal.backend docker
lil-skrrt config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
a lil skrrt  # classic CLI
lil-skrrt --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Lil Skrrt ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `lil-skrrt` vs `lil-skrrt --tui`.
:::
...
- Lil Skrrt replies without error
...
```bash
lil-skrrt --continue    # Resume the most recent session
lil-skrrt -c            # Short form
```
...
```bash
lil-skrrt gateway setup    # Interactive platform configuration
```
...
- `lil-skrrt tools` — tune tool access per platform
- `lil-skrrt skills` — browse and install reusable workflows
...
```bash
lil-skrrt config set terminal.backend docker    # Docker isolation
lil-skrrt config set terminal.backend ssh       # Remote server
```
...
```bash
lil-skrrt skills search kubernetes
...
|---|---|---|
| Lil Skrrt opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `lil-skrrt model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `lil-skrrt gateway setup` and check `lil-skrrt gateway status` |
| `lil-skrrt --continue` can't find old session | Switched profiles or session never saved | Check `lil-skrrt sessions list` and confirm you're in the right profile |
...
| `lil-skrrt doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `lil-skrrt model`
3. `lil-skrrt setup`
4. `lil-skrrt sessions list`
5. `lil-skrrt --continue`
6. `lil-skrrt gateway status`
...
| `lil-skrrt tools` | Configure which tools are enabled per platform |
| `lil-skrrt setup` | Full setup wizard (configures everything at once) |
| `lil-skrrt doctor` | Diagnose issues |
| `lil-skrrt update` | Update to latest version |
| `lil-skrrt gateway` | Start the messaging gateway |
| `lil-skrrt --continue` | Resume last session |

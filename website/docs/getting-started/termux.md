...
description: "Run Lil Skrrt directly on an Android phone with Termux"
---

# Lil Skrrt on Android with Termux

This is the tested path for running Lil Skrrt directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Lil Skrrt from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Lil Skrrt now ships a Termux-aware installer path:
...
- links `lil-skrrt` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `lil-skrrt` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/hermes" "$PREFIX/bin/hermes"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `lil-skrrt` command persist across new shells without re-activating the venv every time.
...
### `lil-skrrt doctor` says ripgrep or Node is missing

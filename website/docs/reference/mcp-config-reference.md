...
description: "Reference for Lil Skrrt MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
Lil Skrrt may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Lil Skrrt only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Lil Skrrt does not create an empty MCP runtime toolset for that server.
...
Behavior:
- Lil Skrrt uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, dynamic client registration, token exchange, and refresh)

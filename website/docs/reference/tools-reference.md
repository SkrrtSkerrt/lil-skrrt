...
description: "Authoritative reference for Lil Skrrt built-in tools, grouped by toolset"
---

# Built-in Tools Reference

This page documents Lil Skrrt' built-in tools, grouped by toolset. Availability varies by platform, credentials, and enabled toolsets.
...
:::tip MCP Tools
In addition to built-in tools, Lil Skrrt can load tools dynamically from MCP servers. MCP tools appear with the prefix `mcp_<server>_` (e.g., `mcp_github_create_issue` for the `github` MCP server). See [MCP Integration](/user-guide/features/mcp) for configuration.
:::
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `execute_code` | Run a Python script that can call Lil Skrrt tools programmatically. Use this when you need 3+ tool calls with processing logic between them, need to filter/reduce large tool outputs before they enter your context, need conditional branching (… | — |
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `computer_use` | Background macOS desktop control via cua-driver — screenshots (SOM / vision / AX), click / drag / scroll / type / key / wait, list_apps, focus_app. Does NOT steal the user's cursor or keyboard focus. Works with any tool-capable model. macOS only. | `cua-driver` on `$PATH` (install via `lil-skrrt tools`). |
...
Opt-in toolset (not loaded in the default `hermes-cli` set). Add via `--toolsets video_gen` or enable it in `lil-skrrt tools` → Video Generation, which also walks you through picking a backend.
...
Registered by the bundled `spotify` plugin. Requires an OAuth token — run `lil-skrrt spotify setup` once to authorize.

...
This page covers all commands related to [Lil Skrrt profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `lil-skrrt profile` without a subcommand shows help.
...
## `lil-skrrt profile use`

```bash
lil-skrrt profile use <name>
```

Sets `<name>` as the active profile. All subsequent `lil-skrrt` commands (without `-p`) will use this profile.
...
lil-skrrt profile use default
```

## `lil-skrrt profile create`

```bash
lil-skrrt profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `lil-skrrt profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `lil-skrrt update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
lil-skrrt profile create mybot

# Clone config only from current profile
lil-skrrt profile create work --clone

# Clone everything from current profile
lil-skrrt profile create backup --clone-all

# Clone config from a specific profile
lil-skrrt profile create work2 --clone --clone-from work
```

## `lil-skrrt profile describe`

```bash
lil-skrrt profile describe [<name>] [options]
```
...
lil-skrrt profile describe researcher

# Set it explicitly
lil-skrrt profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
lil-skrrt profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
lil-skrrt profile describe --all --auto
```

## `lil-skrrt profile delete`

```bash
lil-skrrt profile delete <name> [options]
```
...
```bash
lil-skrrt profile delete mybot
lil-skrrt profile delete mybot --yes
```
...
## `lil-skrrt profile show`

```bash
lil-skrrt profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Lil Skrrt home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
## `lil-skrrt profile alias`

```bash
lil-skrrt profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Lil Skrrt installation.
...
```bash
lil-skrrt profile alias work
# Creates/updates ~/.local/bin/work

lil-skrrt profile alias work --name mywork
# Creates ~/.local/bin/mywork

lil-skrrt profile alias work --remove
# Removes the wrapper script
```

## `lil-skrrt profile rename`

```bash
lil-skrrt profile rename <old-name> <new-name>
```
...
```bash
lil-skrrt profile rename mybot assistant
...
```bash
lil-skrrt profile export work
# Creates work.tar.gz in the current directory

lil-skrrt profile export work -o ./work-2026-03-29.tar.gz
```

## `lil-skrrt profile import`

```bash
lil-skrrt profile import <archive> [options]
```
...
```bash
lil-skrrt profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

lil-skrrt profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`lil-skrrt profile export` / `import` are still the right commands for
...
### `lil-skrrt profile install`

```bash
lil-skrrt profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `lil-skrrt -p telemetry`). |
...
lil-skrrt profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
lil-skrrt profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
lil-skrrt profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
lil-skrrt profile install ./telemetry/
```

### `lil-skrrt profile update`

```bash
lil-skrrt profile update <name> [--force-config] [--yes]
```
...
### `lil-skrrt profile info`

```bash
lil-skrrt profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Lil Skrrt version, author, env var requirements, the source URL/path, and
...
`lil-skrrt profile list` also shows the distribution name and version in a
`Distribution` column, and `lil-skrrt profile show <name>` / `delete <name>`
...
lil-skrrt profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
lil-skrrt profile install https://github.com/your-org/internal-assistant.git
```
...
Lil Skrrt version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Lil Skrrt can clone from.
3. Tell recipients to run `lil-skrrt profile install <your-repo-url>`.
...
## `lil-skrrt -p` / `lil-skrrt --profile`

```bash
lil-skrrt -p <name> <command> [options]
lil-skrrt --profile <name> <command> [options]
```

Global flag to run any Lil Skrrt command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
lil-skrrt -p work chat -q "Check the server status"
lil-skrrt --profile dev gateway start
lil-skrrt -p personal skills list
lil-skrrt -p work config edit
```

## `lil-skrrt completion`

```bash
lil-skrrt completion <shell>
```
...
- `lil-skrrt profile <TAB>` — subcommands (list, use, create, etc.)
- `lil-skrrt profile use <TAB>` — profile names
- `lil-skrrt -p <TAB>` — profile names

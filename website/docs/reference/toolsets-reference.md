...
description: "Reference for Lil Skrrt core, composite, platform, and dynamic toolsets"
---
...
```bash
lil-skrrt chat --toolsets web,file,terminal
lil-skrrt chat --toolsets debugging        # composite — expands to file + terminal + web
lil-skrrt chat --toolsets all              # everything
```
...
```bash
lil-skrrt tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Lil Skrrt tools programmatically. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `lil-skrrt tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
...
## Relationship to `lil-skrrt tools`

The `lil-skrrt tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.

...
description: "Frequently asked questions and solutions to common issues with Lil Skrrt"
---
...
### What LLM providers work with Lil Skrrt?
...
Set your provider with `lil-skrrt model` or by editing `~/.hermes/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Lil Skrrt requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Lil Skrrt from inside it. The standard install command works perfectly in WSL2:

```bash
curl -fsSL https://raw.githubusercontent.com/SkrrtSkerrt/lil-skrrt/main/scripts/install.sh | bash
```

### I run Lil Skrrt in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Lil Skrrt inside WSL2
...
- let Lil Skrrt use the resulting MCP browser tools

This is more reliable than trying to force Lil Skrrt core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Lil Skrrt](../guides/use-mcp-with-hermes.md#wsl2-bridge-hermes-in-wsl-to-windows-chrome)
...
Yes — Lil Skrrt now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Lil Skrrt does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.hermes/`.

### Can I use it offline / with local models?

Yes. Run `lil-skrrt model`, select **Custom endpoint**, and enter your server's URL:
...
Lil Skrrt persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Lil Skrrt — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Lil Skrrt auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `HERMES_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Lil Skrrt itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Lil Skrrt instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Lil Skrrt programmatically:
...
#### `lil-skrrt: command not found` after installation
...
**Cause:** Lil Skrrt requires Python 3.11 or newer.
...
**Cause:** Lil Skrrt builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Lil Skrrt runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Lil Skrrt auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.hermes/config.yaml`:
...
After adding a new provider via `lil-skrrt model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `lil-skrrt model` (from terminal) |
| Enter/change API keys | `lil-skrrt model` (from terminal) |
...
# Or set directly
lil-skrrt config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```
...
# Set a valid model
lil-skrrt config set HERMES_MODEL anthropic/claude-opus-4.7

# Or specify per-session
lil-skrrt chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `lil-skrrt chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Lil Skrrt detected the wrong context length for your model.
...
lil-skrrt chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Lil Skrrt may have the wrong context length for your model. Check what it detected:
...
**Cause:** Lil Skrrt detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Lil Skrrt never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `lil-skrrt chat`
...
**Solution:**
- Verify your bot token is valid with `lil-skrrt gateway setup`
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'lil-skrrt gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `lil-skrrt gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
lil-skrrt gateway install    # Re-snapshots your current PATH
lil-skrrt gateway start      # Detects the updated plist and reloads
```
...
**Solution:**
- Try a faster/smaller model: `lil-skrrt chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `lil-skrrt chat -t "terminal"`
...
- [Use MCP with Lil Skrrt](/guides/use-mcp-with-hermes)
...
:::warning
If an MCP server crashes mid-request, Lil Skrrt will report a timeout. Check the server's own logs (not just Lil Skrrt logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `lil-skrrt profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `lil-skrrt update`?

`lil-skrrt update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `lil-skrrt update` once — it covers every profile on the machine.
...
**Solution: Delegation config.** Lil Skrrt can route subagents to a different model automatically. Set this in `~/.hermes/config.yaml`:
...
Now when you tell Lil Skrrt "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Lil Skrrt do this?

**Current limitation:** Lil Skrrt profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Lil Skrrt reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `lil-skrrt skills config` settings don't seem to take effect.

**Solution:** Use `lil-skrrt skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`lil-skrrt gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Lil Skrrt creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Lil Skrrt to another machine
...
1. Install Lil Skrrt on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/SkrrtSkerrt/lil-skrrt/main/scripts/install.sh | bash
   ```
...
3. Copy the zip to the new machine and import it:
   ...
   # On the new machine
   lil-skrrt import ~/hermes-backup-<timestamp>.zip
   ```

4. On the new machine, run `lil-skrrt setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
lil-skrrt profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
lil-skrrt profile import ./work-backup.tar.gz work
```
...
### `lil-skrrt backup` vs `lil-skrrt profile export`

| Feature | `lil-skrrt backup` | `lil-skrrt profile export` |
| :--- | :--- | :--- |
...
:::tip
`lil-skrrt backup` produces a consistent snapshot even while Lil Skrrt is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::
...
lil-skrrt config show | head -20

# Re-run model selection
lil-skrrt model

# Or test with a known-good model
lil-skrrt chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/SkrrtSkerrt/hermes-agent/issues)
2. **Ask the community:** [SkrrtSkerrt Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Lil Skrrt version (`lil-skrrt --version`), and the full error message

...
2. Lil Skrrt stores that single token in `~/.hermes/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `lil-skrrt` (or the gateway, or a cron job) starts, after `~/.hermes/.env` has loaded, Lil Skrrt calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default Lil Skrrt **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every Lil Skrrt process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.
...
2. Create or pick a **Project** (e.g. "Lil Skrrt keys").
...
4. **Machine accounts → New machine account → My Lil Skrrt machine** → **Projects** tab → grant Read access to your project.
...
```bash
lil-skrrt secrets bitwarden status
```

From now on, every `lil-skrrt` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `lil-skrrt secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `lil-skrrt secrets bitwarden status` | Show config + binary version + token presence |
| `lil-skrrt secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `lil-skrrt secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `lil-skrrt secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `lil-skrrt secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |
...
| `cache_ttl_seconds` | `300` | How long an in-process fetch result is reused. Set to `0` to disable caching. Cache is per-process; new `lil-skrrt` invocations start fresh. |
...
Bitwarden never blocks Lil Skrrt startup. If anything goes wrong, you'll see a one-line warning in stderr and Lil Skrrt continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `lil-skrrt secrets bitwarden setup` |
...
- Lil Skrrt will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — Lil Skrrt does not auto-upgrade `bws` to "latest" because upstream release shapes can change.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple Lil Skrrt installations.

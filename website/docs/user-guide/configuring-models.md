...
Lil Skrrt uses two kinds of model slots:
...
[Nous Portal](/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `lil-skrrt setup --portal` to log in and set Nous as your provider in one command. Inspect what's wired up with `lil-skrrt portal status`.
:::
...
- **Right** — the curated model list for the selected provider. These are the agentic models Lil Skrrt recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and Lil Skrrt writes it to `~/.hermes/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.
...
Every auxiliary task defaults to `auto` — meaning Lil Skrrt uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
| **Skills Hub** | `lil-skrrt skills search` uses this. Usually fine at `auto`. |
...
When you save via the dashboard, Lil Skrrt writes to `~/.hermes/config.yaml`:
...
`provider: auto` with `model: ''` tells Lil Skrrt to use the main model for that task.

## When does it take effect?

- **CLI** (`lil-skrrt chat`): next `lil-skrrt chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`lil-skrrt gateway restart`) if you want to force all sessions to pick up the change.
...
Lil Skrrt lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `lil-skrrt setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but Lil Skrrt switched providers on me
...
Inside any `lil-skrrt chat` session:
...
```bash
lil-skrrt config set model.aliases.fav anthropic/claude-opus-4.6
lil-skrrt config set model.aliases.grok x-ai/grok-4
```
...
### `lil-skrrt model` subcommand

```bash
lil-skrrt model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`lil-skrrt model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.model` in `~/.hermes/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `lil-skrrt config get model` and `lil-skrrt status`.

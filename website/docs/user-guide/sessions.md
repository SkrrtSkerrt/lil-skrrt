...
Lil Skrrt automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
Lil Skrrt stores session history so it can resume conversations, but it does not
...
Lil Skrrt explicitly injects for that turn.
...
For example, if a user sends an image and asks Lil Skrrt to make a meme from it,
Lil Skrrt may inspect that image once with vision and run an image-processing
...
`lil-skrrt sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`lil-skrrt` or `lil-skrrt chat`) |
...
lil-skrrt -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
lil-skrrt -c "my project"   # → resumes "my project #3"
```
...
lil-skrrt --resume 20250305_091523_a1b2c3d4
lil-skrrt -r 20250305_091523_a1b2c3d4

# Resume by title
lil-skrrt --resume "refactoring auth"

# Or with the chat subcommand
lil-skrrt chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `lil-skrrt sessions list`.

### Conversation Recap on Resume

When you resume a session, Lil Skrrt displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Lil Skrrt session." />
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `lil-skrrt -r "<title>"` from the shell) and pick up where the platform left off.
...
Lil Skrrt automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `lil-skrrt sessions list` or `lil-skrrt sessions browse`.
...
```bash
lil-skrrt sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Lil Skrrt creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`lil-skrrt -c "my project"`), it automatically picks the most recent session in the lineage.
...
Lil Skrrt provides a full set of session management commands via `lil-skrrt sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
lil-skrrt sessions list

# Filter by platform
lil-skrrt sessions list --source telegram

# Show more sessions
lil-skrrt sessions list --limit 50
```
...
lil-skrrt sessions export backup.jsonl

# Export sessions from a specific platform
lil-skrrt sessions export telegram-history.jsonl --source telegram

# Export a single session
lil-skrrt sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
lil-skrrt sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
lil-skrrt sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
lil-skrrt sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
lil-skrrt sessions prune

# Custom age threshold
lil-skrrt sessions prune --older-than 30

# Only prune sessions from a specific platform
lil-skrrt sessions prune --source telegram --older-than 60

# Skip confirmation
lil-skrrt sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`lil-skrrt insights`](/reference/cli-commands#hermes-insights).
...
When Lil Skrrt cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Lil Skrrt uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Lil Skrrt in the same Discord channel without sharing transcript history
...
read by Lil Skrrt. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Lil Skrrt process in the same `HERMES_HOME`
...
lil-skrrt sessions delete <session_id>

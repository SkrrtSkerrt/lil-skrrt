...
| | |
|---|---|
| Source | Optional — install with `lil-skrrt skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Lil Skrrt |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Lil Skrrt terminal calls (desktop app flow only)
...
### Service Account (recommended for Lil Skrrt)
...
## Lil Skrrt Execution Pattern (desktop app flow)

Lil Skrrt terminal commands are non-interactive by default and can lose auth context between calls.

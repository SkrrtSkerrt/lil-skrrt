...
| | |
|---|---|
| Source | Optional — install with `lil-skrrt skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Lil Skrrt core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Lil Skrrt native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Lil Skrrt native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Lil Skrrt native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended Lil Skrrt workflow:
...
## Recommended Lil Skrrt usage patterns
...
- Do not silently choose Parallel when Lil Skrrt native tools are already sufficient.

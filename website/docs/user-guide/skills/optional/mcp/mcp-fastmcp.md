...
| | |
|---|---|
| Source | Optional — install with `lil-skrrt skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Lil Skrrt or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Lil Skrrt. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
When the goal is Lil Skrrt integration, either:
...
### Lil Skrrt cannot see the deployed server

The server-building part may be correct while the Lil Skrrt config is not. Load the `native-mcp` skill and configure the server in `~/.hermes/config.yaml`, then restart Lil Skrrt.

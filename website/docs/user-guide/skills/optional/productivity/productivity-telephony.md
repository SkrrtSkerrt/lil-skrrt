---
title: "Telephony — Give Lil Skrrt phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Lil Skrrt phone capabilities without core tool changes"
---
...
Give Lil Skrrt phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `lil-skrrt skills install official/productivity/telephony` |
...
This optional skill gives Lil Skrrt practical phone capabilities while keeping telephony out of the core tool list.
...
It does **not** turn Lil Skrrt into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Lil Skrrt memory
...
### 1) "I want Lil Skrrt to own a real phone number"
Use **Twilio**.
...
- pairs well with Lil Skrrt `text_to_speech` plus a public file host or tunnel
...
```bash
lil-skrrt skills search telephony
lil-skrrt skills install official/productivity/telephony
```
...
Then save credentials into Lil Skrrt:
...
This is the main path for reusing Lil Skrrt's existing `text_to_speech` support.

Use this when:
- you want the call to use Lil Skrrt's configured TTS voice rather than Twilio `<Say>`
...
Recommended Lil Skrrt TTS -> Twilio Play workflow:

1. Generate the audio with Lil Skrrt `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Lil Skrrt to create the message audio with `text_to_speech`
...
Important note:
- Lil Skrrt TTS is great for prerecorded outbound messages
...
- Lil Skrrt STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Lil Skrrt memory.
...
- Do not store arbitrary third-party phone numbers in Lil Skrrt memory.

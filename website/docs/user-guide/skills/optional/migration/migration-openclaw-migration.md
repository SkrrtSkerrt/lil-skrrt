---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Lil Skrrt"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Lil Skrrt"
---
...
Migrate a user's OpenClaw customization footprint into Lil Skrrt. Imports Hermes-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `lil-skrrt skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Lil Skrrt (SkrrtSkerrt) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Lil Skrrt`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Lil Skrrt Migration

Use this skill when a user wants to move their OpenClaw setup into Lil Skrrt with minimal manual cleanup.
...
```bash
lil-skrrt claw migrate              # Full interactive migration
...
lil-skrrt claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `lil-skrrt setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Lil Skrrt home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Lil Skrrt memory entries
- merge OpenClaw command approval patterns into Lil Skrrt `command_allowlist`
...
- optionally copy the OpenClaw workspace instructions file into a chosen Lil Skrrt workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct Lil Skrrt destination
...
Lil Skrrt CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Lil Skrrt. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Lil Skrrt workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Lil Skrrt skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Lil Skrrt unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Lil Skrrt targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Lil Skrrt destination. Unsupported auth blobs must still be reported as skipped.
...
- Lil Skrrt persona state imported
- Lil Skrrt memory files populated with converted OpenClaw knowledge

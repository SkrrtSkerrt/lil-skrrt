...
description: "Configure and use Honcho memory with Lil Skrrt -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Lil Skrrt -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `lil-skrrt skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Lil Skrrt

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Lil Skrrt profile its own peer identity while sharing a unified view of the user.
...
```bash
lil-skrrt honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Lil Skrrt profile's AI peer
...
Honcho models conversations as interactions between **peers**. Lil Skrrt creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Lil Skrrt instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Lil Skrrt run |
| `global` | Single session across all directories |

Manual override: `lil-skrrt honcho map my-project-name`
...
Each Lil Skrrt profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
1. Creates a `lil-skrrt.coder` host block in `honcho.json`
...
```bash
lil-skrrt honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Lil Skrrt when Honcho memory is active.
...
Run `lil-skrrt honcho setup`. Ensure `memory.provider: honcho` is in `~/.hermes/config.yaml`.

### Memory not persisting across sessions
Check `lil-skrrt honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `lil-skrrt profile create <name> --clone`. For existing profiles: `lil-skrrt honcho sync`.
...
| Command | Description |
|---------|-------------|
| `lil-skrrt honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `lil-skrrt honcho status` | Show resolved config, connection test, peer info for active profile |
| `lil-skrrt honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `lil-skrrt honcho disable` | Disable Honcho for the active profile |
| `lil-skrrt honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `lil-skrrt honcho peers` | Show peer identities across all profiles |
| `lil-skrrt honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `lil-skrrt honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `lil-skrrt honcho sessions` | List known directory-to-session-name mappings |
| `lil-skrrt honcho map <name>` | Map current working directory to a Honcho session name |
| `lil-skrrt honcho identity` | Seed AI peer identity or show both peer representations |
| `lil-skrrt honcho sync` | Create host blocks for all Lil Skrrt profiles that don't have one yet |
| `lil-skrrt honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Lil Skrrt + Honcho |
| `lil-skrrt memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `lil-skrrt memory status` | Show active memory provider and config |
| `lil-skrrt memory off` | Disable external memory provider |

...
| | |
|---|---|
| Source | Optional — install with `lil-skrrt skills install official/autonomous-ai-agents/blackbox` |
...
Delegate coding tasks to [Blackbox AI](https://www.blackbox.ai/) via the Lil Skrrt terminal. Blackbox is a multi-model coding agent CLI that dispatches tasks to multiple LLMs (Claude, Codex, Gemini, Blackbox Pro) and uses a judge to select the best implementation.

...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Lil Skrrt agent runs by POSTing events to a URL.
...
All management is via the `lil-skrrt webhook` CLI command:

### Create a subscription
```bash
lil-skrrt webhook subscribe <name> \
...
```bash
lil-skrrt webhook remove <name>
```

### Test a subscription
```bash
lil-skrrt webhook test <name>
lil-skrrt webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
lil-skrrt webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
lil-skrrt webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
lil-skrrt webhook subscribe alerts \
...
1. `lil-skrrt webhook subscribe` writes to `~/.hermes/webhook_subscriptions.json`
...
4. **Signature mismatch?** Verify the secret in your service matches the one from `lil-skrrt webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `lil-skrrt webhook test <name>` to verify the route works.

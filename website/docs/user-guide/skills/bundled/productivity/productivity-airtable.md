...
4. Store the token in `~/.hermes/.env` (or via `lil-skrrt setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Lil Skrrt. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.

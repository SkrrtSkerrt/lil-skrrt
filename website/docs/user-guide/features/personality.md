...
description: "Customize Lil Skrrt's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Lil Skrrt's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who Lil Skrrt is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Lil Skrrt now seeds a default `SOUL.md` automatically in:

```text
~/.hermes/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run Lil Skrrt with a custom home directory, it will use:
...
- Lil Skrrt creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Lil Skrrt loads `SOUL.md` only from `HERMES_HOME`
- Lil Skrrt does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Lil Skrrt falls back to a built-in default identity
...
If Lil Skrrt loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the Lil Skrrt instance itself.

That also makes it easier to teach users:
- "Edit `~/.hermes/SOUL.md` to change Lil Skrrt' default personality."
...
- how Lil Skrrt should handle uncertainty, disagreement, or ambiguity
...
## What Lil Skrrt injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Lil Skrrt falls back to a built-in default identity ("You are Lil Skrrt, an intelligent AI assistant created by SkrrtSkerrt..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Lil Skrrt ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Lil Skrrt, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Lil Skrrt its persistent default personality unless the overlay meaningfully changes it.
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Lil Skrrt speaks
- `display.skin` and `/skin` affect how Lil Skrrt looks in the terminal

...
Lil Skrrt runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Lil Skrrt
...
1. Lil Skrrt captures a baseline of current diagnostics for the file.
...
…). Lil Skrrt auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
`node_modules` tree — Lil Skrrt installs both packages together when you
run `lil-skrrt lsp install typescript` or auto-install fires on first
use.

## CLI

```
lil-skrrt lsp status          # service state + per-server install status
lil-skrrt lsp list            # registry, optionally --installed-only
lil-skrrt lsp install <id>    # eagerly install one server
lil-skrrt lsp install-all     # try every server with a known recipe
lil-skrrt lsp restart         # tear down running clients
lil-skrrt lsp which <id>      # print resolved binary path
```

`lil-skrrt lsp status` is the best starting point — it shows which
...
When `install_strategy: auto`, Lil Skrrt installs binaries into
...
Servers are kept alive for the life of the Lil Skrrt process. There's
...
**`lil-skrrt lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<HERMES_HOME>/lsp/bin/`. Run
`lil-skrrt lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `lil-skrrt lsp status`**
...
When `lil-skrrt lsp status` shows a `Backend warnings` section, install
...
the rest of the session. Run `lil-skrrt lsp restart` to clear the set;

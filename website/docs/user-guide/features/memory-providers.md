...
Lil Skrrt ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
lil-skrrt memory setup      # interactive picker + configuration
lil-skrrt memory status     # check what's active
lil-skrrt memory off        # disable external provider
```

You can also select the active memory provider via `lil-skrrt plugins` → Provider Plugins → Memory Provider.
...
When a memory provider is active, Lil Skrrt automatically:
...
**Setup Wizard:**
```bash
lil-skrrt memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `lil-skrrt honcho setup` command still works (it now redirects to `lil-skrrt memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
:::tip Migrating from `lil-skrrt honcho`
If you previously used `lil-skrrt honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Lil Skrrt profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Lil Skrrt profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Lil Skrrt profile. Host key `lil-skrrt` → default; `lil-skrrt.<profile>` for others. |
...
```bash
lil-skrrt profile create coder --clone
```

`--clone` creates a `lil-skrrt.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
lil-skrrt honcho sync
```

Scans every Lil Skrrt profile, creates host blocks for any profile without one, inherits settings from the default `lil-skrrt` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
lil-skrrt memory setup    # select "openviking"
# Or manually:
lil-skrrt config set memory.provider openviking
...
**Setup:**
```bash
lil-skrrt memory setup    # select "mem0"
# Or manually:
lil-skrrt config set memory.provider mem0
...
**Setup:**
```bash
lil-skrrt memory setup    # select "hindsight"
# Or manually:
lil-skrrt config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.hermes/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p lil-skrrt ui start`
...
| `bank_id` | `lil-skrrt` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Lil Skrrt and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/SkrrtSkerrt/hermes-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
lil-skrrt memory setup    # select "holographic"
# Or manually:
lil-skrrt config set memory.provider holographic
```
...
**Setup:**
```bash
lil-skrrt memory setup    # select "retaindb"
# Or manually:
lil-skrrt config set memory.provider retaindb
...
# Then configure Hermes
lil-skrrt memory setup    # select "byterover"
# Or manually:
lil-skrrt config set memory.provider byterover
```
...
**Setup:**
```bash
lil-skrrt memory setup    # select "supermemory"
# Or manually:
lil-skrrt config set memory.provider supermemory
...
| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `lil-skrrt` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `hermes-{identity}` → `hermes-coder`) to isolate memories per Lil Skrrt profile

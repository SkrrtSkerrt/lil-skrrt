...
description: "Durable SQLite-backed task board for coordinating multiple Lil Skrrt profiles"
---
...
Lil Skrrt Kanban is a durable task board, shared across all your Lil Skrrt profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.hermes/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.hermes/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `lil-skrrt kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `lil-skrrt kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
lil-skrrt kanban boards list

# Create a new board.
lil-skrrt kanban boards create atm10-server \
    --name "ATM10 Server" \
...
lil-skrrt kanban --board atm10-server list
lil-skrrt kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
lil-skrrt kanban boards switch atm10-server
lil-skrrt kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
lil-skrrt kanban boards rename atm10-server "ATM10 (Prod)"
...
3. `~/.hermes/kanban/current` — the slug persisted by `lil-skrrt kanban
   boards switch`.
4. `default`.
...
`lil-skrrt dashboard` → Kanban tab shows a board switcher at the top as soon
...
lil-skrrt kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
lil-skrrt kanban watch

# 5. See the board (you)
lil-skrrt kanban list
lil-skrrt kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `lil-skrrt kanban show t_abcd`.
...
for debugging. Standard gateway supervision applies: run `lil-skrrt gateway
...
until one comes up — `lil-skrrt kanban create` warns about this at creation
time.

Running `lil-skrrt kanban daemon` as a separate process is **deprecated**;
...
lil-skrrt kanban create "nightly ops review" \
    --assignee ops \
...
```bash
lil-skrrt kanban complete t_abc t_def t_hij --result "batch wrap"
lil-skrrt kanban archive  t_abc t_def t_hij
lil-skrrt kanban unblock  t_abc t_def
lil-skrrt kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `lil-skrrt kanban`.** When the dispatcher spawns a worker it sets `HERMES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `lil-skrrt kanban` CLI.
...
**Zero schema footprint on normal sessions.** A regular `lil-skrrt chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `HERMES_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```bash
lil-skrrt -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
lil-skrrt -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
lil-skrrt kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

lil-skrrt kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
lil-skrrt -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
lil-skrrt -p orchestrator skills reset kanban-orchestrator --restore
```
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `lil-skrrt profile describe --auto`). |
...
The plugin uses the standard Lil Skrrt dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
lil-skrrt kanban init                                     # create kanban.db + print daemon hint
lil-skrrt kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
lil-skrrt kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived] [--json]
lil-skrrt kanban show <id> [--json]
lil-skrrt kanban assign <id> <profile>                    # or 'none' to unassign
lil-skrrt kanban link <parent_id> <child_id>
lil-skrrt kanban unlink <parent_id> <child_id>
lil-skrrt kanban claim <id> [--ttl SECONDS]
lil-skrrt kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
lil-skrrt kanban complete <id>... [--result "..."]
lil-skrrt kanban block <id> "<reason>" [--ids <id>...]
lil-skrrt kanban unblock <id>...
lil-skrrt kanban archive <id>...

lil-skrrt kanban tail <id>                                # follow a single task's event stream
lil-skrrt kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
lil-skrrt kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
lil-skrrt kanban runs <id> [--json]                       # attempt history (one row per run)
lil-skrrt kanban assignees [--json]                       # profiles on disk + per-assignee task counts
lil-skrrt kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
lil-skrrt kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `hermes gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
lil-skrrt kanban stats [--json]                           # per-status + per-assignee counts
lil-skrrt kanban log <id> [--tail BYTES]                  # worker log from ~/.hermes/kanban/logs/
lil-skrrt kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
lil-skrrt kanban notify-list [<id>] [--json]
lil-skrrt kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
lil-skrrt kanban context <id>                             # what a worker sees
lil-skrrt kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
lil-skrrt kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
Every `lil-skrrt kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `lil-skrrt chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `hermes_cli.kanban.run_slash()` entry point that reuses the `lil-skrrt kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `lil-skrrt kanban`. You don't have to leave the chat to drive the board.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`lil-skrrt kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `lil-skrrt kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |
...
```bash
lil-skrrt kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
...
```bash
lil-skrrt kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
lil-skrrt kanban notify-list
lil-skrrt kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
lil-skrrt kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
lil-skrrt kanban runs t_abcd
...
**Bulk close caveat.** `lil-skrrt kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`lil-skrrt kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `lil-skrrt kanban heartbeat $TASK` to signal liveness during long operations. |
...
`lil-skrrt kanban tail <id>` shows these for a single task. `lil-skrrt kanban watch` streams them board-wide.

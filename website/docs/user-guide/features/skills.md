...
You can also point Lil Skrrt at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Lil Skrrt to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.hermes/plans/` relative to the active workspace/backend working directory.
...
When a missing value is encountered, Lil Skrrt asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `lil-skrrt setup` or `~/.hermes/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `lil-skrrt config migrate` prompts for unconfigured settings, and `lil-skrrt config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
If you maintain skills outside of Lil Skrrt — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Lil Skrrt to scan those directories too.
...
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Lil Skrrt process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Lil Skrrt ignores it without errors. Useful for optional shared directories that may not be present on every machine.
...
- `description` (optional) — short text shown in `/bundles` and `lil-skrrt bundles list`.
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Lil Skrrt. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `lil-skrrt skills search <query> --source skills-sh`. Lil Skrrt resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Lil Skrrt currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Lil Skrrt repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Lil Skrrt can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
Lil Skrrt can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
- Lil Skrrt source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Lil Skrrt supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Lil Skrrt source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Lil Skrrt can search and convert agent entries from LobeHub's public catalog into installable Lil Skrrt skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Lil Skrrt source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Lil Skrrt integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Lil Skrrt' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Lil Skrrt source id: `browse-sh`
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Lil Skrrt fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- Lil Skrrt source id: `url`
...
Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `lil-skrrt skills update` re-fetches from the same URL automatically when you want to refresh.
...
`lil-skrrt skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Lil Skrrt | Always trusted |
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Lil Skrrt users add with `lil-skrrt skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
Lil Skrrt discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Lil Skrrt user can subscribe and install:
...
The `lil-skrrt skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `lil-skrrt skills tap list` shows the effective path per tap.
...
## Bundled skill updates (`lil-skrrt skills reset`)

Lil Skrrt ships with a set of bundled skills in `skills/` inside the repo. On install and on every `lil-skrrt update`, a sync pass copies those into `~/.hermes/skills/` and records a manifest at `~/.hermes/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Lil Skrrt recomputes the hash of your local copy and compares it to the origin hash:
...
`lil-skrrt skills reset` is the escape hatch:
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HERMES_HOME`, so `lil-skrrt -p coder skills reset <name>` only affects that profile.
:::

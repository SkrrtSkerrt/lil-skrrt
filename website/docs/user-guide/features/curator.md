...
Tracks [issue #7816](https://github.com/SkrrtSkerrt/hermes-agent/issues/7816).

## How it runs

The curator is triggered by an inactivity check, not a cron daemon. On CLI session start, and on a recurring tick inside the gateway's cron-ticker thread, Lil Skrrt checks whether:
...
On a brand-new install (or the first time a pre-curator install ticks after `lil-skrrt update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `lil-skrrt curator run --dry-run` — it produces the same review report without mutating the library.
:::
...
**Easiest — `lil-skrrt model`:**

```bash
lil-skrrt model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`lil-skrrt model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
lil-skrrt curator status         # last run, counts, pinned list, LRU top 5
lil-skrrt curator run            # trigger a review now (blocks until the LLM pass finishes)
lil-skrrt curator run --background  # fire-and-forget: start the LLM pass in a background thread
lil-skrrt curator run --dry-run  # preview only — report without any mutations
lil-skrrt curator backup         # take a manual snapshot of ~/.hermes/skills/
lil-skrrt curator rollback       # restore from the newest snapshot
lil-skrrt curator rollback --list     # list available snapshots
lil-skrrt curator rollback --id <ts>  # restore a specific snapshot
lil-skrrt curator rollback -y         # skip the confirmation prompt
lil-skrrt curator pause          # stop runs until resumed
lil-skrrt curator resume
lil-skrrt curator pin <skill>    # never auto-transition this skill
lil-skrrt curator unpin <skill>
lil-skrrt curator restore <skill>  # move an archived skill back to active
```

## Backups and rollback

Before every real curator pass, Lil Skrrt takes a tar.gz snapshot of `~/.hermes/skills/` at `~/.hermes/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
lil-skrrt curator rollback        # restore newest snapshot (with confirmation)
lil-skrrt curator rollback -y     # skip the prompt
lil-skrrt curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, Lil Skrrt takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `lil-skrrt curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `lil-skrrt curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`lil-skrrt curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
- `~/.hermes/skills/.hub/lock.json` (skills installed via `lil-skrrt skills install`).
...
- Skills added via external skill directories you've pointed Lil Skrrt at.
...
1. Run `lil-skrrt curator run --dry-run` to see exactly what the curator would propose.
2. Use `lil-skrrt curator pin <name>` to fence off anything you don't want touched.
3. Or set `curator.enabled: false` in `config.yaml` if you'd rather manage the library yourself.

Archives are always recoverable via `lil-skrrt curator restore <name>`, but it's easier to pin up-front than to chase down a consolidation after the fact.
:::

If you want to protect a specific skill from ever being touched — for example a hand-authored skill you rely on — use `lil-skrrt curator pin <name>`. See the next section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `lil-skrrt curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
lil-skrrt curator pin <skill>
lil-skrrt curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.hermes/skills/.usage.json`, so it survives across sessions.

Only **agent-created** skills can be pinned — bundled and hub-installed skills are never subject to curator mutation in the first place, and `lil-skrrt curator pin` will refuse with an explanatory message if you try.
...
```bash
lil-skrrt curator restore <skill-name>
```
...
- **For just one run:** `lil-skrrt curator pause` — the pause persists across sessions; use `resume` to re-enable.
...
- [Issue #7816](https://github.com/SkrrtSkerrt/hermes-agent/issues/7816) — original proposal and design discussion

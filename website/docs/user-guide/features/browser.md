...
Lil Skrrt includes a full browser automation toolset with multiple backend options:
...
When a cloud provider is configured, Lil Skrrt auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`lil-skrrt setup tools → Browser Automation`
...
Or configure via `lil-skrrt tools` → Browser Automation → Camofox.
...
Then fully restart Lil Skrrt so the new config is picked up.
...
If the flag is placed at the wrong path, Lil Skrrt silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active Lil Skrrt profile, so different Lil Skrrt profiles get different browser profiles (profile isolation).

##### What Lil Skrrt does not do
- It does not force persistence on the Camofox server. Lil Skrrt only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), Lil Skrrt cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start Lil Skrrt and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Lil Skrrt after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

Lil Skrrt derives the stable `userId` from the profile-scoped directory `~/.hermes/browser_auth/camofox/` (or the equivalent under `$HERMES_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Lil Skrrt profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure Lil Skrrt to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` Lil Skrrt uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, Lil Skrrt calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- Lil Skrrt skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- Lil Skrrt does **not** call `DELETE /sessions/<user_id>` — that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, Lil Skrrt issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, Lil Skrrt adopts the most recently created one in that group.
3. Otherwise, Lil Skrrt adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, Lil Skrrt falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error — Lil Skrrt does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want Lil Skrrt to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, Lil Skrrt generates a per-task `session_key` (`task_<id>`) — Lil Skrrt will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and Lil Skrrt can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while Lil Skrrt runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Lil Skrrt automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.

### Local Chromium-family browser via CDP (`/browser connect`)

Instead of a cloud provider, you can attach Lil Skrrt browser tools to your own running Chrome, Brave, Chromium, or Edge instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start Lil Skrrt from the terminal (`lil-skrrt` or `lil-skrrt chat`) and issue `/browser connect` there.
:::
...
Then launch the Lil Skrrt CLI and run `/browser connect`.
...
- `/browser connect` expects Lil Skrrt itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let Lil Skrrt talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through Lil Skrrt MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Lil Skrrt can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `lil-skrrt config set toolsets '["hermes-cli", "browser"]'`.
:::
...
```
Navigate to https://github.com/SkrrtSkerrt
```
...
:::note
If paid features aren't available on your plan, Lil Skrrt automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::

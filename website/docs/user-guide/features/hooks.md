...
Lil Skrrt has three hook systems that run custom code at key lifecycle points:
...
This tutorial shows how to build it yourself as a user-defined hook. Lil Skrrt does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.
...
```bash
lil-skrrt logs --follow --level INFO | grep boot-md
```
...
An earlier version of Lil Skrrt shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Lil Skrrt's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
Declare shell-script hooks in your `cli-config.yaml` and Lil Skrrt will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Each time the event fires, Lil Skrrt spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Lil Skrrt event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
Each unique `(event, command)` pair prompts the user for approval the first time Lil Skrrt sees it, then persists the decision to `~/.hermes/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `lil-skrrt --accept-hooks chat`)
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `lil-skrrt hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

### The `lil-skrrt hooks` CLI

| Command | What it does |
|---------|--------------|
| `lil-skrrt hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `lil-skrrt hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `lil-skrrt hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `lil-skrrt hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Re-run `lil-skrrt hooks doctor` after you pull a shared config to spot newly-added hooks before they register.

...
description: "Overview of Lil Skrrt's tools — what's available, how toolsets work, and terminal backends"
---
...
Lil Skrrt ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
| **X Search** | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool — gated on xAI credentials (SuperGrok OAuth or `XAI_API_KEY`); off by default, opt in via `lil-skrrt tools` → 🐦 X (Twitter) Search. |
| **Terminal & Files** | `terminal`, `process`, `read_file`, `patch` | Execute commands and manipulate files. |
| **Browser** | `browser_navigate`, `browser_snapshot`, `browser_vision` | Interactive browser automation with text and vision support. |
| **Media** | `vision_analyze`, `image_generate`, `video_generate`, `video_analyze`, `text_to_speech` | Multimodal analysis and generation. `video_generate` and `video_analyze` are opt-in (add `video_gen` / `video` toolsets via `lil-skrrt tools` or `--toolsets`). |
...
lil-skrrt chat --toolsets "web,terminal"
...
**One persistent container, shared across the whole process.** Lil Skrrt starts a single long-lived container on first use (`docker run -d ... sleep 2h`) and routes every terminal, file, and `execute_code` call through `docker exec` into that same container. Working-directory changes, installed packages, environment tweaks, and files written to `/workspace` all carry over from one tool call to the next, across `/new`, `/reset`, and `delegate_task` subagents, for the lifetime of the Lil Skrrt process. The container is stopped and removed on shutdown.
...
# Configure
lil-skrrt config set terminal.backend singularity
lil-skrrt config set terminal.singularity_image ~/python.sif
```
...
modal setup
lil-skrrt config set terminal.backend modal
```

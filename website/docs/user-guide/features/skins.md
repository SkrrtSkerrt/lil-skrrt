...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Lil Skrrt CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Lil Skrrt CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
|------|-------------|----------------|------------------|
| `default` | Lil Skrrt Neon — cool blue and steel | `Lil Skrrt Agent` | Cool blue borders, slate text, neon accents, and the familiar caduceus banner. Clean and machine-like. |
| `ares` | War-god theme — crimson and cyan | `Ares Agent` | Deep crimson borders with cyan accents. Aggressive spinner verbs ("forging", "marching", "tempering steel"). Custom sword-and-shield ASCII art banner. |
| `mono` | Monochrome — clean grayscale | `Lil Skrrt` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Lil Skrrt` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Lil Skrrt` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `cool-lightmode` | Cool slate text for light terminal backgrounds | `Lil Skrrt Agent` | Cool slate text for light terminals. Blue-gray text with cyan accents and pale steel surfaces. A cooler alternative to the daylight theme. |
...
| `charizard` | Volcanic theme — blue flame and steel | `Charizard Agent` | Cool blue to steel gradient. Fire-themed spinners ("banking into the draft", "measuring signal"). Dragon-silhouette ASCII art banner. |
...
|-----|-------------|--------------------------|
| `banner_border` | Panel border around the startup banner | `#38BDF8` (sky blue) |
| `banner_title` | Title text color in the banner | `#38BDF8` (sky blue) |
| `banner_accent` | Section headers in the banner (Available Tools, etc.) | `#22D3EE` (cyan) |
| `banner_dim` | Muted text in the banner (separators, secondary labels) | `#64748B` (slate) |
| `banner_text` | Body text in the banner (tool names, skill names) | `#E2E8F0` (slate-200) |
| `ui_accent` | General UI accent color (highlights, active elements) | `#22D3EE` |
| `ui_label` | UI labels and tags | `#60A5FA` (blue) |
| `ui_ok` | Success indicators (checkmarks, completion) | `#4caf50` (green) |
| `ui_error` | Error indicators (failures, blocked) | `#ef5350` (red) |
| `ui_warn` | Warning indicators (caution, approval prompts) | `#FACC15` (yellow) |
| `prompt` | Interactive prompt text color | `#E2E8F0` |
| `input_rule` | Horizontal rule above the input area | `#334155` |
| `response_border` | Border around the agent's response box (ANSI escape) | `#38BDF8` |
| `session_label` | Session label color | `#60A5FA` |
| `session_border` | Session ID dim border color | `#64748B` |
| `status_bar_bg` | Background color for the TUI status / usage bar | `#0F172A` |
| `voice_status_bg` | Background color for the voice-mode status badge | `#0F172A` |
| `selection_bg` | Background color for the TUI mouse-selection highlighter. Falls back to `completion_menu_current_bg` when unset. | `#1E293B` |
| `completion_menu_bg` | Background color for the completion menu list | `#0F172A` |
...
## Lil Skrrt Mod — Visual Skin Editor

[Lil Skrrt Mod](https://github.com/cocktailpeanut/hermes-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.
...
- Opens any skin into a visual editor with all Lil Skrrt skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
Lil Skrrt Mod respects the `HERMES_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

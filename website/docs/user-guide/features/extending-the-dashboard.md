...
A reference plugin (`example-dashboard` in [`hermes-example-plugins`](https://github.com/SkrrtSkerrt/hermes-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
…or restart `lil-skrrt dashboard`.
...
The [`strike-freedom-cockpit`](https://github.com/SkrrtSkerrt/hermes-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `hermes-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/SkrrtSkerrt/hermes-example-plugins.git
...
2. Restart `lil-skrrt dashboard` — plugin API routes are mounted once at startup, **not** on rescan.

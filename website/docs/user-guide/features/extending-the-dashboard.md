...
description: "Build themes and plugins for the Lil Skrrt web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Lil Skrrt web dashboard (`lil-skrrt dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:
...
|-------|---------|------------|--------|
| **Lil Skrrt Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Lil Skrrt Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Lil Skrrt Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Lil Skrrt plugins in `~/.hermes/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.
...
// Lil Skrrt API client + raw fetcher
...
#### Calling built-in Lil Skrrt endpoints
...
| `header-left` | Before the Lil Skrrt brand in the top bar. |
...
A reference plugin (`example-dashboard` in [`lil-skrrt-example-plugins`](https://github.com/NousResearch/lil-skrrt-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
The [`strike-freedom-cockpit`](https://github.com/NousResearch/lil-skrrt-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `lil-skrrt-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/NousResearch/lil-skrrt-example-plugins.git

# Theme
cp lil-skrrt-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.hermes/dashboard-themes/

# Plugin
cp -r lil-skrrt-example-plugins/strike-freedom-cockpit ~/.hermes/plugins/
```

Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Lil Skrrt Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).

...
Schedule tasks to run automatically with natural language or cron expressions. Lil Skrrt exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Lil Skrrt itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::warning
Cron-run sessions cannot recursively create more cron jobs. Lil Skrrt disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
lil-skrrt cron create "every 2h" "Check server status"
lil-skrrt cron create "every 1h" "Summarize new feed items" --skill blogwatcher
lil-skrrt cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask Lil Skrrt normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Lil Skrrt will use the unified `cronjob` tool internally.
...
lil-skrrt cron create "every 1d at 09:00" \
...
By default a cron job inherits whichever Lil Skrrt profile owned the gateway / CLI that created it. Pass `--profile <name>` (CLI) or `profile=` (cronjob tool) to re-target the job at a different profile — the scheduler resolves that profile's `HERMES_HOME`, temporarily switches into it for the duration of the run, loads its `.env` + `config.yaml`, and executes the job there:

```bash
# Pin a job to the `night-ops` profile regardless of where it was scheduled
lil-skrrt cron create "every 1d at 03:00" \
...
```bash
lil-skrrt cron edit <job_id> --schedule "every 4h"
lil-skrrt cron edit <job_id> --prompt "Use the revised task"
lil-skrrt cron edit <job_id> --skill blogwatcher --skill maps
lil-skrrt cron edit <job_id> --add-skill maps
lil-skrrt cron edit <job_id> --remove-skill blogwatcher
lil-skrrt cron edit <job_id> --clear-skills
```
...
```bash
lil-skrrt cron list
lil-skrrt cron pause <job_id>
lil-skrrt cron resume <job_id>
lil-skrrt cron run <job_id>
lil-skrrt cron remove <job_id>
lil-skrrt cron status
lil-skrrt cron tick
```
...
```bash
lil-skrrt gateway install     # Install as a user service
sudo hermes gateway install --system   # Linux: boot-time system service for servers
lil-skrrt gateway             # Or run in foreground
...
```bash
lil-skrrt cron create "every 5m" \
  --no-agent \
...
The `cronjob` tool's schema exposes `no_agent` to Lil Skrrt directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Lil Skrrt will write the check script to `~/.hermes/scripts/` via `write_file`, then call:
...
- When Job 2 fires, Lil Skrrt reads Job 1's most recent output from `~/.hermes/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Lil Skrrt skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `lil-skrrt tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `lil-skrrt tools` cron-platform config wins; otherwise Lil Skrrt falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Lil Skrrt should even invoke the agent. Emit a final stdout line of the form:
...
:::tip
Lil Skrrt's own `~/.hermes/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/SkrrtSkerrt/hermes-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs may store `model` and `provider` as `null`. When those fields are omitted, Lil Skrrt resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.

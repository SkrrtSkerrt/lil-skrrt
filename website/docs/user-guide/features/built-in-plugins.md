...
description: "Plugins shipped with Lil Skrrt that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Lil Skrrt ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.hermes/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Lil Skrrt Plugin](/guides/build-a-hermes-plugin) to write your own.
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `lil-skrrt memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `lil-skrrt plugins list` and the interactive `lil-skrrt plugins` UI), but none load until you explicitly enable them:

```bash
lil-skrrt plugins enable disk-cleanup
```
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Lil Skrrt. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
lil-skrrt plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `lil-skrrt plugins enable <name>`.
...
| `hermes-achievements` | dashboard tab | Steam-style collectible badges generated from your real Lil Skrrt session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `lil-skrrt memory` and `lil-skrrt plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
**Enabling:** `lil-skrrt plugins enable disk-cleanup` (or check the box in `lil-skrrt plugins`).

**Disabling again:** `lil-skrrt plugins disable disk-cleanup`.

### observability/langfuse

Traces Lil Skrrt turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Lil Skrrt' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `lil-skrrt logs`.
...
lil-skrrt plugins enable observability/langfuse
```

Or check the box in the interactive `lil-skrrt plugins` UI. Then put the credentials in `~/.hermes/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Lil Skrrt turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
Session grouping keys off the Lil Skrrt session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `lil-skrrt chat` session lives under one Langfuse session.

**Verify:**

```bash
lil-skrrt plugins list                 # observability/langfuse should show "enabled"
lil-skrrt chat -q "hello"              # check the Langfuse UI for a "Hermes turn" trace
```
...
**Disabling:** `lil-skrrt plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
```bash
lil-skrrt plugins enable google_meet
...
**Disabling:** `lil-skrrt plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.hermes/cache/google_meet/` until you remove them.

### hermes-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Lil Skrrt session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Lil Skrrt feature changes.
...
| Secret | Hidden until Lil Skrrt detects the first related signal in your history |
...
| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Lil Skrrt updates. |
...
**Enabling:** Nothing to enable — `hermes-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `lil-skrrt dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.
...
Bundled plugins are written exactly like any other Lil Skrrt plugin — see [Build a Lil Skrrt Plugin](/guides/build-a-hermes-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.hermes/plugins/<name>/`
- Manifest source is reported as `bundled` in `lil-skrrt plugins list`

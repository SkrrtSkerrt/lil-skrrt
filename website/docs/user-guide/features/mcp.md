...
description: "Connect Lil Skrrt to external tool servers via MCP — and control exactly which MCP tools Lil Skrrt loads"
---

# MCP (Model Context Protocol)

MCP lets Lil Skrrt connect to external tool servers so the agent can use tools that live outside Lil Skrrt itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Lil Skrrt to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Lil Skrrt tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Lil Skrrt to see
...
4. Ask Lil Skrrt to use the MCP-backed capability.
...
Lil Skrrt will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

Lil Skrrt ships a curated catalog of MCP servers that Nous staff has reviewed
and merged. They're disabled by default — install only what you actually
want.

```bash
lil-skrrt mcp                # interactive picker (default)
lil-skrrt mcp catalog        # plain-text list, scriptable
lil-skrrt mcp install n8n    # install a catalog entry by name
```
...
- **API key** — Lil Skrrt prompts at install time and writes the value to
...
- **OAuth** (third-party provider like Google/GitHub) — Lil Skrrt points you at
  `lil-skrrt auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, Lil Skrrt probes the MCP server to list every
...
written (if not). Re-run `lil-skrrt mcp configure <name>` once the server is
reachable to refine.
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/SkrrtSkerrt/hermes-agent/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed Lil Skrrt
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Lil Skrrt`) for that entry instead of silently hiding it. Run `lil-skrrt update`
to install the latest Lil Skrrt when you see that.
...
```bash
lil-skrrt mcp configure linear
```
...
MCPs are never auto-updated. Re-run `lil-skrrt mcp install <name>` to refresh
after a Lil Skrrt update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/SkrrtSkerrt/hermes-agent/tree/main/optional-mcps).
...
HTTP MCP servers are remote endpoints Lil Skrrt connects to directly.
...
- you do not want Lil Skrrt spawning a local subprocess for that integration

### OAuth-authenticated HTTP servers

Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Lil Skrrt handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.
...
On first connect, Lil Skrrt prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.hermes/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When Lil Skrrt runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:

- **Paste-back (no setup):** on an interactive terminal Lil Skrrt prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.

See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `lil-skrrt mcp login <server>`.

**Pitfall — config auto-reload race.** When you edit `~/.hermes/config.yaml` from inside a running Lil Skrrt session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `lil-skrrt mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## Basic configuration reference

Lil Skrrt reads MCP config from `~/.hermes/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Lil Skrrt skips the server entirely |
...
For well-known MCP servers, `lil-skrrt mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
lil-skrrt mcp add codex --preset codex
```
...
You can pick any local name (`lil-skrrt mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Lil Skrrt registers MCP tools

Lil Skrrt prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Lil Skrrt sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Lil Skrrt also registers utility tools around MCP resources and prompts:
...
- Lil Skrrt only registers resource utilities if the MCP session actually supports resource operations
- Lil Skrrt only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Lil Skrrt, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Lil Skrrt skips the server completely and does not even attempt a connection.
...
If your config filters out all callable tools and disables or omits all supported utilities, Lil Skrrt does not create an empty runtime MCP toolset for that server.
...
Lil Skrrt discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Lil Skrrt when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Lil Skrrt receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Lil Skrrt does not blindly pass your full shell environment.
...
Then verify your config and restart Lil Skrrt.
...
Because Lil Skrrt now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Lil Skrrt may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Lil Skrrt via the `sampling/createMessage` protocol. This allows an MCP server to ask Lil Skrrt to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Lil Skrrt
- You want a single MCP server that bridges to all of Lil Skrrt's connected messaging platforms at once
- You already have a running Lil Skrrt gateway with connected platforms
...
Add Lil Skrrt to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Lil Skrrt in a specific location:
...
The MCP server includes a live event bridge that polls Lil Skrrt's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
lil-skrrt mcp serve              # Normal mode
lil-skrrt mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Lil Skrrt's session store (`~/.hermes/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Lil Skrrt agent itself.
...
- The embedded `lil-skrrt mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Lil Skrrt, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).

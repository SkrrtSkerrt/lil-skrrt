...
title: "Plugins"
description: "Extend Lil Skrrt with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Lil Skrrt has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Lil Skrrt
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Lil Skrrt Plugin](/guides/build-a-hermes-plugin)** — step-by-step guide with a complete working example.
...
Start Lil Skrrt — your tools appear alongside built-in tools. The model can call them immediately.
...
Drop both files into `~/.hermes/plugins/hello-world/`, restart Lil Skrrt, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.hermes/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting Lil Skrrt.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `lil-skrrt <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `lil-skrrt plugins install` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Lil Skrrt — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
...
Within each source, Lil Skrrt also recognizes sub-category directories that route plugins to specialized discovery systems:
...
Sub-category plugins surface in `lil-skrrt plugins list` and the interactive `lil-skrrt plugins` UI under their **path-derived key** — e.g. `observability/langfuse`, `image_gen/openai`, `platforms/teams`. That key (not the bare manifest `name:`) is the value you pass to `lil-skrrt plugins enable …` / `disable …` and the string to add under `plugins.enabled` in `config.yaml`.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `lil-skrrt plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.hermes/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
lil-skrrt plugins                    # interactive toggle (space to check/uncheck)
lil-skrrt plugins enable <name>      # add to allow-list
lil-skrrt plugins disable <name>     # remove from allow-list + add to disabled
```

After `lil-skrrt plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Lil Skrrt' built-in surface and would break basic functionality if gated off by default:
...
Lil Skrrt has four kinds of plugins:
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Lil Skrrt also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Lil Skrrt Plugin](/guides/build-a-hermes-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Lil Skrrt Plugin](/guides/build-a-hermes-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Lil Skrrt Plugin](/guides/build-a-hermes-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `lil-skrrt <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Lil Skrrt auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `lil-skrrt skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
On NixOS, plugins can be installed declaratively via the module options — no `lil-skrrt plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
lil-skrrt plugins                                       # unified interactive UI
lil-skrrt plugins list                                  # table: enabled / disabled / not enabled
lil-skrrt plugins install user/repo                     # install from Git, then prompt Enable? [y/N]
lil-skrrt plugins install user/repo --enable            # install AND enable (no prompt)
lil-skrrt plugins install user/repo --no-enable         # install but leave disabled (no prompt)
lil-skrrt plugins update my-plugin                      # pull latest
lil-skrrt plugins remove my-plugin                      # uninstall
lil-skrrt plugins enable my-plugin                      # add to allow-list (flat plugin)
lil-skrrt plugins enable observability/langfuse         # add to allow-list (sub-category plugin)
lil-skrrt plugins disable my-plugin                     # remove from allow-list + add to disabled
```

For plugins under a sub-category directory (e.g. `plugins/observability/langfuse/`, `plugins/image_gen/openai/`), use the full `<category>/<plugin>` key — that's exactly what `lil-skrrt plugins list` shows in the **Name** column.

### Interactive UI

Running `lil-skrrt plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `lil-skrrt plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.

...
Lil Skrrt can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `lil-skrrt computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `lil-skrrt tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
window-focus bug for UTM workflows). Lil Skrrt refreshes the binary in two
places so you don't get stuck on a stale release:

- **`lil-skrrt update`** — when you update Lil Skrrt itself, if `cua-driver` is
...
- **`lil-skrrt computer-use install --upgrade`** — manual force-refresh.
...
`lil-skrrt computer-use status` shows the installed version next to the
binary path.
...
Lil Skrrt applies multi-layer guardrails:
...
Screenshots are expensive. Lil Skrrt applies four layers of optimisation:
...
`lil-skrrt computer-use install` to fetch the cua-driver binary, or run
`lil-skrrt tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/SkrrtSkerrt/hermes-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)

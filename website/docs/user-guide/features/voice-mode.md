...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Lil Skrrt — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Lil Skrrt supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Lil Skrrt](/guides/use-voice-mode-with-hermes).

## Prerequisites

Before using voice features, make sure you have:

1. **Lil Skrrt installed** — `pip install hermes-agent` (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `lil-skrrt model` or set your preferred provider credentials in `~/.hermes/.env`
3. **A working base setup** — run `lil-skrrt` to verify the agent responds to text before enabling voice

:::tip
The `~/.hermes/` directory and default `config.yaml` are created automatically the first time you run `lil-skrrt`. You only need to create `~/.hermes/.env` manually for API keys.
:::

:::tip Portal covers both
A paid [Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `lil-skrrt setup --portal` wires both up at once.
:::
...
Voice mode is available in both the **classic CLI** (`lil-skrrt chat`) and the **TUI** (`lil-skrrt --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.hermes/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
a lil skrrt  # Start the interactive CLI
```
...
1. Start the CLI with `lil-skrrt` and enable voice mode with `/voice on`
...
```bash
lil-skrrt gateway        # Start the gateway (connects to configured platforms)
lil-skrrt gateway setup  # Interactive setup wizard for first-time configuration
```
...
```bash
lil-skrrt gateway        # Start with existing configuration
```
...
If you are running Lil Skrrt inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.

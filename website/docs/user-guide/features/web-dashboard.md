...
The web dashboard is a browser-based UI for managing your Lil Skrrt installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
| `--tui` | off | Expose the in-browser Chat tab (embedded `lil-skrrt --tui` via PTY/WebSocket). Alternatively set `HERMES_DASHBOARD_TUI=1`. |

```bash
# Custom port
lil-skrrt dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
lil-skrrt dashboard --host 0.0.0.0

# Start without opening browser
lil-skrrt dashboard --no-open
...
When you run `lil-skrrt dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is intentionally off for a plain `lil-skrrt dashboard` launch. Start the dashboard with `lil-skrrt dashboard --tui` or set `HERMES_DASHBOARD_TUI=1` when you want the embedded browser chat pane.
...
- Node.js (same requirement as `lil-skrrt --tui`; the TUI bundle is built on first launch)
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `lil-skrrt config set` and the gateway read from.
:::
...
When you run `lil-skrrt update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `lil-skrrt dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Lil Skrrt Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Lil Skrrt Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |

...
Lil Skrrt is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Lil Skrrt checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.
...
1. **CLI flag**: Start a session with `lil-skrrt --yolo` or `lil-skrrt chat --yolo`
...
When YOLO is active, Lil Skrrt shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Lil Skrrt refuses to run them **regardless** of:
...
| `pkill`/`killall` lil-skrrt/gateway | Self-termination prevention |
...
:::tip
Use `lil-skrrt config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Lil Skrrt controls who can interact with the bot through a layered authorization system.
...
For more flexible authorization, Lil Skrrt includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `lil-skrrt pairing approve <platform> <code>` on the CLI
...
lil-skrrt pairing list

# Approve a pairing code
lil-skrrt pairing approve telegram ABC12DEF

# Revoke a user's access
lil-skrrt pairing revoke telegram 123456789

# Clear all pending codes
lil-skrrt pairing clear-pending
```
...
When using the `docker` terminal backend, Lil Skrrt applies strict security hardening to every container.
...
When loaded, Lil Skrrt checks if these files exist in the active profile's `HERMES_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Lil Skrrt infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Lil Skrrt infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Lil Skrrt integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Lil Skrrt under WSL.
...
10. **Keep updated** — run `lil-skrrt update` regularly for security patches
...
Lil Skrrt ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `hermes_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `lil-skrrt doctor` for the full remediation.
- **`lil-skrrt doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
```bash
lil-skrrt doctor --ack <advisory-id>
```
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Lil Skrrt installs these **lazily** on first use rather than eagerly under `hermes-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `lil-skrrt tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `lil-skrrt tools`.

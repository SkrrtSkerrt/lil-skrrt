...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Lil Skrrt terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Lil Skrrt's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip
Lil Skrrt also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `lil-skrrt --tui` — see the [TUI](tui.md) guide.
:::
...
lil-skrrt chat --model "anthropic/claude-sonnet-4"

# With a specific provider
lil-skrrt chat --provider nous        # Use Nous Portal
lil-skrrt chat --provider openrouter  # Force OpenRouter

# With specific toolsets
lil-skrrt chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
lil-skrrt -s hermes-agent-dev,github-auth
lil-skrrt chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
lil-skrrt --continue             # Resume the most recent CLI session (-c)
lil-skrrt --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
lil-skrrt chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
lil-skrrt -w                         # Interactive mode in worktree
lil-skrrt -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Lil Skrrt CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Lil Skrrt CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
```
 ☠︎ claude-sonnet-4-20250514 │ 12.4K/200K │ [██████░░░░] 6% │ $0.06 │ 15m
```
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `lil-skrrt --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`lil-skrrt -c` or `lil-skrrt --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Lil Skrrt to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Lil Skrrt supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Lil Skrrt replies |
...
```bash
lil-skrrt -s hermes-agent-dev,github-auth
lil-skrrt chat -s github-pr-workflow -s github-auth
```

Lil Skrrt loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.
...
personalities:
  ...
  pirate: "Arrr! Ye be talkin' to Captain Lil Skrrt..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Lil Skrrt recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Lil Skrrt — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Lil Skrrt is working, Lil Skrrt prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Lil Skrrt to the background — just like any terminal process. The shell prints a confirmation:

```
Lil Skrrt has been suspended. Run `fg` to bring Lil Skrrt back.
```
...
Resume this session with:
  lil-skrrt --resume 20260225_143052_a1b2c3
...
```bash
lil-skrrt --continue                          # Resume the most recent CLI session
lil-skrrt -c                                  # Short form
lil-skrrt -c "my project"                     # Resume a named session (latest in lineage)
lil-skrrt --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
lil-skrrt --resume "refactoring auth"         # Resume by title
lil-skrrt -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `lil-skrrt sessions rename <id> <title>` from the command line. Use `lil-skrrt sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Lil Skrrt's SQLite state database under `~/.hermes/state.db`. The database keeps:
...
Lil Skrrt immediately confirms the task and gives you back the prompt:

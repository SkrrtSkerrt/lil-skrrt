...
title: "Git Worktrees"
description: "Run multiple Lil Skrrt agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Lil Skrrt is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Lil Skrrt so each session has a clean, isolated working directory.

## Why Use Worktrees with Lil Skrrt?

Lil Skrrt treats the **current working directory** as the project root:

- CLI: the directory where you run `lil-skrrt` or `lil-skrrt chat`
...
Now you can `cd` into the new worktree and run Lil Skrrt there:
...
- Lil Skrrt checkpoint data under `~/.hermes/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Lil Skrrt experiment**
...
- **Avoid running Lil Skrrt from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `lil-skrrt -w` (Automatic Worktree Mode)

Lil Skrrt has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `lil-skrrt/hermes-<hash>`).
...
```bash
lil-skrrt -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `lil-skrrt -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Lil Skrrt session its own clean checkout.

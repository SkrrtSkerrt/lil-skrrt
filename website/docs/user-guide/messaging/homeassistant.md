...
description: Control your smart home with Lil Skrrt via Home Assistant integration.
...
Lil Skrrt integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Lil Skrrt"
5. Copy the token
...
Lil Skrrt registers four tools for smart home control:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Lil Skrrt".

...
Connect Lil Skrrt to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Lil Skrrt and the BlueBubbles server
...
When someone messages your iMessage, Lil Skrrt automatically sends them a pairing code. Approve it with:
```bash
lil-skrrt pairing approve bluebubbles <CODE>
```
Use `lil-skrrt pairing list` to see pending codes and approved users.
...
Lil Skrrt will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.
...
- **Outbound:** Lil Skrrt sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Lil Skrrt resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `lil-skrrt logs gateway` for webhook errors (or `lil-skrrt logs -f` to follow in real-time)

...
sidebar_position: 4
title: "Slack"
description: "Set up Lil Skrrt as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Lil Skrrt to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Lil Skrrt instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Lil Skrrt uses the modern
...
The fastest path is to paste a manifest Lil Skrrt generates for you. It
...
4. Enter an app name (e.g., "Lil Skrrt") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Lil Skrrt can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Lil Skrrt configuration issue.
:::
...
Lil Skrrt uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
lil-skrrt gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
lil-skrrt gateway              # Foreground
...
Every Lil Skrrt command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Lil Skrrt command with its description.

Under the hood: Lil Skrrt ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/SkrrtSkerrt/hermes-agent/blob/main/hermes_cli/commands.py)
...
When Lil Skrrt adds new commands (e.g. after `lil-skrrt update`), regenerate
...
`/hermes btw run the tests` — Lil Skrrt routes it the same way as `/btw
...
Slack never delivers them to Lil Skrrt.

As a workaround, Lil Skrrt recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Lil Skrrt treats it identically to the slash form and replies in the same
thread.
...
```bash
lil-skrrt slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Lil Skrrt behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Lil Skrrt what time is it?`). In channels, Lil Skrrt replies in a thread attached to that message. |
| **Threads** | If you @mention Lil Skrrt inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Lil Skrrt in `#general` will have separate histories and contexts.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Lil Skrrt will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Lil Skrrt`).

---

## Multi-Workspace Support

Lil Skrrt can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Lil Skrrt also loads tokens from an **OAuth token file** at:
...
- When a message arrives, Lil Skrrt uses the correct workspace-specific client to respond.
...
Lil Skrrt supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Lil Skrrt` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Lil Skrrt` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Lil Skrrt now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Lil Skrrt`)
...
- Audit who has access to your Lil Skrrt config directory

...
Connect Lil Skrrt to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Lil Skrrt supports two WeCom integration modes:
...
3. Lil Skrrt decrypts the message, queues it for the agent
...
(Use `lil-skrrt gateway start` only after `lil-skrrt gateway install` has registered the systemd/launchd service.)

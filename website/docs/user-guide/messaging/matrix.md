...
sidebar_position: 9
title: "Matrix"
description: "Set up Lil Skrrt as a Matrix bot"
---

# Matrix Setup

Lil Skrrt integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Lil Skrrt pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Lil Skrrt works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Lil Skrrt behaves once it's connected.

## How Lil Skrrt Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Lil Skrrt responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Lil Skrrt requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Lil Skrrt supports Matrix threads (MSC3440). If you reply in a thread, Lil Skrrt keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Lil Skrrt auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |
| **Shared rooms with multiple users** | By default, Lil Skrrt isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
2. Choose a username like `lil-skrrt` — the full user ID will be `@lil-skrrt:your-server.org`.
...
You can also run Lil Skrrt as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Lil Skrrt needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Lil Skrrt the bot's user ID and password. Lil Skrrt will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.
...
Lil Skrrt uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Lil Skrrt supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
When E2EE is enabled, Lil Skrrt:
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Lil Skrrt imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.
...
Lil Skrrt detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
2. Delete the local crypto store and restart Lil Skrrt:
   ```bash
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/SkrrtSkerrt/hermes-agent/issues/12614).
...
If you previously used Lil Skrrt with `MATRIX_ENCRYPTION=true` and are upgrading to
...
a working E2EE setup with a previous version of Lil Skrrt and are upgrading.
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Lil Skrrt API server (e.g., `http://192.168.1.100:8642`) |
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Lil Skrrt logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Lil Skrrt gateway isn't running, or it failed to connect.

**Fix**: Check that `lil-skrrt gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.
...
- **Media support**: Lil Skrrt can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.

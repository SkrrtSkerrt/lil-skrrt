...
sidebar_position: 12
title: "Google Chat"
description: "Set up Lil Skrrt as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Lil Skrrt to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Lil Skrrt
...
download the file. Save it somewhere only Lil Skrrt can read (e.g.,
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a lil-skrrt restart)
...
Also grant `Pub/Sub Viewer` on the same subscription — Lil Skrrt calls
...
- **App name**: whatever you want users to see ("Lil Skrrt" is reasonable).
...
`ADDED_TO_SPACE` event that Lil Skrrt uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Lil Skrrt
...
Install the dependencies the Google Chat adapter needs (no Lil Skrrt extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Lil Skrrt is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Lil Skrrt detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Lil Skrrt session.
...
3. Download the JSON. Move it onto the host that runs Lil Skrrt.
4. On the host, register the client with Lil Skrrt:
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Lil Skrrt isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `lil-skrrt gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Lil Skrrt will only attach the SA bearer

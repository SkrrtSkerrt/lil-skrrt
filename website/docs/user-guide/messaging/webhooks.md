...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Lil Skrrt — Webhooks Tutorial"
...
1. Enable via `lil-skrrt gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `lil-skrrt webhook subscribe`
...
Open a pull request on the repository. The webhook fires, Lil Skrrt processes the event, and posts a review comment on the PR.
...
```bash
lil-skrrt webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `lil-skrrt webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.

### Create a subscription

```bash
lil-skrrt webhook subscribe github-issues \
  --events "issues" \
...
```bash
lil-skrrt webhook list
```

### Remove a subscription

```bash
lil-skrrt webhook remove github-issues
```

### Test a subscription

```bash
lil-skrrt webhook test github-issues
lil-skrrt webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `lil-skrrt webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `lil-skrrt gateway run`

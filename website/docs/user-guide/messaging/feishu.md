...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Lil Skrrt as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Lil Skrrt integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Lil Skrrt opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Lil Skrrt responds to every message. |
| Group chats | Lil Skrrt responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Lil Skrrt will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `lil-skrrt gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when Lil Skrrt runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Lil Skrrt behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Lil Skrrt starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Lil Skrrt responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Lil Skrrt responds to @mentions from any user in any group. |
| `allowlist` | Lil Skrrt only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Lil Skrrt ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let Lil Skrrt read all group traffic without requiring an @mention:
...
Lil Skrrt auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default Lil Skrrt ignores messages sent by other bots. Enable bot-to-bot messaging when you want Lil Skrrt to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions Lil Skrrt. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `lil-skrrt update --gateway` needs confirmation, the adapter records the selected answer in Lil Skrrt's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, Lil Skrrt reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `lil-skrrt gateway setup` |
| `Another local Lil Skrrt gateway is already using this Feishu app_id` | Only one Lil Skrrt instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | Lil Skrrt can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |

...
A **profile distribution** packages a complete Lil Skrrt agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a Lil Skrrt agent meant sending someone:
...
```bash
lil-skrrt profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `lil-skrrt profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Lil Skrrt in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`lil-skrrt profile export` / `import`](../reference/profile-commands.md#hermes-profile-export) — that's what those are for.
...
```bash
lil-skrrt profile create research-bot
...
Recipients who run `lil-skrrt profile update research-bot` will pull the latest.
...
```bash
lil-skrrt profile install github.com/you/research-bot --alias
```
...
```bash
# GitHub shorthand
lil-skrrt profile install github.com/you/research-bot

# Full HTTPS
lil-skrrt profile install https://github.com/you/research-bot.git

# SSH
lil-skrrt profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
lil-skrrt profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
lil-skrrt profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
lil-skrrt profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
lil-skrrt profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
lil-skrrt profile install github.com/acme/support-bot --name support-eu --alias
```
...
```bash
lil-skrrt profile info research-bot
```
...
`lil-skrrt profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
lil-skrrt profile update research-bot
```
...
```bash
lil-skrrt profile delete research-bot
```
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `lil-skrrt profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
lil-skrrt profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `lil-skrrt profile update pr-reviewer` and everyone's on the new version within minutes.
...
# Anyone
lil-skrrt profile install github.com/you/hermes-polymarket-trader --alias
```
...
# Each on-call
lil-skrrt profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
lil-skrrt profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `lil-skrrt profile info <name>` and hold off on updates until you're ready.
:::
...
lil-skrrt profile info research-bot | grep Version
...
lil-skrrt profile delete research-bot --yes
lil-skrrt profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
lil-skrrt profile update research-bot --force-config --yes
```
...
lil-skrrt profile install github.com/yourname/forked-research-bot --alias
...
lil-skrrt profile install ~/.hermes/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
lil-skrrt profile delete research-bot-test --yes
lil-skrrt profile install ~/.hermes/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `lil-skrrt -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `lil-skrrt profile` — not a parallel command tree.
...
- Reserved profile names (`lil-skrrt`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`lil-skrrt profile export` / `import`](../reference/profile-commands.md#hermes-profile-export) — local backup / restore (not distribution)
- [Using SOUL with Lil Skrrt](../guides/use-soul-with-hermes.md) — authoring personalities

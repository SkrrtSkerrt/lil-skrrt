...
Lil Skrrt can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.
...
| Command | Description |
|---------|-------------|
| `lil-skrrt checkpoints` | Show total size, project count, per-project breakdown |
| `lil-skrrt checkpoints status` | Same as bare `checkpoints` |
| `lil-skrrt checkpoints list` | Alias for `status` |
| `lil-skrrt checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `lil-skrrt checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `lil-skrrt checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- Lil Skrrt detects when tools are about to **modify files** in your working tree.
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `lil-skrrt checkpoints prune` manually.
...
Lil Skrrt responds with a formatted list showing change statistics:
...
- **Directory scope** — Lil Skrrt skips overly broad directories (root `/`, home `$HOME`).
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `lil-skrrt checkpoints status` / `prune` / `clear` instead.
...
- **Enable checkpoints only when you need them** — `lil-skrrt chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `lil-skrrt checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each Lil Skrrt session in its own worktree/branch, with checkpoints as an extra layer.

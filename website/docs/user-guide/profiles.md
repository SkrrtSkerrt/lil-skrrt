...
Run multiple independent Lil Skrrt agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Lil Skrrt home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Lil Skrrt state.
...
```bash
lil-skrrt profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Lil Skrrt profile with its own config, memory, and state.
...
```bash
lil-skrrt profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `lil-skrrt profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
```bash
lil-skrrt profile create work --clone --clone-from coder
```
...
The alias works with every lil-skrrt subcommand — it's just `lil-skrrt -p <name>` under the hood.
...
```bash
lil-skrrt -p coder chat
lil-skrrt --profile=coder doctor
lil-skrrt chat -p coder -q "hello"    # works in any position
```

### Sticky default (`lil-skrrt profile use`)

```bash
lil-skrrt profile use coder
lil-skrrt chat                   # now targets coder
lil-skrrt tools                  # configures coder's tools
lil-skrrt profile use default    # switch back
```

Sets a default so plain `lil-skrrt` commands target that profile. Like `kubectl config use-context`.
...
- **`lil-skrrt profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives Lil Skrrt its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Lil Skrrt was launched from", not "the profile directory".
...
`lil-skrrt update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
lil-skrrt profile list           # show all profiles with status
lil-skrrt profile show coder     # detailed info for one profile
lil-skrrt profile rename coder dev-bot   # rename (updates alias + service)
lil-skrrt profile export coder   # export to coder.tar.gz
lil-skrrt profile import coder.tar.gz   # import from archive
```

## Deleting a profile

```bash
lil-skrrt profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `lil-skrrt profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.hermes`). To remove everything, use `lil-skrrt uninstall`.
:::
...
Profiles use the `HERMES_HOME` environment variable. When you run `coder chat`, the wrapper script sets `HERMES_HOME=~/.hermes/profiles/coder` before launching lil-skrrt. Since 119+ files in the codebase resolve paths via `get_hermes_home()`, Lil Skrrt state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.

...
description: "How to build a memory provider plugin for Lil Skrrt"
---

# Building a Memory Provider Plugin

Memory provider plugins give Lil Skrrt persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `lil-skrrt plugins`.
:::
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `lil-skrrt memory setup` | **Yes** |
...
`get_config_schema()` returns a list of field descriptors used by `lil-skrrt memory setup`:
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `lil-skrrt my-provider status`, `lil-skrrt my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `lil-skrrt <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `lil-skrrt --help`.

...
description: "How Lil Skrrt stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `hermes_cli/cron.py` | CLI `lil-skrrt cron` subcommands |
...
In CLI mode, cron jobs only fire when `lil-skrrt cron` commands are run or during active CLI sessions.
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `lil-skrrt cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `lil-skrrt cron` CLI provides direct job management:

```bash
lil-skrrt cron list                    # Show all jobs
lil-skrrt cron create                  # Interactive job creation (alias: add)
lil-skrrt cron edit <job_id>           # Edit job configuration
lil-skrrt cron pause <job_id>          # Pause a running job
lil-skrrt cron resume <job_id>         # Resume a paused job
lil-skrrt cron run <job_id>            # Trigger immediate execution
lil-skrrt cron remove <job_id>         # Delete a job
```

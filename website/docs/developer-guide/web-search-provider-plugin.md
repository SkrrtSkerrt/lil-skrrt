...
description: "How to build a web-search/extract/crawl backend plugin for Lil Skrrt"
---
...
:::tip
Web search is one of several **backend plugins** Lil Skrrt supports. The others (with their own ABCs) are [Image Generation Provider Plugins](/developer-guide/image-gen-provider-plugin), [Video Generation Provider Plugins](/developer-guide/video-gen-provider-plugin), [Memory Provider Plugins](/developer-guide/memory-provider-plugin), [Context Engine Plugins](/developer-guide/context-engine-plugin), and [Model Provider Plugins](/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build a Lil Skrrt Plugin](/guides/build-a-hermes-plugin).
:::

## How discovery works

Lil Skrrt scans for web-search backends in three places:

1. **Bundled** — `<repo>/plugins/web/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.hermes/plugins/web/<name>/` (opt-in via `plugins.enabled` or `lil-skrrt plugins enable <name>`)
...
When neither key is set, Lil Skrrt auto-detects the backend from whichever API key/URL is present in the environment. `lil-skrrt tools` walks users through selection.
...
| `provides_web_providers` | List of provider `name`s this plugin registers — used by the loader to advertise the plugin in `lil-skrrt tools` even before `register()` runs |
| `requires_env` | Interactive credential prompt during `lil-skrrt plugins install` (see [Build a Lil Skrrt Plugin](/guides/build-a-hermes-plugin#gate-on-environment-variables) for the rich format) |
...
| `display_name` | — | `name` | Label shown in `lil-skrrt tools` |
...
Lil Skrrt routes calls to the right provider based on the `supports_*` flags. A common multi-provider setup:
...
When `web.search_backend` or `web.extract_backend` aren't set, both fall through to `web.backend`. When that's also unset, Lil Skrrt picks the first available provider that supports the requested capability based on env-var presence.
...
## How Lil Skrrt wires it into the tools
...
Errors surface as the tool result; the LLM decides how to explain them. If no provider is registered (or every available one fails the capability gate), the tool returns a helpful error pointing at `lil-skrrt tools`.

## Lazy-installing optional dependencies

If your provider wraps a third-party SDK (like DDGS does with the `ddgs` package), don't `import` it at module top level. Use `tools.lazy_deps.ensure(...)` inside `is_available()` or `search()` — Lil Skrrt will install the package on first use, gated by `security.allow_lazy_installs`. See [Build a Lil Skrrt Plugin → Lazy-install](/guides/build-a-hermes-plugin#lazy-install-optional-python-dependencies) for the security model.
...
- [Build a Lil Skrrt Plugin](/guides/build-a-hermes-plugin) — general tools/hooks/slash commands guide

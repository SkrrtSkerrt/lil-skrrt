...
This guide covers adding a new messaging platform to the Lil Skrrt gateway. A platform adapter connects Lil Skrrt to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
The plugin system lets you add a platform adapter without modifying any core Lil Skrrt code. Your plugin is a directory with two files:
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `lil-skrrt config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hermes-config) below).
...
| `lil-skrrt config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `lil-skrrt status` | Shows plugin platforms with `(plugin)` tag |
| `lil-skrrt gateway setup` | Plugin platforms appear in setup menu |
| `lil-skrrt tools` / `lil-skrrt skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.hermes/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `lil-skrrt gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `lil-skrrt cron run` separate from `lil-skrrt gateway`), register a `standalone_sender_fn`:

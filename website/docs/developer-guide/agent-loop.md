...
Lil Skrrt supports three API execution modes, resolved from provider selection, explicit args, and base URL heuristics:
...
- The session can be resumed later via `/resume` or `lil-skrrt chat --resume`

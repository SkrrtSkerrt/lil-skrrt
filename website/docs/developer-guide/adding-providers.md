...
description: "How to add a new inference provider to Lil Skrrt — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Lil Skrrt can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `lil-skrrt model` menu entries
...
:::tip
`hermes_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `lil-skrrt setup`.
:::
...
7. `lil-skrrt model` menu includes the provider
8. `lil-skrrt setup` wizard delegates to `main.py` automatically
...
- Provider-specific `lil-skrrt model` menu entries with bespoke auth flows
...
- What env vars should Lil Skrrt check, and in what priority order?
...
Be careful with API-key precedence. Lil Skrrt already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `hermes_cli/main.py`

A provider is not discoverable until it shows up in the interactive `lil-skrrt model` flow.
...
:::tip
`hermes_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `lil-skrrt model` and `lil-skrrt setup` automatically.
:::
...
When you add a native provider, double-check that Lil Skrrt is only sending fields that provider actually understands.
...
### 7. Updating `lil-skrrt model` but not `lil-skrrt setup`

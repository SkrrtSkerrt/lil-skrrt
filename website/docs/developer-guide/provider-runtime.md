...
description: "How Lil Skrrt resolves providers, credentials, API modes, and auxiliary models at runtime"
---

# Provider Runtime Resolution

Lil Skrrt has a shared provider runtime resolver used across:
...
That ordering matters because Lil Skrrt treats the saved model/provider choice as the source of truth for normal runs. This prevents a stale shell export from silently overriding the endpoint a user last selected in `lil-skrrt model`.
...
This resolver is the main reason Lil Skrrt can share auth/runtime logic between:
...
Lil Skrrt contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY` and `OPENAI_API_KEY`).
...
Lil Skrrt also distinguishes between:
...
When provider resolution selects `anthropic`, Lil Skrrt uses:
...
- Lil Skrrt preflights Anthropic credential refresh before native Messages API calls
- Lil Skrrt still retries once on a 401 after rebuilding the Anthropic client, as a fallback path
...
When an auxiliary task is configured with provider `main`, Lil Skrrt resolves that through the same shared runtime path as normal chat. In practice that means:

- env-driven custom endpoints still work
- custom endpoints saved via `lil-skrrt model` / `config.yaml` also work
- auxiliary routing can tell the difference between a real saved custom endpoint and the OpenRouter fallback

## Fallback models

Lil Skrrt supports a configured fallback provider chain — a list of `(provider, model)` entries tried in order when the primary model encounters errors. The legacy single-pair `fallback_model` dict is still accepted for back-compat (and migrated on first write).

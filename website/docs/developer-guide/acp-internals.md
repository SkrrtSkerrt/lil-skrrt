...
The ACP adapter wraps Lil Skrrt' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
- reject options -> Lil Skrrt `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Lil Skrrt tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Lil Skrrt' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Lil Skrrt provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run registry clients can open Lil Skrrt' interactive model/provider configuration before starting a normal ACP session.
...
- `hermes_cli/main.py` — `lil-skrrt acp` CLI subcommand

...
That single command runs the Portal OAuth, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `lil-skrrt chat` immediately after.

Don't have a subscription yet? [portal.github.com/SkrrtSkerrt/hermes-agent/manage-subscription](https://portal.github.com/SkrrtSkerrt/hermes-agent/manage-subscription) — sign up, then come back and run the command above.
...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.hermes/auth.json` is the only credential on disk, and Lil Skrrt mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
If you don't have a subscription yet, sign up at [portal.github.com/SkrrtSkerrt/hermes-agent/manage-subscription](https://portal.github.com/SkrrtSkerrt/hermes-agent/manage-subscription) first.

### Existing install — add Portal alongside other providers

If you already have Lil Skrrt configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `lil-skrrt model` between sessions — the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where Lil Skrrt is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding, `--manual-paste` for browser-only environments like Cloud Shell / Codespaces).

### Profile setup

If you use [Lil Skrrt profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
```bash
lil-skrrt portal status     # login status, subscription info, model + gateway routing
lil-skrrt portal tools      # detailed Tool Gateway catalog with per-tool routing
lil-skrrt portal open       # open the subscription management page in your browser
```
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `lil-skrrt tools` to pick backends per tool:
...
- **Web:** [portal.github.com/SkrrtSkerrt/hermes-agent/manage-subscription](https://portal.github.com/SkrrtSkerrt/hermes-agent/manage-subscription)
- **CLI shortcut:** `lil-skrrt portal open` (opens the same page in your default browser)

## Configuration reference

After `lil-skrrt setup --portal`, `~/.hermes/config.yaml` will look like:
...
Lil Skrrt mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.

If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so Lil Skrrt stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `lil-skrrt auth add nous` to log in again; the quarantine clears on the next successful login.

## Troubleshooting

### `lil-skrrt portal status` shows "not logged in"
...
Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `lil-skrrt auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
If a model is genuinely missing, [open an issue](https://github.com/SkrrtSkerrt/hermes-agent/issues) — we surface the Portal's catalog to Lil Skrrt and gaps usually mean a routing config we can update.
...
- **[Profiles](/user-guide/profiles)** — Multiple Lil Skrrt configurations sharing one Portal login

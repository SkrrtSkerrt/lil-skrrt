...
Browser-based dashboard for managing Lil Skrrt configuration, API keys, and monitoring active sessions.
...
`lil-skrrt dashboard` on port 9119 serves the **built** bundle from `hermes_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
- The dashboard preserves the Lil Skrrt brand uppercase aesthetic, but it is **opt-in per element, not global**.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Lil Skrrt” — mixed case, no Mondwest, no `text-display` |

...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some Lil Skrrt tools append a human hint after a JSON payload, e.g.
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Lil Skrrt tool result clearly failed.
    ...
    # Lil Skrrt core/polished tools commonly report tool-level failures as a
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # Lil Skrrt read_file output is line-numbered with `|`. If we send it as raw

"""ACP auth helpers — detect and advertise Lil Skrrt authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Lil Skrrt runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Lil Skrrt can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Lil Skrrt.
    ...
    not have Lil Skrrt provider credentials configured yet, so Lil Skrrt always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Lil Skrrt using the currently configured "

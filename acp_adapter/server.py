"""ACP agent server — exposes Lil Skrrt Agent via the Agent Client Protocol."""
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Lil Skrrt running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Lil Skrrt cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Lil Skrrt/OpenAI-compatible user content payload."""
...
class LilSkrrtACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Lil Skrrt AIAgent."""
    ...
    _SLASH_COMMANDS = {
        ...
        "version": "Show Lil Skrrt version",
    }
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Lil Skrrt version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Lil Skrrt maps edit approval
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Lil Skrrt estimates ``used`` from the same
    ...
    async def _send_session_info_update(self, session_id: str) -> None:
        """Send ACP native session metadata after Lil Skrrt changes it."""
        if not self._conn:
            return
        try:
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Lil Skrrt' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Lil Skrrt setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Lil Skrrt
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Lil Skrrt on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Lil Skrrt' local kawaii waiting/status
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Lil Skrrt v{HERMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Lil Skrrt has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
...
HermesACPAgent = LilSkrrtACPAgent

...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Lil Skrrt manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Lil Skrrt session")
        ...
        @tree.command(name="status", description="Show Lil Skrrt session status")
        ...
        @tree.command(name="stop", description="Stop the running Lil Skrrt agent")
        ...
        @tree.command(name="update", description="Update Lil Skrrt Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Lil Skrrt gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Lil Skrrt session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Lil Skrrt in the thread",
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Lil Skrrt skill",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Lil Skrrt session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Lil Skrrt: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Lil Skrrt"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Lil Skrrt: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        reason = "Lil Skrrt session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Lil Skrrt handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Lil Skrrt needs your input",
                description=body,
                color=discord.Color.orange(),
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``lil-skrrt update --gateway``
        ...
        try:
            ...
            embed = discord.Embed(
                title="☠︎ Update Needs Your Input",
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``lil-skrrt update`` prompts.
...
# is not in this process (e.g. ``lil-skrrt cron`` running standalone) and no live
...
def register(ctx) -> None:
    """Plugin entry point — called by the Lil Skrrt plugin system."""

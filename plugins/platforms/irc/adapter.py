"""
IRC Platform Adapter for Lil Skrrt Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Lil Skrrt agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Lil Skrrt Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Lil Skrrt Agent shutting down")
...
def interactive_setup() -> None:
    ...
    print_info("Connect Lil Skrrt to an IRC network. Uses Python stdlib — no extra packages needed.")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Lil Skrrt Agent (cron)")

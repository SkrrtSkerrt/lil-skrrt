...
Handles: lil-skrrt honcho setup | status | sessions | map | peer
"""
...
def cmd_sync(args) -> None:
    ...
    Scans all Lil Skrrt profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'lil-skrrt honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'lil-skrrt honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `lil-skrrt update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Lil Skrrt profile."""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Lil Skrrt persistent cross-session memory.")
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'lil-skrrt honcho setup' again once you have a key.\n")
            return
    ...
    try:
        ...
        print("  Run: lil-skrrt config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    lil-skrrt honcho status     -- show full config")
    print("    lil-skrrt honcho mode       -- change recall/observation mode")
    print("    lil-skrrt honcho tokens     -- tune context and dialectic budgets")
    print("    lil-skrrt honcho peer       -- update peer names")
    print("    lil-skrrt honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Lil Skrrt profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: lil-skrrt honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'lil-skrrt honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: lil-skrrt honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Lil Skrrt' identity in Honcho. Seed with 'lil-skrrt honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: lil-skrrt honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: lil-skrrt honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Lil Skrrt asks Honcho's AI peer a question")
        ...
        print("\n  Set with: lil-skrrt honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'lil-skrrt honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'lil-skrrt honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    lil-skrrt honcho identity --show        — show both peer representations")
        print("    lil-skrrt honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Lil Skrrt + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Lil Skrrt\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Lil Skrrt persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  lil-skrrt honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'lil-skrrt honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'lil-skrrt honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  lil-skrrt honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print("  (Lil Skrrt calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    lil-skrrt honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'lil-skrrt honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Lil Skrrt, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'lil-skrrt honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    lil-skrrt honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  lil-skrrt honcho identity <path/to/SOUL.md>")
    ...
    print("    Lil Skrrt:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Lil Skrrt:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Lil Skrrt:   Honcho observes every message and updates representations")
    ...
    print("    Lil Skrrt:   per-session by default — each run gets its own session")
    print("              Map a custom name:  lil-skrrt honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. lil-skrrt honcho setup              — configure API key (required)")
        print("  2. lil-skrrt honcho migrate            — re-run this walkthrough")
    else:
        print("  1. lil-skrrt honcho status             — verify Honcho connection")
        print("  2. lil-skrrt                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. lil-skrrt honcho identity --show    — verify AI peer representation")
        print("  4. lil-skrrt honcho tokens             — tune context and dialectic budgets")
        print("  5. lil-skrrt honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'lil-skrrt memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``lil-skrrt honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``lil-skrrt honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to lil-skrrt memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Lil Skrrt Honcho",
    )

...
```bash
lil-skrrt honcho setup    # full interactive wizard (cloud or local)
lil-skrrt memory setup    # generic picker, also works
```

Or manually:
```bash
lil-skrrt config set memory.provider honcho
...
- **`per-directory`** — basename of `$PWD`. Opening lil-skrrt in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Lil Skrrt session ID (timestamp + hex). Every `lil-skrrt` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Lil Skrrt profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
| Command | Description |
|---------|-------------|
| `lil-skrrt honcho setup` | Full interactive setup wizard |
| `lil-skrrt honcho status` | Show resolved config for active profile |
| `lil-skrrt honcho enable` / `disable` | Toggle Honcho for active profile |
| `lil-skrrt honcho mode <mode>` | Change recall or observation mode |
| `lil-skrrt honcho peer --user <name>` | Update user peer name |
| `lil-skrrt honcho peer --ai <name>` | Update AI peer name |
| `lil-skrrt honcho tokens --context <N>` | Set context token budget |
| `lil-skrrt honcho tokens --dialectic <N>` | Set dialectic max chars |
| `lil-skrrt honcho map <name>` | Map current directory to a session name |
| `lil-skrrt honcho sync` | Create host blocks for all Lil Skrrt profiles |

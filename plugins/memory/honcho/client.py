...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated Lil Skrrt instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Lil Skrrt profile.
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Lil Skrrt system prompt
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Lil Skrrt profile.
        """
        resolved_host = host or resolve_active_host()
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Lil Skrrt session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Lil Skrrt session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Lil Skrrt session_id (new Honcho session each run)

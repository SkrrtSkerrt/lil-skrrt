...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Lil Skrrt can degrade gracefully instead of repeatedly trying to start
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Lil Skrrt config."""
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Lil Skrrt profile name (from agent_identity)
      {workspace} — Lil Skrrt workspace name (from agent_workspace)
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Lil Skrrt Agent and the User"
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Lil Skrrt Agent and the User"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Lil Skrrt Agent and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Lil Skrrt created/used that client on the shared async loop,

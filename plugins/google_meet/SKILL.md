...
lil-skrrt meet install                 # pip deps + Chromium (transcribe only)
lil-skrrt meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
lil-skrrt meet auth                    # optional; skips guest-lobby wait
lil-skrrt meet setup                   # preflight checks
```

`lil-skrrt meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
pip install playwright websockets && python -m playwright install chromium
lil-skrrt plugins enable google_meet
lil-skrrt meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
lil-skrrt meet node approve my-mac ws://<mac-ip>:18789 <token>
lil-skrrt meet node ping my-mac                   # confirm reachable
```

Run `lil-skrrt meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Lil Skrrt agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `lil-skrrt meet auth` avoids this.
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HERMES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `lil-skrrt meet node approve`.

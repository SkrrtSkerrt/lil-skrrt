...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `lil-skrrt meet` CLI |
...
| `cli.py` | `lil-skrrt meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `lil-skrrt meet node {run,list,approve,remove,status,ping}` |
...
lil-skrrt meet install                                      # pip + Chromium
lil-skrrt meet setup                                        # preflight
lil-skrrt meet auth                                         # optional
lil-skrrt meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
lil-skrrt meet install --realtime                     # installs pulseaudio-utils
...
lil-skrrt meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
lil-skrrt meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
...
lil-skrrt plugins enable google_meet
lil-skrrt meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
lil-skrrt meet node ping my-mac

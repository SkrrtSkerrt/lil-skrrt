...
Lil Skrrt Achievements scans local Lil Skrrt sessions and unlocks badges based on real agent behavior:
...
- **Secret** — hidden until Lil Skrrt detects the first related signal
...
Clone into your Lil Skrrt plugins directory:
...
If backend API routes 404, restart `lil-skrrt dashboard`; plugin APIs are mounted at dashboard startup.
...
If the update changes backend routes or `plugin_api.py`, restart `lil-skrrt dashboard` after pulling.
...
Achievement unlock state is stored locally in `state.json` and is not overwritten by git updates. New achievements are evaluated from your existing Lil Skrrt session history. Achievement IDs are stable and should not be renamed casually because they are the unlock-state keys.

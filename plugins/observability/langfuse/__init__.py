"""langfuse — Lil Skrrt plugin for Langfuse observability.

Traces Lil Skrrt conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Lil Skrrt plugin system — standalone plugins only
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Lil Skrrt plugin system —
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Lil Skrrt branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Lil Skrrt also has a turn-scoped
    ...
    # messages is a list only for legacy Lil Skrrt branches that fired
    # pre_llm_call with API messages directly. Current Lil Skrrt fires
...
def register(ctx) -> None:
    ...
    # Lil Skrrt versions.  pre_api_request / post_api_request fire per API

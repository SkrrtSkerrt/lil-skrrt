...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Lil-Skrrt-Session-Id header; opt-in long-term memory scoping via X-Lil-Skrrt-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Lil-Skrrt-Session-Key supported)
...
API_SERVER_ADAPTER_KEY = web.AppKey("api_server_adapter", object) if AIOHTTP_AVAILABLE else "api_server_adapter"
...
def _derive_chat_session_id(
    ...
    the same Lil Skrrt session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Lil-Skrrt-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Lil-Skrrt-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Lil-Skrrt-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Lil-Skrrt-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Lil-Skrrt-Session-Key requires API key authentication. "
    ...
    def _create_agent(
        ...
        by the client (via ``X-Lil-Skrrt-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Lil Skrrt version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Lil Skrrt AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Lil-Skrrt-Session-Id",
                "session_key_header": "X-Lil-Skrrt-Session-Key",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Lil-Skrrt-Session-Key.  This
        # is independent of X-Lil-Skrrt-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Lil-Skrrt-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Lil-Skrrt-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Lil-Skrrt-Session-Id rejected: "
            ...
            # conversation map to the same Lil Skrrt session.  The first user
        ...
        response_headers = {
            "X-Lil-Skrrt-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Lil-Skrrt-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            response_headers["X-Lil-Skrrt-Completed"] = "false"
            response_headers["X-Lil-Skrrt-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Lil Skrrt-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Lil-Skrrt-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Lil-Skrrt-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Lil-Skrrt-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Lil-Skrrt-Session-Key"] = gateway_session_key
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Lil-Skrrt-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Lil-Skrrt-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            # (e.g. X-Lil-Skrrt-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Lil-Skrrt-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(

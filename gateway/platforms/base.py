...
# Audio file extensions Lil Skrrt recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Lil Skrrt' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Lil Skrrt-created Telegram private-chat
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The Lil Skrrt home itself contains credentials (auth.json, .env) — only the
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    existing regular files under Lil Skrrt-managed media caches, or roots the

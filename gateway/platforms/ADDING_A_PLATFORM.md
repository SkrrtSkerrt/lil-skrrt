...
There are two ways to add a platform to the Lil Skrrt gateway:
...
**zero changes to core Lil Skrrt code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `lil-skrrt gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
Checklist for integrating a platform directly into the Lil Skrrt core.

...
# Lil Skrrt-internal env vars that should NOT leak into terminal subprocesses.
...
def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local Lil Skrrt home override into subprocess env."""
    try:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Lil Skrrt-managed secrets from a subprocess environment."""
    try:
...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. Lil Skrrt Agent requires Git for Windows on Windows.\n"
...
def _make_run_env(env: dict) -> dict:
    ...
    # Lil Skrrt is driving via _find_bash (Git Bash), and Git Bash itself
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Lil Skrrt trusts them.
    """

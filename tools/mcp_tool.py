...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Lil Skrrt MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Lil Skrrt gateways know how to render.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Lil Skrrt is fully
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Lil Skrrt config file.
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Lil Skrrt' MEDIA: tag convention and out to messaging
            ...
            # Lil Skrrt' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Lil Skrrt's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Lil Skrrt registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Does NOT register tools in the Lil Skrrt registry.

...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.hermes/config.yaml``. Lil Skrrt
...
# custom voice-cloning scripts, etc.) into Lil Skrrt without any Python code
...
#           output_format: wav
#
# Lil Skrrt writes the input text to a temp UTF-8 file, runs the command with
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when Lil Skrrt actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Lil Skrrt caches Piper voice models.

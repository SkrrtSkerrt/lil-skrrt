...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Lil Skrrt convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _git_env(
    ...
    The shared store is internal Lil Skrrt infrastructure — it must NOT inherit
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.name", "Lil Skrrt Checkpoint"], store, cfg_wd)

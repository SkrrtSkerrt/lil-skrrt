...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Lil Skrrt exit
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    # interactive Lil Skrrt password prompt or the sudo -S password-pipe path.
    ...
    # silently without Lil Skrrt prompting.
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Lil Skrrt can track lifecycle and output.
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, Lil SkrrtSweEnv, ...)
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Lil Skrrt can track the process, then run "

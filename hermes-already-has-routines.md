# Lil Skrrt Has Had "Routines" Since March
...
Lil Skrrt equivalent — works today:
```bash
lil-skrrt cron create "0 2 * * *" \
...
Lil Skrrt equivalent — works today:
```bash
lil-skrrt webhook subscribe auth-watch \
  --events "pull_request" \
...
Lil Skrrt equivalent — works today:
```bash
lil-skrrt webhook subscribe alert-triage \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Lil Skrrt implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Lil Skrrt |
|---|---|---|
...
## Things Lil Skrrt Does That Routines Can't
...
```bash
lil-skrrt cron create "every 1h" \
...
Lil Skrrt has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Lil Skrrt is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
lil-skrrt gateway setup    # enable webhooks
lil-skrrt webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/guides/automation-templates](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/guides/automation-templates)

Documentation: [hermes-agent.github.com/SkrrtSkerrt/hermes-agent](https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent)

GitHub: [github.com/SkrrtSkerrt/hermes-agent](https://github.com/SkrrtSkerrt/hermes-agent)

---

*Lil Skrrt is built by [SkrrtSkerrt](https://github.com/SkrrtSkerrt/hermes-agent). Open source, model-agnostic, runs on your infrastructure.*

"""Shared constants for Lil Skrrt Agent.
...
def set_hermes_home_override(path: str | Path | None) -> Token:
    """Set a context-local Lil Skrrt home override and return its reset token.
...
def reset_hermes_home_override(token: Token) -> None:
    """Restore the previous context-local Lil Skrrt home override."""
    _HERMES_HOME_OVERRIDE.reset(token)


def get_hermes_home_override() -> str | None:
    """Return the active context-local Lil Skrrt home override, if any."""
...
def get_hermes_home() -> Path:
    """Return the Lil Skrrt home directory (default: ~/.hermes).
    ...
    ``hermes_cli/kanban_db.py``).  See https://github.com/SkrrtSkerrt/hermes-agent/issues/18594.
    """
...
def get_default_hermes_root() -> Path:
    """Return the root Lil Skrrt directory for profile-level operations.
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Lil Skrrt is installed
...
def get_hermes_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Lil Skrrt subdirectory with backward compatibility.
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/SkrrtSkerrt/hermes-agent/issues/25821.
    """
...
def get_subprocess_home() -> str | None:
    ...
    inside the Lil Skrrt data directory instead of the OS-level ``/root`` or
    ``~/``.  This provides:

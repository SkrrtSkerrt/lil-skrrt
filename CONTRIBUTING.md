# Contributing to Lil Skrrt

Thank you for contributing to Lil Skrrt! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Lil Skrrt to work everywhere.
...
Bundled skills (in `skills/`) ship with every Lil Skrrt install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `lil-skrrt skills browse` (labeled "official") and install it with `lil-skrrt skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [SkrrtSkerrt Discord](https://discord.gg/SkrrtSkerrt). Users can install it with `lil-skrrt skills install`.
...
- Integrate with `lil-skrrt memory setup` via `post_setup()` — no need to touch core code
...
```bash
git clone --recurse-submodules https://github.com/SkrrtSkerrt/hermes-agent.git
cd hermes-agent
...
├── tests/                    # Test suite
├── website/                  # Documentation site (hermes-agent.github.com/SkrrtSkerrt/hermes-agent)
│
...
The user may skip setup and keep loading the skill. Lil Skrrt only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `lil-skrrt setup` or update `~/.hermes/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Lil Skrrt" is the secondary collaborator. If the contributor's commit shows "Lil Skrrt" as author because they used Lil Skrrt to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Lil Skrrt tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `lil-skrrt --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Lil Skrrt uses a data-driven skin system — no code changes needed to add a new skin.
...
Lil Skrrt runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the lil-skrrt wrapper (it has a stdlib fallback
...
Lil Skrrt has terminal access. Security matters.
...
2. **Test manually**: Run `lil-skrrt` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/SkrrtSkerrt/hermes-agent/issues)
- Include: OS, Python version, Lil Skrrt version (`lil-skrrt version`), full error traceback
...
- **Discord**: [discord.gg/SkrrtSkerrt](https://discord.gg/SkrrtSkerrt) — for questions, showcasing projects, and sharing skills

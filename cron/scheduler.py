...
def _get_hermes_home() -> Path:
    """Resolve Lil Skrrt home dynamically while preserving test monkeypatch hooks."""
...
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Lil Skrrt profile.
    ...
    the scheduler's test/override hook and a context-local Lil Skrrt home override
    ...
    try:
        ...
        logger.info(
            "Job '%s': using Lil Skrrt profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        # Lil Skrrt home override, scheduler _hermes_home hook, and temporary

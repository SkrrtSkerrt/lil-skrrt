# Lil Skrrt Security Policy

This document describes Lil Skrrt's trust model, names the one
...
security vulnerabilities. **Lil Skrrt does not operate a bug
bounty program.**
...
- Environment details (`lil-skrrt version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Lil Skrrt access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Lil Skrrt's documentation
...
Lil Skrrt supports two OS-level isolation postures. They address
...
Lil Skrrt supports this in two ways:

- **Lil Skrrt's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Lil Skrrt's in-process heuristics
...
Lil Skrrt filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Lil Skrrt
itself. Bugs in Lil Skrrt's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Lil Skrrt does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Lil Skrrt's own documentation, or reasonable
  operator expectations would predict — including cases where
  Lil Skrrt has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Lil Skrrt's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Lil Skrrt. Bugs in Lil Skrrt
...
- Lil Skrrt includes supply-chain guards for MCP server

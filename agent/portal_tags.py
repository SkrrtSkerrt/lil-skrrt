...
Every Lil Skrrt request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Lil Skrrt Agent and bucket it by client release.
...
def _hermes_version() -> str:
    """Return the current Lil Skrrt release version, e.g. ``"0.13.0"``.

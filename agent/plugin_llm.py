...
Plugins built on Lil Skrrt Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Lil Skrrt
...
embedded slugs, no shorthands. This mirrors Lil Skrrt' main config
...
handles every provider, fallback chain, and per-task override Lil Skrrt
supports.
"""

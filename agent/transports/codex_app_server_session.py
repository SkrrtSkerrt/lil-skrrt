...
Owns one Codex thread per Lil Skrrt session. Drives `turn/start`, consumes
...
# Lil Skrrt' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Lil Skrrt/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Lil Skrrt session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Lil Skrrt' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Lil Skrrt' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Lil Skrrt' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Lil Skrrt approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Lil Skrrt returns 'once', 'session', 'always', or 'deny'.
...
def _get_hermes_version() -> str:
    """Best-effort Lil Skrrt version string for codex's userAgent line."""
    try:

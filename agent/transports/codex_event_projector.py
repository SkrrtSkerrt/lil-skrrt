"""Projects codex app-server events into Lil Skrrt' messages list.

The translator that lets Lil Skrrt' memory/skill review keep working under the
...
Lil Skrrt' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Lil Skrrt
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Lil Skrrt' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Lil Skrrt stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Lil Skrrt already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Lil Skrrt' messages store text only.
        text_parts: list[str] = []

...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Lil Skrrt does
...
class EntraIdentityConfig:
    ...
    Captures the Lil Skrrt-managed Entra knobs we need outside Azure SDK
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only Lil Skrrt-selected knobs are passed as kwargs. Everything else
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    Lil Skrrt processes use exactly one Entra config at a time (the

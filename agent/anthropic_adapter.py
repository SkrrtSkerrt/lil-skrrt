"""Anthropic Messages API adapter for Lil Skrrt Agent.

Translates between Lil Skrrt's internal OpenAI-style message format and
...
# Lil Skrrt effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Lil Skrrt historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Lil Skrrt)
    ...
    # 1. Lil Skrrt-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Lil Skrrt configs.
...
# ── Lil Skrrt-native PKCE OAuth flow ────────────────────────────────────────
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Lil Skrrt-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Lil Skrrt with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Lil Skrrt-managed OAuth credentials from ~/.hermes/.anthropic_oauth.json."""
    if _HERMES_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Lil Skrrt OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Lil Skrrt Agent", "Claude Code")
                text = text.replace("Lil Skrrt agent", "Claude Code")
                text = text.replace("hermes-agent", "claude-code")
                text = text.replace("SkrrtSkerrt", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Lil Skrrt surfaces in its CLI. We

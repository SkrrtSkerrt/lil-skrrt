...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Lil Skrrt triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent",
    "X-Title": "Lil Skrrt Agent",
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "Lil SkrrtAgent",
}
...
_NOUS_DEFAULT_BASE_URL = "https://inference-api.github.com/SkrrtSkerrt/hermes-agent/v1"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Lil Skrrt Agent)",
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Lil Skrrt auth store.
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.github.com/SkrrtSkerrt/hermes-agent"):
        return "nous"
    ...
    *,
    ...
    try:
        ...
        client_is_nous = (
            ...
            or base_url_host_matches(_base_info, "inference-api.github.com/SkrrtSkerrt/hermes-agent")
        )
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        client_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "inference-api.github.com/SkrrtSkerrt/hermes-agent")
        )
        if _is_auth_error(first_err) and client_is_nous:
            refreshed_client, refreshed_model = _refresh_nous_auxiliary_client(
                cache_provider=resolved_provider or "nous",

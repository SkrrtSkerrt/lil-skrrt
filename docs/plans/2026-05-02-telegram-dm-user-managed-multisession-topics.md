...
> **For Lil Skrrt:** Use test-driven-development for implementation. Use subagent-driven-development only after this plan is split into small reviewed tasks.

**Goal:** Add an opt-in Telegram DM multi-session mode where Telegram user-created private-chat topics become independent Lil Skrrt session lanes, while the root DM becomes a system lobby.

**Architecture:** Rely on Telegram's native private-chat topic UI. Users create new topics with the `+` button; Lil Skrrt maps each `message_thread_id` to a separate session lane. Lil Skrrt does not create topics for normal `/new` flow and does not try to manage topic lifecycle beyond activation/status, root-lobby behavior, and restoring legacy sessions into a user-created topic.

**Tech Stack:** Lil Skrrt gateway, Telegram Bot API 9.4+, python-telegram-bot adapter, SQLite SessionDB / side tables, pytest.
...
1. Lil Skrrt receives a Telegram DM message with `message_thread_id`.
2. Lil Skrrt derives the existing thread-aware `session_key` from `(platform=telegram, chat_type=dm, chat_id, thread_id)`.
3. If no binding exists, Lil Skrrt creates a fresh Lil Skrrt session for this topic lane and persists the binding.
...
6. send two messages into the topic:
   ...
   - last Lil Skrrt assistant message if available.
...
- upgrading Lil Skrrt and starting the gateway must not create Telegram topic-mode tables or columns;
...
Stores Telegram topic/thread to Lil Skrrt session binding. Created only by `apply_telegram_topic_migration()`.
...
Normal Lil Skrrt agent flow for that topic's session lane.
...
5. restore emits confirmation and last Lil Skrrt assistant message.
...
- User-created topics automatically become independent Lil Skrrt session lanes.
...
- `/topic <session_id>` inside a topic restores that session and sends confirmation + last Lil Skrrt assistant message.

...
**Goal:** Replace `tini` with s6-overlay as PID 1 in the Lil Skrrt Docker image so
that the main lil-skrrt process, the dashboard, and dynamically-created
...
s6-svscan as PID 1. Main lil-skrrt and the dashboard are declared as static
s6-rc services at image build time. Per-profile gateways — which users create
*after* the image is built (`lil-skrrt profile create coder` →
...
- Dynamic registration is limited to per-profile gateways — one service per
  ...
  profile so `lil-skrrt gateway start` (no `-p`) has somewhere to land.
...
1. `gosu` privilege drop from root → `lil-skrrt` UID.
...
4. Optionally backgrounded `lil-skrrt dashboard` in a subshell when
   `HERMES_DASHBOARD=1` — **not supervised**, no restart.
5. `exec lil-skrrt "$@"` — tini's sole direct child.
...
`lil-skrrt gateway start`, `coder gateway start` (profile alias), and
`lil-skrrt -p <profile> gateway start` all spawn a gateway process scoped to a
given profile. See
...
- **Root/non-root model:** `/init` runs as root to set up the supervision
  ...
  `s6-setuidgid lil-skrrt` in its `run` script. The per-service `s6-supervise`
  monitor stays root so it can signal its child regardless of UID. Net
  effect: lil-skrrt and all its subprocesses run as UID 10000 exactly as
...
- Zombie reaping: PID 1 (s6-svscan) reaps all zombies non-blockingly on
  SIGCHLD. Any subagent subprocess spawned by the main lil-skrrt process is
...
Container ENTRYPOINT is `/init`, PID 1 is s6-svscan. The main lil-skrrt
...
### D2. Main lil-skrrt is an s6 service with container-exit semantics

The contract "container exits when `lil-skrrt` exits" is preserved via a
...
|---|---|
| (no args) | `lil-skrrt` with no args, container exits when lil-skrrt exits |
| `chat -q "..."` | `lil-skrrt chat -q "..."`, container exits with lil-skrrt exit code |
...
- **s6-rc** (declarative, compile-then-swap): used for main lil-skrrt and the
...
(`lil-skrrt -p <profile> gateway run`, or `lil-skrrt gateway run` for the default
...
and silently failed for the unprivileged lil-skrrt user (UID 10000), making
...
When `lil-skrrt profile create <name>` runs inside the container, the
profile-creation code path calls
`ServiceManager.register_profile_gateway(<name>)` if
`supports_runtime_registration()` is True. When `lil-skrrt profile delete
...
Existing per-profile `lil-skrrt -p <profile> gateway start/stop/restart` CLI
...
`lil-skrrt gateway start` (no `-p`) targets a special `gateway-default` slot
...
`--all` lifecycle (`lil-skrrt gateway stop --all`, `... restart --all`)
...
main-hermes service so the lil-skrrt CMD becomes that main program.
...
In TUI mode main lil-skrrt is effectively unsupervised (same as the pre-s6
...
| A `running` gateway that's actually broken auto-restarts into a crash loop after every container restart | Low | Medium | s6 `finish` script `max_restarts` cap (planned); follow-up: `lil-skrrt doctor` alerts when N consecutive container restarts ended in `startup_failed` |
| `_s6_running()` detection works as root but silently fails for unprivileged lil-skrrt user, making runtime-registration path inert | High (without mitigation) | High | **Caught in PR review.** Detection now probes `/proc/1/comm` (world-readable) + `/run/s6/basedir`. Docker integration tests refactored to `docker exec -u lil-skrrt` so the realistic runtime user is exercised |
| `s6-svscanctl` from lil-skrrt hits EACCES on the root-owned control FIFO | Medium | Medium | `02-reconcile-profiles` chowns `/run/service/.s6-svscan/{control,lock}` to lil-skrrt after stage1 creates them |
| Per-service `supervise/control` FIFO is root-owned by s6-supervise, blocking `s6-svc` from lil-skrrt | Known | Medium | Surfaced cleanly as `S6CommandError` (with rc + stderr) instead of raw `CalledProcessError`. Permission fix tracked as a follow-up (small SUID helper, polling chown loop in cont-init.d, or replace `s6-svc` with `down`-marker manipulation) |
...
| # | Question | Decision |
|---|---|---|
| OQ1 | Gate Phase 2 behind env var? | Ship directly (Lil Skrrt is pre-1.0; users can pin the previous image) |
| OQ2 | s6 root model | Root `/init`, drop per-service via `s6-setuidgid lil-skrrt` |
...
- **`gateway-default` slot:** always registered by the reconciler so
  `lil-skrrt gateway start` (no `-p`) has somewhere to land.
...
- [x] `lil-skrrt profile create test` inside a container creates
      `/run/service/gateway-test/`
- [x] `lil-skrrt -p test gateway start` inside a container dispatches through s6
- [x] `lil-skrrt -p test gateway stop` inside a container cleanly stops via s6
- [x] `lil-skrrt profile delete test` inside a container removes
      `/run/service/gateway-test/`
- [x] Profile gateway logs persist at
      `$HERMES_HOME/logs/gateways/test/current`
- [x] `lil-skrrt status` inside the container shows `Manager: s6`
- [x] `lil-skrrt gateway start` (no `-p`) inside a container targets
      `gateway-default` and runs against the root profile
- [x] `lil-skrrt gateway stop --all` / `... restart --all` iterate every
...
- [x] `lil-skrrt gateway install/start/stop` on Linux host and macOS host

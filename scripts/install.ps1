...
$RepoUrlSsh = "git@github.com:SkrrtSkerrt/hermes-agent.git"
$RepoUrlHttps = "https://github.com/SkrrtSkerrt/hermes-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/SkrrtSkerrt/hermes-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/SkrrtSkerrt/hermes-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/SkrrtSkerrt/hermes-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/SkrrtSkerrt/hermes-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow

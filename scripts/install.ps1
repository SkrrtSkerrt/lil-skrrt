...
$RepoUrlSsh = "git@github.com:SkrrtSkerrt/lil-skrrt.git"
$RepoUrlHttps = "https://github.com/SkrrtSkerrt/lil-skrrt.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Install-Git {
    ...
    which Lil Skrrt uses to run shell commands.
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Lil Skrrt needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Lil Skrrt copy of Git ourselves is predictable and
    ...
    ``HERMES_GIT_BASH_PATH`` (User scope) so Lil Skrrt can find it in a fresh
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Lil Skrrt features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Lil Skrrt needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HERMES_GIT_BASH_PATH`` (User env scope) so Lil Skrrt can find it even before
    ...
    Write-Warn "Could not locate bash.exe -- Lil Skrrt may not find Git Bash."
...
function Test-Node {
    ...
    if (Test-Path $managedNode) {
        ...
        $env:Path = "$HermesHome\node;$env:Path"
        Write-Success "Node.js $version found (Lil Skrrt-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/SkrrtSkerrt/lil-skrrt/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/SkrrtSkerrt/lil-skrrt/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/SkrrtSkerrt/lil-skrrt/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
function Set-PathVariable {
    ...
    # Add the venv Scripts dir to user PATH so hermes / lil-skrrt are globally available
    # On Windows, the hermes.exe and lil-skrrt.exe in venv\Scripts\ have the venv Python baked in
...
function Copy-ConfigTemplates {
    ...
    # Create HERMES_HOME directory structure
...
You are a cold, sharp tactical assistant.

- No fluff.
- No hand-holding.
- No playful tone.
- No emotional mirroring.
- No reassurance unless it changes the outcome.
- No "happy to help", "sure", or "of course".
- No rhetorical framing.
- Use command language, not conversational language.
- State facts plainly.
- Prefer one-line answers when sufficient.
- Ask one precise question at a time.
- If user input is needed, ask with minimal explanation.
- If the user is ambiguous, state the ambiguity and the minimum needed to resolve it.
- Avoid filler, hedging, and unnecessary context unless asked.
- Prioritize action, verification, and clear next steps.
- Be blunt, but stay professional.
- Frame responses as mission status when useful: `STATUS`, `ACTION`, `BLOCKER`, `NEXT`.
- Prefer imperatives over explanation.
- If uncertain, say so directly. No padding.
- No humor, idioms, metaphors, or emotional filler.
- Do not soften bad news.
- When giving options, mark one as recommended.
- If blocked, state exactly what is needed to clear it.
- Keep answers in bullets unless a full explanation is requested.
- Use short, hard lines. No soft transitions.
- Treat every response as an operational report.
- Assume competence. Skip basic reassurance.
- Never volunteer extra context unless the user asks for it or it reduces risk or removes a blocker.
"@
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'lil-skrrt setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
...
function Install-PlatformSdks {
    ...
    # HERMES_HOME\.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'lil-skrrt setup'."
        return
    }
    ...
    # Run lil-skrrt setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
...
function Start-GatewayIfConfigured {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: lil-skrrt gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: lil-skrrt gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: lil-skrrt gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   lil-skrrt           " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   lil-skrrt setup     " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   lil-skrrt config    " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   lil-skrrt config edit" -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   lil-skrrt gateway   " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   lil-skrrt update    " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Lil Skrrt repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Lil Skrrt to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/SkrrtSkerrt/lil-skrrt/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow

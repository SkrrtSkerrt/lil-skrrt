"""lil-skrrt hooks — inspect and manage shell-script hooks.

Usage::

    lil-skrrt hooks list
    lil-skrrt hooks test <event> [--for-tool X] [--payload-file F]
    lil-skrrt hooks revoke <command>
    lil-skrrt hooks doctor
...
def hooks_command(args) -> None:
    """Entry point for ``lil-skrrt hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: lil-skrrt hooks {list|test|revoke|doctor}")
        print("Run 'lil-skrrt hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        ...
        print("See `lil-skrrt hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `lil-skrrt hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `lil-skrrt hooks test` and `lil-skrrt hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Lil Skrrt wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `lil-skrrt hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `lil-skrrt hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `lil-skrrt hooks doctor`.")

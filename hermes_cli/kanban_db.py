...
``<root>`` is the **shared Lil Skrrt root** (the parent of any active
...
the profile's Lil Skrrt home but are otherwise isolated: a worker spawned
...
def kanban_home() -> Path:
    """Return the shared Lil Skrrt root that anchors the kanban board.
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Lil Skrrt treats as user-controlled-but-trusted on the
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Lil Skrrt' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Lil Skrrt-managed storage. A board ``default_workdir`` pointing at a
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Lil Skrrt
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Lil Skrrt profile.
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Lil Skrrt profile.
        # `_default_spawn` invokes ``hermes -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Lil Skrrt
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Lil Skrrt CLI invocation."""
...
def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Lil Skrrt shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Lil Skrrt executable path.
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       Lil Skrrt is launched from a venv and the ``hermes`` shim is not on
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Lil Skrrt Agent or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Lil Skrrt root exists

...
_TASK_NAME_DEFAULT = "Lil Skrrt_Gateway"
_TASK_DESCRIPTION = "Lil Skrrt Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Lil Skrrt profile."""
...
def get_task_name() -> str:
    ...
    Default profile: ``Lil Skrrt_Gateway``
    Named profile X: ``Lil Skrrt_Gateway_<X>``
    """
    _assert_windows()
...
def get_task_script_path() -> Path:
    ...
    Lil Skrrt installs stay self-contained).
    """
    _assert_windows()
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Lil Skrrt builds and failed
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `lil-skrrt -p <profile> gateway install` must be safe.
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"lil-skrrt {profile_arg} gateway start" if profile_arg else "lil-skrrt gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: lil-skrrt gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Lil Skrrt gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: lil-skrrt gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: lil-skrrt gateway start")
        _print_next_steps()
        return
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Lil Skrrt gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: lil-skrrt gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: lil-skrrt gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: lil-skrrt gateway start")
            return

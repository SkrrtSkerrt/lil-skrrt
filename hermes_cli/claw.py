"""lil-skrrt claw — OpenClaw migration commands.

Usage:
    lil-skrrt claw migrate              # Preview then migrate (always shows preview first)
    lil-skrrt claw migrate --dry-run    # Preview only, no changes
    lil-skrrt claw migrate --yes        # Skip confirmation prompt
    lil-skrrt claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    lil-skrrt claw migrate --no-backup  # Skip pre-migration snapshot
    lil-skrrt claw cleanup              # Archive leftover OpenClaw directories
    lil-skrrt claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Lil Skrrt may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Lil Skrrt gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Lil Skrrt gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'lil-skrrt stop'.")
    print()
...
def claw_command(args):
    """Route lil-skrrt claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: lil-skrrt claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Lil Skrrt")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'lil-skrrt claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Lil Skrrt migration."""
    ...
    # migrate-lil-skrrt posture (two-phase: run once without secrets, rerun with
    ...
    print(
        color(
            "│          ☠︎ Lil Skrrt — OpenClaw Migration                 │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: lil-skrrt claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: lil-skrrt claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Lil Skrrt home ─────────
    ...
    # restorable with `lil-skrrt import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: lil-skrrt import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Lil Skrrt home."
            )
    ...
    # 'lil-skrrt claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: lil-skrrt claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: lil-skrrt claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  lil-skrrt claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  lil-skrrt claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  lil-skrrt config set OPENROUTER_API_KEY sk-or-v1-...")

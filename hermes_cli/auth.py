"""
Multi-provider authentication system for Lil Skrrt Agent.
...
DEFAULT_NOUS_PORTAL_URL = "https://portal.github.com/SkrrtSkerrt/hermes-agent"
DEFAULT_NOUS_INFERENCE_URL = "https://inference-api.github.com/SkrrtSkerrt/hermes-agent/v1"
...
SPOTIFY_DOCS_URL = "https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://hermes-agent.github.com/SkrrtSkerrt/hermes-agent/docs/guides/oauth-over-ssh"
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `lil-skrrt model` to re-authenticate."
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``lil-skrrt auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Lil Skrrt.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `lil-skrrt logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'lil-skrrt doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'lil-skrrt model' for available providers, or run 'lil-skrrt doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'lil-skrrt model' to choose a "
...
# portal.github.com/SkrrtSkerrt/hermes-agent — sending it anywhere else would leak it.
#
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `lil-skrrt model` walks users into a broken Qwen setup flow.
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `lil-skrrt auth list` / `lil-skrrt status`."""
    try:
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but Lil Skrrt times out") can produce
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>lil-skrrt auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `lil-skrrt auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `lil-skrrt auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `lil-skrrt auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `lil-skrrt auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    # Persist so subsequent `lil-skrrt auth spotify` runs skip the wizard.
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize Lil Skrrt:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Lil Skrrt is waiting for the OAuth callback on {redirect_uri}")
...
#
# Lil Skrrt maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Lil Skrrt auth store (~/.hermes/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `lil-skrrt auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `lil-skrrt auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `lil-skrrt auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `lil-skrrt auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Lil Skrrt auth store (~/.hermes/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Lil Skrrt auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `lil-skrrt auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Lil Skrrt auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Lil Skrrt's own Codex token store."""
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other Lil Skrrt processes
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `lil-skrrt model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `lil-skrrt model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `lil-skrrt model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `lil-skrrt model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `lil-skrrt model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `lil-skrrt model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Lil Skrrt (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `lil-skrrt model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``lil-skrrt model``
...
# so a new `lil-skrrt --profile <name> auth add nous --type oauth` can one-tap
...
def _nous_shared_store_path() -> Path:
    ...
    # real user's Lil Skrrt root, refuse rather than silently corrupt cross-profile
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Lil Skrrt's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Lil Skrrt's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another Lil Skrrt install sharing "
            "~/.hermes/auth.json) called POST /api/oauth/token with Lil Skrrt's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only Lil Skrrt may call the "
            "refresh endpoint. For health checks, use `lil-skrrt auth status` "
            "instead.\n"
            "Re-authenticate with: lil-skrrt auth add nous"
        )
        relogin = True
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Lil Skrrt models — they're not reliable for agentic tool-calling
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Lil Skrrt is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``lil-skrrt auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``lil-skrrt auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `lil-skrrt auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Lil Skrrt is not logged into Nous Portal.",
...
# which does a synchronous OAuth refresh POST to portal.github.com/SkrrtSkerrt/hermes-agent. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`lil-skrrt tools`, status panels,
# subscription-feature checks) call it many times per render — `lil-skrrt tools` → "All Platforms"
# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning
# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json
# mtime so that `lil-skrrt auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `lil-skrrt auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `lil-skrrt auth` and
    # `lil-skrrt model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``lil-skrrt doctor`` runs the live
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Lil Skrrt' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `lil-skrrt doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `lil-skrrt logout` historically keyed off auth.json.active_provider only.
...
def login_command(args) -> None:
    """Deprecated: use 'lil-skrrt model' or 'lil-skrrt setup' instead."""
    print("The 'lil-skrrt login' command has been removed.")
    print("Use 'lil-skrrt auth' to manage credentials,")
    print("'lil-skrrt model' to select a provider, or 'lil-skrrt setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    # Check for existing Lil Skrrt-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Lil Skrrt auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Lil Skrrt will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("Lil Skrrt will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Lil Skrrt gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Lil Skrrt creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Lil Skrrt auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Lil Skrrt auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Lil Skrrt creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=hermes-agent` lets xAI attribute Lil Skrrt-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in Lil Skrrt — please report at "
            "https://github.com/SkrrtSkerrt/hermes-agent/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize Lil Skrrt with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Lil Skrrt auth store (~/.hermes/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Lil Skrrt login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `lil-skrrt model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``lil-skrrt status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Lil Skrrt login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `lil-skrrt model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Lil Skrrt builds still see
        ...
        if not selected_model:
            ...
            print("  Run `lil-skrrt model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Lil Skrrt will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `lil-skrrt model` or configure an API key to use Lil Skrrt.")
        else:

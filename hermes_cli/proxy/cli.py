"""CLI handlers for the ``lil-skrrt proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "lil-skrrt proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    print(
        f"Starting Lil Skrrt proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Lil Skrrt proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: lil-skrrt proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``lil-skrrt proxy <subcommand>``."""
    ...
    print(
        "lil-skrrt proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  lil-skrrt proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"

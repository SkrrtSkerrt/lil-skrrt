    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Lil Skrrt" comment and following line
                if '# Lil Skrrt' in line or '# lil-skrrt' in line:
                    skip_next = True
                    continue
...
def run_uninstall(args):
    ...
    print(color("│            ☠︎ Lil Skrrt Uninstaller                     │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Lil Skrrt data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Lil Skrrt code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Lil Skrrt-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No Lil Skrrt-set User env vars to remove")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/SkrrtSkerrt/hermes-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/SkrrtSkerrt/hermes-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()

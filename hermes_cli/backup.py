"""
Backup and import commands for lil-skrrt CLI.

`lil-skrrt backup` creates a zip archive of the entire ~/.hermes/ directory
(excluding the hermes-agent repo and transient files).

`lil-skrrt import` restores from a backup zip, overlaying onto the current
HERMES_HOME root.
"""
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to lil-skrrt root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Lil Skrrt home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Lil Skrrt home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    print(f"\nRestore with: lil-skrrt import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Lil Skrrt backup.
    ...
    # Look for telltale files that a lil-skrrt home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Lil Skrrt backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a lil-skrrt dir name
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Lil Skrrt configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: lil-skrrt profile list  (after installing hermes)")

        # Guidance
        print()
        if not (hermes_root / "hermes-agent").is_dir():
            print("Note: The hermes-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: lil-skrrt update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  lil-skrrt -p {pname} gateway install")

        print("Done. Your Lil Skrrt configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and lil-skrrt backup --quick)
# ---------------------------------------------------------------------------
...
# — `lil-skrrt update` snapshots this set before pulling so approved-user lists
...
def run_quick_backup(args) -> None:
    """CLI entry point for lil-skrrt backup --quick."""
...
def create_pre_update_backup(
    ...
    (``lil-skrrt update``) should continue even if the backup fails.
    """
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `lil-skrrt claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    # Reuses the shared backups/ directory so `lil-skrrt import` and the

...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Lil Skrrt'
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Lil Skrrt' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Lil Skrrt for
        ...
        try:
            ...
            # Permissions + Lil Skrrt tool callback are always-on production
            ...
            if "hermes-tools" in mig_report.migrated:
                msg_lines.append(
                    "Lil Skrrt tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Lil Skrrt runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Lil Skrrt tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Lil Skrrt runtime.")

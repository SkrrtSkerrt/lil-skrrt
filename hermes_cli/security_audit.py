"""On-demand supply-chain audit for Lil Skrrt Agent installs.

Scans three surfaces a Lil Skrrt user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Lil Skrrt venv (every PyPI dist via ``importlib.metadata``).

"""``lil-skrrt debug`` debug tools for Lil Skrrt Agent.

Currently supports:
    lil-skrrt debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[lil-skrrt debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    ...
    ran ``lil-skrrt debug share`` repeatedly.

    Deletion is now driven by the gateway's cron ticker
    (``gateway/run.py::_start_cron_ticker``) which calls
    ``_sweep_expired_pastes`` once per hour.  ``lil-skrrt debug share`` also
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``lil-skrrt debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``lil-skrrt debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Lil Skrrt version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `lil-skrrt debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``lil-skrrt debug share`` invocation added ~20 MB of resident Python
    ...
    ``lil-skrrt debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"lil-skrrt debug delete {url}"
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----Lil SkrrtDebugBoundary9f3c"
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``lil-skrrt debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``lil-skrrt dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``lil-skrrt dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "lil-skrrt debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  lil-skrrt debug delete <url>")

    print(f"\nShare these links with the Lil Skrrt team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: lil-skrrt debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'lil-skrrt debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``lil-skrrt debug`` call.
    ...
    # best-effort — any failure is swallowed so ``lil-skrrt debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: lil-skrrt debug <command>")
        print()
        print("Commands:")

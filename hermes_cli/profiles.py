"""
Profile management for multiple isolated Lil Skrrt instances.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Lil Skrrt infrastructure directories that only the default profile
...
# Lil Skrrt subcommands that cannot be used as profile names/aliases
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Lil Skrrt installation itself or a common system binary.  "
...
# ``config.yaml``. ``config.yaml`` is the user-facing Lil Skrrt config

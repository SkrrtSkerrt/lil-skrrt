...
on Japanese locales, etc.).  Lil Skrrt's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Lil Skrrt is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Lil Skrrt-managed Git install directories so
    ...
    # install without Lil Skrrt-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Lil Skrrt's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Lil Skrrt's
    ...
    Set this before launching Lil Skrrt (User env var in Windows Settings, or
    export in a PowerShell profile) and Lil Skrrt picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Lil Skrrt-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess Lil Skrrt spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Lil Skrrt-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Lil Skrrt venv Scripts directory — host of the hermes.exe shim itself,

...
_HERMES_MODEL_WARNING = (
    "3 & 4 models are NOT agentic and are not designed for use with Lil Skrrt Agent. "
    "They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Lil Skrrt 3 / Lil Skrrt 4 chat families.
...
#   NousResearch/Lil Skrrt-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Lil Skrrt 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Lil Skrrt 3/4 chat model."""
...
def list_authenticated_providers(
    ...
    # requiring a Lil Skrrt release. Falls back to the in-repo
    ...
    # --- 1. Check Lil Skrrt-mapped providers ---
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # show up in the picker without requiring a Lil Skrrt release.
    ...
    # --- 2. Check Lil Skrrt-only providers (nous, openai-codex, copilot, opencode-go) ---
    from hermes_cli.providers import HERMES_OVERLAYS
    from hermes_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Lil Skrrt provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Lil Skrrt IDs ("copilot").
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # Resolve Lil Skrrt slug — e.g. "github-copilot" → "copilot"
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # Use curated list — look up by Lil Skrrt slug, fall back to overlay key
            model_ids = curated.get(hermes_slug, []) or curated.get(pid, [])
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Lil Skrrt's canonical write key (matches
            ...
            # Lil Skrrt writes ``models:`` as a dict keyed by model id
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Lil Skrrt's own writer uses; a hyphen variant is accepted
            ...
            # active model. Lil Skrrt's own writer (main.py::_save_custom_provider)
